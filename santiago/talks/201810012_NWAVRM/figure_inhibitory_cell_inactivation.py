...
if PANELS[2]:
    ...
    axBars = plt.subplot(gs[:2,0])
    meanEachCond = [noPVsupDiff.mean(), noSOMsupDiff.mean()]
    semEachCond = [noPVsupDiff.std()/np.sqrt(len(noPVsupDiff)),
                   noSOMsupDiff.std()/np.sqrt(len(noSOMsupDiff))]
    posEachCond = [0,1]
    for cond in [0,1]:
        pBar = plt.bar(posEachCond[cond],meanEachCond[cond], ec=cellTypeColours[cond], fc='w', lw=3, align='center')
        pErr = plt.errorbar(posEachCond[cond],meanEachCond[cond], yerr=semEachCond[cond], color=cellTypeColours[cond])
    plt.xlim([-1,2])
    plt.axhline(0,color='k')
    extraplots.boxoff(axBars)
    yLims = plt.ylim()
    extraplots.significance_stars(posEachCond, yLims[1]*1, yLims[1]*0.05, gapFactor=0.2)
    axBars.set_xticks(posEachCond)
    axBars.set_xticklabels(['no PV','no SOM'], rotation=45, fontsize=fontSizeLabels+2)
    plt.ylim(yLims)
    plt.ylabel('Change in supression index', fontsize=fontSizeLabels+2)
