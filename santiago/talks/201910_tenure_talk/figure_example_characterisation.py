...
figSize = [11,8] # In inches

fontSizeLabels = 11 #figparams.fontSizeLabels
fontSizeTicks = 11 #figparams.fontSizeTicks
...
if PANELS[1]: 
    ...
    for indCell, cell in enumerate(cellData):
        ...
        pRaster, hcond, zline = extraplots.raster_plot(bandSpikeTimesFromEventOnset,
                                                       bandIndexLimitsEachTrial,rasterTimeRange,
                                                       trialsEachCond=trialsEachCond,labels=bandLabels,
                                                       colorEachCond=colorEachCond)
        axRaster.annotate(panelLabels[indCell], xy=(labelPosX[1],labelPosY[indCell]),
                          xycoords='figure fraction',fontsize=fontSizePanel, fontweight='bold')
        plt.setp(pRaster, ms=2, color='k')
        ...
        rect = patches.Rectangle((0.0,yLims[1]*1.02),1.0,yLims[1]*0.04,linewidth=1,
                                 edgecolor=soundColor,facecolor=soundColor,clip_on=False)
...
#                          responseByCell + sustainedErrors[indCell], alpha=0.2, color=cellTypeColours[indCell], edgecolor='none')
        #plt.plot([bands[0],bands[-1]], np.tile(responseByCell[0], 2), '--', color='0.4', lw=1)
        plt.plot(plt.xlim(), np.tile(responseByCell[0], 2), '--', color='0.5', lw=1.5)
