...
SAVE_FIGURE = 1
outputDir = '/tmp/'
figFilename = 'Fig_summary_inactivation_on_suppression' # Do not include extension
...
gs = gridspec.GridSpec(1,4,width_ratios=[1, 1, 0.5, 1], height_ratios=[1])
...
#summaryFileName = 'all_inactivated_cells_stats_full.npz' # With new cells after Sept 2019
...
if PANELS[2]:
    ...
    EXCLUDE_NO_SUPP = 0 # Ignore cells with no suppression during control condition
    AS_PERCENT = 1 # Calculate change as percentage
    if EXCLUDE_NO_SUPP:
        noPVsupDiff = noPVsupDiff[PVsustainedSuppressionNoLaser>0]
        noSOMsupDiff = noSOMsupDiff[SOMsustainedSuppressionNoLaser>0]
    if AS_PERCENT:
        noPVsupDiff = 100*noPVsupDiff/np.mean(PVsustainedSuppressionNoLaser)
        noSOMsupDiff = 100*noSOMsupDiff/np.mean(SOMsustainedSuppressionNoLaser)
    ...
    axInset = plt.subplot(gs[0,2])
  
    width = 0.6
    loc = np.arange(1,3)

    SIMedians = [np.median(noPVsupDiff), np.median(noSOMsupDiff)]
    #SIMedians = [np.mean(noPVsupDiff), np.mean(noSOMsupDiff)]
    ...
    #plt.axhline(0, color='0.75',ls = '--', zorder=-4)
    
    if AS_PERCENT:
        yLims = (-40,30)
        plt.ylim(yLims)
        axInset.set_yticks(np.arange(yLims[0],yLims[1]+1,10))
    else:
        yLims = (-0.2,0.1)
        plt.ylim(yLims)
    plt.xlim(0.5,2.5)
...
#     axInset.yaxis.tick_right()
    ...
    plt.ylabel(r'Change in suppression index (%)',fontsize=fontSizeLabels+2)
    axInset.set_xticks([1,2])
    axInset.set_xticklabels(['No\nPV', 'No\nSOM'],fontsize=fontSizeLabels+2)
    #axInset.yaxis.set_label_position('right')
    #plt.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
    extraplots.set_ticks_fontsize(axInset,fontSizeTicks)
