...
def block_data_cells(df,block):
    """Return only cells that actually have at least one finite response for this sound block.

    This is important for incomplete FOVs: cells from an FOV that never contained the
    requested block legitimately have all-NaN block-specific columns and must not be
    counted in block-specific "all cells" plots or rasters.
    """
    cols=[c for _,c in condition_columns(df,block)]
    if not cols:
        return df.iloc[0:0].copy()
    M=df[cols].apply(pd.to_numeric,errors="coerce").to_numpy(float)
    keep=np.any(np.isfinite(M),axis=1)
    return df.loc[keep].copy()

def responsive_only(df,block):
    base=block_data_cells(df,block)
    c=f"{SHORT[block]}_responsive_fdr"
    return base[base[c].fillna(False).astype(bool)].copy() if c in base else base.iloc[0:0].copy()
...
def plot_metric_family(df,block,area,layer_label,out,branch):
    use=block_data_cells(df,block) if branch=="all_cells" else responsive_only(df,block)
...
def plot_profile(df,block,area,layer_label,out,branch):
    use=block_data_cells(df,block) if branch=="all_cells" else responsive_only(df,block); cols=condition_columns(use,block)
    if not cols: return {}
...
def preference_distribution(df,block,responsive=True):
    ...
    use=responsive_only(df,block) if responsive else block_data_cells(df,block)
    if not responsive: use=use[use[metric].notna()].copy()
...
def plot_primary_selectivity_fov(df,block,area,layer_label,out,branch,statrec=None):
    spec=SELECTIVITY_SPEC[block]; metric=spec["metric"]; use=block_data_cells(df,block) if branch=="all_cells" else responsive_only(df,block)
    ...
    for branch in ('all_cells','responsive_only'):
        ...
        for sort_metric,suffix in ((f"{SHORT[block]}_preferred_condition",'PREFERRED'),(f"{SHORT[block]}_lifetime_sparseness",'SPARSENESS')):
            ...
            for ax,cls in zip(axes[0],CELL_CLASSES):
                ...
                if len(M):
                    valid_rows=np.any(np.isfinite(M),axis=1)
                    g=g.loc[valid_rows].copy(); M=M[valid_rows]
                if len(M):
...
def main():
    ...
    print("="*80); print("MULTISTIM AREA x LAYER GREEN/YELLOW ANALYSIS v4"); print("="*80)
