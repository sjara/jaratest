...
Example
-------
python sound_tuning_multistim_fov_analysis_v1.py imag039 \
...
def analyze_block(meta: dict[str, Any], plane: int, fov_reference: dict[str, Any],
    ...
    tr = _trial_response(ev, np.asarray(tvec, float))
    ...
    archive = out_dir / f"{subject}_{date}_{session}_p{plane}_{block}_trial_response_archive_v1.npz"
    np.savez_compressed(
        archive,
        schema_version=np.asarray("multistim_v1"), subject=np.asarray(subject), date=np.asarray(date), session=np.asarray(session),
    ...
    xplot = np.log2(values / 1000.0) if block == "pure_tones" else values
    xlabel = "Frequency (kHz; log2 spacing)" if block == "pure_tones" else (
        "Middle tone position (octaves above F0)" if block == "chord_tones" else "FM slope (octaves/s)")
    ...
    ax.set_xticks(xplot); ax.set_xticklabels(xticklabels, rotation=45 if len(values)>8 else 0)
    ...
    ax.set_xticks(np.arange(len(values))); ax.set_xticklabels(xticklabels, rotation=45 if len(values)>8 else 0)
...
def plot_combined_profiles(cells: pd.DataFrame, out_dir: Path) -> None:
    for block in BLOCK_ORDER:
        ...
        vals=np.array([v for v,_ in cols]); x=np.log2(vals/1000) if block=="pure_tones" else vals
        ...
        for ax,cls in zip(axes,CELL_CLASSES):
            ...
            elif block=="chord_tones": ax.set_xlabel("Middle tone octave above F0")
...
def write_report(path: Path, cells: pd.DataFrame, summary: pd.DataFrame, response_stats: dict[str,Any]) -> None:
    lines=["SOUND_TUNING MULTISTIM FOV ANALYSIS v1","="*72,"",
...
def main() -> None:
    ...
    print("="*86); print("SOUND_TUNING MULTISTIM FOV ANALYSIS v1"); print("="*86)
    ...
    save_json(od["root"]/"run_manifest.json",{
        "script_version":"v1","subject":subject,"dates":dates,"area":area,"plane":args.plane,
