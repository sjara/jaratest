#!/usr/bin/env python3
"""
Batch/area-level analysis of two-photon am_tuning sessions (robust-alignment v4).
...
* the first imaging sound event is anchored to the first behavioral trial;
...
Example
-------
python am_tuning_area_analysis_v4.py imag039 \
    --area A1 \
...
def align_events(data2p, eventlocked: np.ndarray, valid_events: np.ndarray, n_behavior_trials: int,
    ...
    Experimental acquisition order is used as a strong prior: the first imaging
    sound event corresponds to the first behavioral sound trial.  From there we
    walk forward through the two event sequences.
    ...
    # Acquisition protocol anchors the first sound event to the first behavior
    # trial.  All subsequent matching is monotonic.
    matched_imaging = [0]
    matched_behavior = [0]
