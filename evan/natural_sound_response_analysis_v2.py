...
Responsiveness
--------------
The PRIMARY natural-sound-responsive label is designed for natural-sound datasets
with relatively few repeats of each individual sound. For each cell, all valid
natural-sound trials are pooled and the mean baseline-corrected response is tested
against zero with a two-sided Monte-Carlo sign-flip test. BH-FDR is then applied
ONCE across cells. This answers the broad question: does this cell respond
consistently to natural sounds overall?

The script ALSO retains the stricter AM-v9-style condition-specific analysis:
each individual natural-sound condition is tested separately, BH-FDR is applied
across cells within that condition, and the union across conditions is saved as a
secondary label. This secondary test can have very low power when there are only
3-10 repeats per sound and is therefore NOT used as the primary cell filter for
the repeat-stability analysis.
...
The saved NPZ trial archive is the input to natural_sound_repeat_stability_v2.py,
...
Analyze every natural-sound session for imag039 on one date:
    ...
    python natural_sound_response_analysis_v2.py imag039 --dates 20260717
...
Inspect session selection without loading imaging data:
    ...
    python natural_sound_response_analysis_v2.py imag039 --dates 20260717 --dry-run
...
def trial_baseline_correct_and_scalar(
    ...
    trial_valid: np.ndarray | None = None,
    ...
    n_cells, n_trials, _ = data.shape
    if trial_valid is None:
        trial_valid = np.ones(n_trials, dtype=bool)
    else:
        trial_valid = np.asarray(trial_valid, dtype=bool).ravel()
        if len(trial_valid) != n_trials:
            raise ValueError(f"trial_valid has length {len(trial_valid)} but data contain {n_trials} trials.")

    # Avoid np.nanmean warnings from event-locked windows that are known to be
    # invalid (for example a final stimulus too close to the end of imaging).
    # We also compute means explicitly from finite counts so a cell-specific
    # all-NaN window remains NaN without emitting RuntimeWarning.
    baseline = np.full((n_cells, n_trials, 1), np.nan, dtype=float)
    valid_idx = np.flatnonzero(trial_valid)
    if len(valid_idx):
        bseg = data[:, valid_idx, :][:, :, baseline_mask]
        bcount = np.sum(np.isfinite(bseg), axis=2)
        bsum = np.nansum(bseg, axis=2)
        bmean = np.full_like(bsum, np.nan, dtype=float)
        np.divide(bsum, bcount, out=bmean, where=bcount > 0)
        baseline[:, valid_idx, 0] = bmean
    ...
    for ti in valid_idx:
        ...
        if not np.any(mask):
            continue
        rseg = bc[:, ti, :][:, mask]
        rcount = np.sum(np.isfinite(rseg), axis=1)
        rsum = np.nansum(rseg, axis=1)
        rmean = np.full(n_cells, np.nan, dtype=float)
        np.divide(rsum, rcount, out=rmean, where=rcount > 0)
        response[:, ti] = rmean
        used_end[ti] = end
...
def analyze_session(meta: dict[str, Any], args: argparse.Namespace, outdir: Path) -> dict[str, Any]:
    ...
    n_invalid_event_windows = int(np.sum(~np.asarray(valid_aligned, dtype=bool)))
    valid_trial_idx = np.flatnonzero(np.asarray(valid_aligned, dtype=bool))
    if len(valid_trial_idx):
        finite_by_trial = np.isfinite(trial_response[:, valid_trial_idx])
        n_all_missing = int(np.sum(~np.any(finite_by_trial, axis=0)))
        n_partial = int(np.sum(np.any(finite_by_trial, axis=0) & ~np.all(finite_by_trial, axis=0)))
    else:
        n_all_missing = 0
        n_partial = 0
    print(
        "Event-window QC: "
        f"invalid eventlocked windows={n_invalid_event_windows}; "
        f"valid windows with no finite primary responses={n_all_missing}; "
        f"valid windows with some cell-specific NaNs={n_partial}"
    )
    ...
    for ci, key in enumerate(unique_keys):
        ...
        if np.any(mask):
            ...
            xsum = np.nansum(x, axis=1)
            means = np.full(n_cells, np.nan, dtype=float)
            np.divide(xsum, nfinite, out=means, where=nfinite > 0)
            condition_means[:,ci] = means
        if np.sum(mask) >= args.min_repeats_for_variance:
            nfinite = np.sum(np.isfinite(x), axis=1)
            ok = nfinite >= max(2, args.min_repeats_for_variance)
            if np.any(ok):
                xc = x[ok] - condition_means[ok, ci, None]
                ss = np.nansum(xc * xc, axis=1)
                condition_vars[ok,ci] = ss / (nfinite[ok] - 1)
                condition_sds[ok,ci] = np.sqrt(condition_vars[ok,ci])
    ...
    # -------------------------------------------------------------------------
    # PRIMARY cell-level natural-sound responsiveness
    # -------------------------------------------------------------------------
    # Per-condition BH-FDR is retained as a strict secondary analysis, but with
    # only a few repeats per natural sound it can be severely underpowered.
    # The primary label therefore tests each cell once using all valid natural-
    # sound trials, then applies BH-FDR across cells.
    condition_resp = condition_q < FDR_ALPHA
    condition_union_resp = np.any(condition_resp, axis=1)
    condition_exc = condition_resp & (condition_means > 0)
    condition_sup = condition_resp & (condition_means < 0)

    global_x = trial_response[:, analysis_trials]
    global_mean = np.full(n_cells, np.nan, dtype=float)
    global_n = np.sum(np.isfinite(global_x), axis=1)
    global_sum = np.nansum(global_x, axis=1)
    np.divide(global_sum, global_n, out=global_mean, where=global_n > 0)
    global_p = amv9.signflip_pvalues_grouped(global_x, args.n_permutations, rng)
    global_q = amv9.bh_fdr(global_p)
    natural_resp = global_q < FDR_ALPHA
    natural_exc = natural_resp & (global_mean > 0)
    natural_sup = natural_resp & (global_mean < 0)

    print(
        f"Natural-sound responsiveness: primary global FDR={int(np.sum(natural_resp))}/{n_cells} cells; "
        f"strict condition-specific union={int(np.sum(condition_union_resp))}/{n_cells} cells"
    )
    ...
    for cell in range(n_cells):
        row: dict[str,Any] = {
            ...
            "natural_suppressed_fdr":bool(natural_sup[cell]),
            "natural_global_mean_response":float(global_mean[cell]) if np.isfinite(global_mean[cell]) else np.nan,
            "natural_global_p_signflip":float(global_p[cell]) if np.isfinite(global_p[cell]) else np.nan,
            "natural_global_q_bh":float(global_q[cell]) if np.isfinite(global_q[cell]) else np.nan,
            "natural_condition_union_responsive_fdr":bool(condition_union_resp[cell]),
            "n_responsive_conditions":int(np.sum(condition_resp[cell])),
    ...
    for ci,(key,label) in enumerate(zip(unique_keys,labels)):
        ...
        for cell in range(n_cells):
            ...
            cond_rows.append({
                ...
                "cell_natural_global_mean_response":float(global_mean[cell]) if np.isfinite(global_mean[cell]) else np.nan,
                "cell_natural_global_p_signflip":float(global_p[cell]) if np.isfinite(global_p[cell]) else np.nan,
                "cell_natural_global_q_bh":float(global_q[cell]) if np.isfinite(global_q[cell]) else np.nan,
                "cell_natural_condition_union_responsive_fdr":bool(condition_union_resp[cell]),
    ...
    np.savez_compressed(
        ...
        schema=np.asarray("natural_sound_response_v2"), subject=np.asarray(subject), date=np.asarray(date), session=np.asarray(session),
        ...
        cell_natural_global_mean_response=global_mean.astype(np.float32),
        cell_natural_global_p_signflip=global_p.astype(np.float32),
        cell_natural_global_q_bh=global_q.astype(np.float32),
        cell_natural_condition_union_responsive_fdr=condition_union_resp.astype(bool),
    ...
    for cls in ("green","yellow"):
        ...
        class_summary.append({
            ...
            "n_condition_union_responsive":int(sub.natural_condition_union_responsive_fdr.sum()),
            "pct_condition_union_responsive":100*float(sub.natural_condition_union_responsive_fdr.mean()) if len(sub) else np.nan,
    ...
    summary = {
        ...
        "n_natural_responsive_global_fdr":int(np.sum(natural_resp)),
        "n_natural_responsive_condition_union_fdr":int(np.sum(condition_union_resp)),
        "primary_responsiveness":"global sign-flip using all valid natural-sound trials per cell; BH-FDR once across cells",
        "secondary_condition_responsiveness":"condition-specific sign-flip; BH-FDR across cells within each condition; union across conditions",
...
def main() -> None:
    ...
    print("="*80)
    print("NATURAL-SOUND RESPONSE / REPEAT-VARIABILITY ANALYSIS v1b")
    print("="*80)
