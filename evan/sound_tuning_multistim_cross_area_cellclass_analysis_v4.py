...
v4 = v3 (signed log2-magnitude FM axes, dedicated selectivity figures) plus the
v2b fixes for incomplete FOVs: cells from FOVs that never contained a sound block
(all-NaN block columns) are excluded from that block's plots, rasters, and stats.
...
def responsive(df,b):
 base=block_data_cells(df,b)
 c=f"{SHORT[b]}_responsive_fdr"; return base[base[c].fillna(False).astype(bool)].copy() if c in base else base.iloc[0:0].copy()
...
def condition_cols(df,b):
 ...
 return sorted(out)
def block_data_cells(df,b):
    """Keep only cells with at least one finite response for the requested sound block."""
    cols=[c for _,c in condition_cols(df,b)]
    if not cols:
        return df.iloc[0:0].copy()
    M=df[cols].apply(pd.to_numeric,errors="coerce").to_numpy(float)
    return df.loc[np.any(np.isfinite(M),axis=1)].copy()
...
def preference_table(df,b,resp_only=True):
 ...
 use=responsive(df,b) if resp_only else block_data_cells(df,b)
 if not resp_only: use=use[use[m].notna()].copy()
...
def plot_metric_cell_points(cells,b,branch,areas,cls,depth,out):
 use=block_data_cells(cells,b) if branch=='all_cells' else responsive(cells,b); avail=[(m,l) for m,l in METRICS[b] if m in use and np.any(np.isfinite(pd.to_numeric(use[m],errors='coerce')))]
 if not avail:return
...
def plot_profile_cells(cells,b,branch,areas,cls,depth,out):
 use=block_data_cells(cells,b) if branch=='all_cells' else responsive(cells,b); cols=condition_cols(use,b)
 if not cols:return
...
def build_stats(cells,areas):
 ...
 for b in BLOCKS:
  ...
  for branch in ("all_cells","responsive_only"):
   use=block_data_cells(cells,b) if branch=="all_cells" else responsive(cells,b); fam=[]
...
def plot_primary_selectivity_cross_area_cells(cells,b,branch,areas,cls,depth,out):
 spec=SELECTIVITY_SPEC[b]; metric=spec["metric"]; use=block_data_cells(cells,b) if branch=="all_cells" else responsive(cells,b)
...
def main():
 ...
 print("="*82); print("MULTISTIM CROSS-AREA CELL-CLASS ANALYSIS v4"); print("="*82)
 ...
 save_json(root/"run_manifest.json",{"version":"v4","subject":subject,"dates":dates,"areas":areas,"primary_replicate":"FOV","trial_reduction":{"enabled":not args.skip_trial_reduction,"fractions":TRIAL_FRACTIONS,"resamples":args.trial_reduction_resamples,"all_individual_significant_primary_YELLOW_results":True}})
 ...
 if "pure_best_frequency_hz" in allc:
  # pd.concat across many area/FOV tables can leave a highly fragmented DataFrame.
  # Make one contiguous copy before adding derived columns to avoid PerformanceWarning.
  allc=allc.copy()
  ...
  allc=allc.assign(pure_best_frequency_log2_hz=np.where(mask,np.log2(bf),np.nan))
 for cls in CELL_CLASSES:
