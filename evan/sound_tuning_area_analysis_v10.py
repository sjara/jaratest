...
Example
-------
python sound_tuning_area_analysis_v10.py imag039 \
    --area A1 \
...
def analyze_session(
    ...
    standalone_cell_selectivity_figure(
        cell_df=cell_df,
        metric="am_rate_lifetime_sparseness",
        ylabel="AM-rate lifetime sparseness",
        title=f"{title_suffix}\nAM-rate selectivity",
        equation_text=r"$S_{life}=\frac{1-(\bar R^2/\overline{R^2})}{1-1/N}$",
        output_path=session_output_dir / f"{subject}_{date}_{session}_p{plane}_am_rate_selectivity_green_yellow.png",
        reference=None,
    )
...
def standalone_cell_selectivity_figure(
    ...
    """Readable per-session GREEN/YELLOW selectivity plot with cells as points."""
    df = cell_df[cell_df["cell_class"].isin(["green", "yellow"])].copy()
    ...
    for pos, class_name, color in [(0, "green", GREEN_COLOR), (1, "yellow", YELLOW_COLOR)]:
        vals = finite_values(df.loc[df["cell_class"] == class_name, metric])
        if len(vals):
            ...
            ax.scatter(
                ...
                color=color,
                alpha=0.34,
                edgecolors="black" if class_name == "yellow" else "none",
                linewidths=0.25,
                zorder=2,
                label=f"{class_name.capitalize()} cells (n={len(vals)})",
            )
            mean_value, _, sem_value, n = mean_sd_sem(vals)
            ax.errorbar(
                pos,
                mean_value,
            ...
            if np.isfinite(mean_value):
                ...
                if np.isfinite(sem_value):
                    display.extend([mean_value-sem_value, mean_value+sem_value])
    ...
    ax.set_title(title + "\nIndividual cells; large point = mean ± SEM", fontsize=13, pad=12)
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    if display:
        apply_robust_y_limits(
    ...
    fig.subplots_adjust(bottom=0.16, top=0.87, left=0.15, right=0.97)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    # Standalone selectivity summaries requested for readability. These retain
    # all existing combined figures and use sessions (not cells) as points.
    # -------------------------------------------------------------------------
    standalone_session_selectivity_figure(
        ...
        stat_key="fade_selectivity_green_vs_yellow",
        reference=0.0,
    )
    standalone_session_selectivity_figure(
        cell_df=cell_df,
        ...
        title=title + "\nAM-rate selectivity",
        equation_text=r"$S_{life}=\frac{1-(\bar R^2/\overline{R^2})}{1-1/N}$",
        output_path=combined_dir / "combined_am_rate_selectivity_green_yellow_SESSION_POINTS.png",
        session_stats=session_stats,
        stat_key="am_rate_lifetime_sparseness_green_vs_yellow",
        reference=None,
    )
