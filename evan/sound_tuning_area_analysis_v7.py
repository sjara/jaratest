...
def save_session_selective_example_figures(
    ...
    for class_name, class_color in [("green", GREEN_COLOR), ("yellow", YELLOW_COLOR)]:
        ...
        for rank, (_, row) in enumerate(cdf.iterrows(), start=1):
            ...
            # Trial-level scalar response version. Each point is the same response
            # amplitude used by the main analysis: mean baseline-corrected dF/F
            # over the 0-1 s response window.
...
def analyze_session(
    ...
    # scalar response-amplitude figure using the same 0-1 s mean metric as the analysis.
