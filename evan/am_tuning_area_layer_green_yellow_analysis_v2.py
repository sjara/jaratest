...
This is the third-stage follow-up analysis for am_tuning_area_analysis_v7.py.
...
Legacy best-vs-rest contrast/index outputs are retained as secondary metrics.

All real frequency / AM-rate axes are plotted on a base-2 logarithmic x-axis
when the tuning values are positive. Categorical GREEN/YELLOW axes remain linear.
...
These are produced by am_tuning_area_analysis_v7.py.
...
CONTINUOUS_METRICS = {
    ...
    # Primary and comparison tuning-selectivity metrics from am_tuning_area_analysis_v7.py.
...
def apply_log2_tuning_axis(ax, values_hz: Iterable[Any], mode: str) -> None:
    ...
    if hi > lo:
        margin_oct = 0.12
        ax.set_xlim(lo / (2**margin_oct), hi * (2**margin_oct))
    else:
...
def log2_bin_edges(values_hz: Iterable[Any], mode: str) -> np.ndarray | None:
    ...
    if len(x) == 0 or np.any(~np.isfinite(x)) or np.any(x <= 0):
        return None
    if len(x) == 1:
        return np.asarray([x[0] / np.sqrt(2), x[0] * np.sqrt(2)], dtype=float)
    lx = np.log2(x)
    mids = 0.5 * (lx[:-1] + lx[1:])
    edges_l = np.empty(len(x) + 1, dtype=float)
    edges_l[1:-1] = mids
    edges_l[0] = lx[0] - (mids[0] - lx[0])
    edges_l[-1] = lx[-1] + (lx[-1] - mids[-1])
    return 2.0 ** edges_l
...
def aggregate_sessions(df: pd.DataFrame, mode: str) -> pd.DataFrame:
    """One row per session x class; continuous entries are means across cells.

    SD columns are retained so plots can show light within-session mean +/- SD,
    while overall uncertainty is SEM across session means.
    """
    rows = []
...
def _plot_session_metric_panel(ax, session_df: pd.DataFrame, metric: str, mode: str, connect_pairs: bool = True) -> None:
    ...
    offsets = np.linspace(-0.05, 0.05, max(len(session_keys), 1))
    ...
    for si, sk in enumerate(session_keys):
        ...
        for xi, cls in zip(x, CELL_CLASSES):
            ...
            if len(sdf)==0 or metric not in sdf:
                continue
            m = pd.to_numeric(pd.Series([sdf.iloc[0][metric]]), errors="coerce").iloc[0]
            sdcol = f"sd_{metric}"
            sd = pd.to_numeric(pd.Series([sdf.iloc[0][sdcol]]), errors="coerce").iloc[0] if sdcol in sdf else np.nan
            if not np.isfinite(m):
                continue
            session_means[cls].append(float(m)); pairs[sk][cls]=float(m)
            xx = xi + offsets[min(si, len(offsets)-1)]
            ax.errorbar(xx, m, yerr=sd if np.isfinite(sd) else None, fmt="o", ms=3.8,
                        elinewidth=.8, capsize=2, lw=0, color=CELL_CLASS_COLORS[cls], alpha=.30, zorder=2)
            display.append(float(m))
            if np.isfinite(sd): display.extend([float(m-sd), float(m+sd)])
    if connect_pairs:
        for p in pairs.values():
            if np.isfinite(p.get("green",np.nan)) and np.isfinite(p.get("yellow",np.nan)):
                ax.plot(x,[p["green"],p["yellow"]],color="0.55",alpha=.20,lw=.75,zorder=1)
    overall=[]; overall_se=[]
    for xi, cls in zip(x, CELL_CLASSES):
        ...
        if np.isfinite(m):
            ax.errorbar(xi,m,yerr=se if np.isfinite(se) else None,fmt="o",ms=10,
                        elinewidth=2.6,capsize=6,lw=0,color=CELL_CLASS_COLORS[cls],zorder=6)
            txt=f"{m:.3g}" + (f" +/- {se:.2g}" if np.isfinite(se) else "") + f"\nn={n} sessions"
            ax.annotate(txt,(xi,m),xytext=(0,11),textcoords="offset points",ha="center",va="bottom",
                        fontsize=7,color=CELL_CLASS_COLORS[cls])
    if np.all(np.isfinite(overall)):
        ax.plot(x,overall,color="black",lw=2.0,alpha=.7,zorder=4)
    ref=CONTINUOUS_METRICS[metric]["reference"]
    if ref is not None: ax.axhline(ref,color="0.45",ls="--",lw=1)
    ax.set_xticks(x,["Green","Yellow"]); ax.set_ylabel(metric_label(metric,mode)); ax.set_title(metric_label(metric,mode))
    ax.grid(axis="y",alpha=.22); ax.spines[["top","right"]].set_visible(False)
    axis_vals=np.asarray(display + [v for pair in zip(overall,overall_se) for v in ([pair[0]] if np.isfinite(pair[0]) else [])],float)
    if len(axis_vals) and np.any(np.isfinite(axis_vals)):
        ylim=optimized_ylim_from_summary(axis_vals,None,[ref] if ref is not None else None)
...
def plot_tuning_profile_session(cell_df: pd.DataFrame, area: str, layer_label: str, mode: str, outdir: Path, suffix: str) -> None:
    """Light session mean +/- within-session SD; dark overall mean +/- SEM."""
    ...
    for cls in CELL_CLASSES:
        color=CELL_CLASS_COLORS[cls]; c_sessions=sdf[sdf.cell_class==cls]; session_profiles=[]
        for _,row in c_sessions.iterrows():
            means=np.asarray([row.get(c,np.nan) for c in cols],float)
            sds=np.asarray([row.get(f"sd_{c}",np.nan) for c in cols],float)
            session_profiles.append(means)
            ax.plot(x,means,color=color,alpha=.24,lw=1.0,marker="o",ms=2.5,zorder=1)
            if np.any(np.isfinite(sds)):
                ax.fill_between(x,means-sds,means+sds,color=color,alpha=.035,zorder=0)
            display.extend(finite_values(means).tolist())
            display.extend(finite_values(np.r_[means-sds,means+sds]).tolist())
        if session_profiles:
            smat=np.vstack(session_profiles); overall=np.nanmean(smat,axis=0)
            sem=np.asarray([stats.sem(smat[:,i],nan_policy="omit") if np.sum(np.isfinite(smat[:,i]))>1 else np.nan for i in range(len(cols))])
            ax.errorbar(x,overall,yerr=sem,fmt="o-",color=color,lw=2.8,ms=5.5,elinewidth=2.0,capsize=4,
                        label=f"{cls.capitalize()} overall ({len(session_profiles)} sessions)",zorder=5)
    ax.axhline(0,color="0.45",ls="--",lw=1); ax.set_xlabel(mode_axis_label(mode)); ax.set_ylabel("Session mean response (dF/F)")
    apply_log2_tuning_axis(ax,values,mode)
    ax.set_title(f"{area} — {layer_label} — {mode_title(mode)}\nlight=session mean +/- SD; dark=overall mean +/- SEM — {suffix.replace('_',' ')}")
    ax.legend(frameon=False); ax.grid(axis="y",alpha=.25); ax.spines[["top","right"]].set_visible(False)
    fig.tight_layout()
    # Current semantics plus backward-compatible session filename.
    fig.savefig(outdir/f"tuning_profile_{suffix}_MEAN_SEM_ONLY.png",dpi=DPI,bbox_inches="tight")
    fig.savefig(outdir/f"tuning_profile_{suffix}_SESSION_MEAN_SEM_ONLY.png",dpi=DPI,bbox_inches="tight")
    plt.close(fig)
...
def plot_preferred_distribution(cell_df: pd.DataFrame, area: str, layer_label: str, mode: str, outdir: Path, suffix: str) -> None:
    ...
    # Two narrow multiplicative offsets preserve grouping on a log2 axis.
    offset=2**0.055
    for cls,factor in (("green",1/offset),("yellow",offset)):
        ...
        ax.plot(x*factor,pct,"o",ms=7,color=CELL_CLASS_COLORS[cls],label=cls.capitalize())
...
def plot_standalone_selectivity_figures(cell_df: pd.DataFrame, session_df: pd.DataFrame, stats_out: dict[str,Any],
    ...
    """Separate cell-point and session-point figures for all four selectivity measures."""
    rng=np.random.default_rng(9917)
    for metric in SELECTIVITY_METRICS:
        if metric not in cell_df: continue
        label=metric_label(metric,mode); ref=CONTINUOUS_METRICS[metric]["reference"]
        stem=metric.replace("tuning_","")
        # Cells.
        fig,ax=plt.subplots(figsize=(7,6.3)); display=[]
        for xi,cls in enumerate(CELL_CLASSES):
            vals=finite_values(cell_df.loc[cell_df.cell_class==cls,metric])
            if len(vals):
                ax.scatter(np.full(len(vals),xi)+rng.uniform(-.09,.09,len(vals)),vals,s=22,alpha=.23,
                           color=CELL_CLASS_COLORS[cls],edgecolors="none")
                m,_,se,n=mean_sd_sem(vals)
                ...
                txt=f"{m:.3g}"+(f" +/- {se:.2g}" if np.isfinite(se) else "")+f"\nn={n} cells"
                ax.annotate(txt,(xi,m),xytext=(0,11),textcoords="offset points",ha="center",fontsize=8,color=CELL_CLASS_COLORS[cls])
                display.extend(vals.tolist())
        ...
        ax.set_title(f"{area} — {layer_label} — {mode_title(mode)}\n{label}\nindividual cells; exploratory pooled p={p:.3g}" if np.isfinite(p) else f"{area} — {layer_label} — {mode_title(mode)}\n{label}\nindividual cells")
        ...
        p=primary.get("p",np.nan); n=primary.get("n_paired_sessions",0)
        ax.set_title(f"{area} — {layer_label} — {mode_title(mode)}\n{label}\nlight=session mean +/- SD; dark=overall mean +/- SEM\npaired Wilcoxon p={p:.3g}, n={n}" if np.isfinite(p) else f"{area} — {layer_label} — {mode_title(mode)}\n{label}\nsession points")
        ax.text(.5,-.15,selectivity_equation_footer(mode),transform=ax.transAxes,ha="center",va="top",fontsize=7.5,wrap=True)
...
def plot_responsiveness(cell_df: pd.DataFrame, session_df: pd.DataFrame, area: str, layer_label: str, mode: str, outdir: Path) -> None:
    metrics=[(m,p,l) for m,(p,l) in BINARY_METRICS.items()]; x=np.arange(len(metrics),dtype=float); offsets={"green":-.16,"yellow":.16}
...
def plot_intensity_condition_grid(cond_df: pd.DataFrame, area: str, layer_label: str, mode: str, outdir: Path, suffix: str,
    ...
    """Session-aware intensity tuning: light session mean +/- SD; dark overall +/- SEM."""
    if len(cond_df)==0: return
    _plot_intensity_condition_grid_pooled(cond_df,area,layer_label,mode,outdir,suffix,metric,label,filename)
    values=np.sort(pd.to_numeric(cond_df.tuning_value_Hz,errors="coerce").dropna().unique()); intensities=np.sort(pd.to_numeric(cond_df.intensity_dB,errors="coerce").dropna().unique())
    if len(values)==0 or len(intensities)==0: return
    ...
    # session x class x condition summaries
    grouped=(cond_df.groupby(["session_key","cell_class","tuning_value_Hz","intensity_dB"],dropna=False)[metric]
             .agg(["mean","std"]).reset_index())
    for ax,intensity in zip(axes,intensities):
        for cls in CELL_CLASSES:
            csub=grouped[(grouped.cell_class==cls)&np.isclose(grouped.intensity_dB,intensity)]; color=CELL_CLASS_COLORS[cls]
            session_profiles=[]
            for sk,sdf in csub.groupby("session_key",sort=True):
                means=[]; sds=[]
                for value in values:
                    row=sdf[np.isclose(sdf.tuning_value_Hz,value)]
                    means.append(float(row.iloc[0]["mean"]) if len(row) else np.nan); sds.append(float(row.iloc[0]["std"]) if len(row) else np.nan)
                means=np.asarray(means,float); sds=np.asarray(sds,float); session_profiles.append(means)
                ax.plot(x,means,color=color,alpha=.22,lw=1,marker="o",ms=2.3)
                if np.any(np.isfinite(sds)): ax.fill_between(x,means-sds,means+sds,color=color,alpha=.03)
            if session_profiles:
                smat=np.vstack(session_profiles); overall=np.nanmean(smat,axis=0)
                sem=np.asarray([stats.sem(smat[:,i],nan_policy="omit") if np.sum(np.isfinite(smat[:,i]))>1 else np.nan for i in range(len(values))])
                ax.errorbar(x,overall,yerr=sem,fmt="o-",ms=5,lw=2.6,elinewidth=1.8,capsize=3,color=color,label=cls.capitalize(),zorder=5)
        ax.axhline(0,color=".45",ls="--",lw=1); apply_log2_tuning_axis(ax,values,mode); ax.set_xlabel(mode_axis_label(mode)); ax.set_ylabel(label); ax.set_title(f"{intensity:g} dB"); ax.grid(axis="y",alpha=.25); ax.legend(frameon=False)
    for ax in axes[len(intensities):]: ax.axis("off")
    fig.suptitle(f"{area} — {layer_label} — {mode_title(mode)}\n{label} — light=session mean +/- SD; dark=overall mean +/- SEM — {suffix.replace('_',' ')}",fontsize=14); fig.tight_layout(rect=[0,0,1,.94])
...
def make_metric_figures(cell_df: pd.DataFrame, area: str, layer_label: str, mode: str, outdir: Path, responsive_only: bool) -> None:
    ...
    sdf=aggregate_sessions(df,mode) if len(df) else pd.DataFrame()
    if len(df)==0: return
    subset_stats=run_statistics(df,sdf,include_binary=False)
    ...
    _plot_pooled_tuning_profile(df,area,layer_label,mode,outdir,suffix)
    plot_tuning_profile_session(df,area,layer_label,mode,outdir,suffix)
    plot_tuning_profiles_sessions_top_cells(df,area,layer_label,mode,outdir,suffix)
    plot_preferred_distribution(df,area,layer_label,mode,outdir,suffix)
    plot_selectivity_rasters(df,area,layer_label,mode,outdir,suffix,"preferred_tuning_value_Hz","")
...
def main() -> None:
    ...
    print("="*86)
    print("AM_TUNING AREA x LAYER GREEN-vs-YELLOW ANALYSIS (v7-compatible)")
    print("="*86)
