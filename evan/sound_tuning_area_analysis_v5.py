...
* Figure-output expansion in v5:
      ...
      - writes clean mean +/- SEM versions without individual points, with
        display ranges based on the plotted means/error bars and numeric
        mean +/- SEM annotations;
      - retains the existing amplitude-threshold selectivity rasters and adds
        FDR sound-responsive-only raster versions.
...
def subset_session_class_summary(cell_df: pd.DataFrame) -> pd.DataFrame:
    """Build the session x class fields needed for subset statistics/plots."""
    rows = []
    ...
    for (session_key, class_name), df in classified.groupby(["session_key", "cell_class"], sort=False):
        ...
        row = {
            ...
            "mean_fade_in_response": float(np.nanmean(df["fade_in_response"])) if np.any(np.isfinite(df["fade_in_response"])) else np.nan,
            "median_fade_in_response": float(np.nanmedian(df["fade_in_response"])) if np.any(np.isfinite(df["fade_in_response"])) else np.nan,
            "median_fade_in_over_fade_out_abs_ratio": float(np.nanmedian(df["fade_in_over_fade_out_abs_ratio"])) if np.any(np.isfinite(df["fade_in_over_fade_out_abs_ratio"])) else np.nan,
            "median_fade_in_over_best_am_abs_ratio": float(np.nanmedian(df["fade_in_over_best_am_abs_ratio"])) if np.any(np.isfinite(df["fade_in_over_best_am_abs_ratio"])) else np.nan,
        }
        # Retain metadata when available.
...
def sound_responsiveness_session_mean_only(
    ...
    """Mean +/- SEM across sessions for responsive/excited/suppressed fractions."""
    metrics = [
        ("pct_sound_responsive_fdr", "Responsive"),
        ("pct_sound_excited_fdr", "Excited"),
        ("pct_sound_suppressed_fdr", "Suppressed"),
    ]
    fig, ax = plt.subplots(figsize=(10.5, 6.5), constrained_layout=True)
    x = np.arange(len(metrics), dtype=float)
    offsets = {"green": -0.09, "yellow": 0.09}
    colors = {"green": GREEN_COLOR, "yellow": YELLOW_COLOR}
    all_means, all_sems = [], []
    for class_name in ["green", "yellow"]:
        cdf = class_summary_df[class_summary_df["cell_class"] == class_name]
        means, sems, ns = [], [], []
        for metric, _ in metrics:
            m, _, se, n = mean_sd_sem(cdf[metric] if metric in cdf else np.array([]))
            means.append(m); sems.append(se); ns.append(n)
        xpos = x + offsets[class_name]
        ax.errorbar(xpos, means, yerr=sems, marker="o", lw=2.2, capsize=5,
                    color=colors[class_name], label=class_name.capitalize())
        for xx, m, se, n in zip(xpos, means, sems, ns):
            annotate_mean_sem_point(
                ax, xx, m, se, n, color=colors[class_name], digits=2,
                x_offset_points=-20 if class_name == "green" else 20,
                y_offset_points=7 if class_name == "green" else 20,
                fontsize=7,
            )
        all_means.extend(means); all_sems.extend(sems)
    ax.set_xticks(x, [name for _, name in metrics])
    ax.set_ylabel("Session percentage (%)")
    ax.set_title("Session-level sound responsiveness: mean ± SEM\n(no individual session points)")
    ax.legend(frameon=False)
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, all_means, all_sems, reference_values=None)
    fig.suptitle(title, fontsize=14)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def combined_sound_response_summary(
    ...
    for class_name, class_df, color, offset in [
        ...
        if matrix.size:
            am_display_values.append(matrix.ravel())
        for row in matrix:
        ...
        am_important_values.extend(finite_values(mean).tolist())
        valid_sem = np.isfinite(mean) & np.isfinite(sem)
        if np.any(valid_sem):
            am_important_values.extend((mean[valid_sem] - sem[valid_sem]).tolist())
            am_important_values.extend((mean[valid_sem] + sem[valid_sem]).tolist())
    ...
    apply_robust_y_limits(
        ax_d,
        am_display_values,
        important_values=am_important_values,
        reference_values=[0.0],
    )
...
def combined_condition_response_mean_only(
    ...
    """Condition responses as mean +/- SEM only, with numeric annotations."""
    ...
    groups = [
        ("All plotted cells", df_all, "black"),
        ("Green", df_all[df_all["cell_class"] == "green"], GREEN_COLOR),
        ("Yellow", df_all[df_all["cell_class"] == "yellow"], YELLOW_COLOR),
    ]
    for ax, (group_name, df, color) in zip(axes[:3], groups):
        means, sems, ns = [], [], []
        for col in condition_cols:
            m, _, se, n = mean_sd_sem(df[col] if col in df else np.array([]))
            means.append(m); sems.append(se); ns.append(n)
        ax.errorbar(x, means, yerr=sems, marker="o", lw=2.2, capsize=5,
                    color=color if color != "black" else "black")
        for xx, m, se, n in zip(x, means, sems, ns):
            annotate_mean_sem_point(ax, xx, m, se, n, color=color, digits=4, fontsize=6.5)
        ax.axhline(0, color="black", ls=":", lw=1)
        ax.set_xticks(x, labels, rotation=45, ha="right")
        ax.set_title(f"{group_name}\nmean ± SEM only")
        ax.set_ylabel("Mean baseline-subtracted dF/F")
        ax.grid(axis="y", alpha=0.22)
        ax.spines[["top", "right"]].set_visible(False)
        apply_mean_error_y_limits(ax, means, sems, reference_values=[0.0])

    ax = axes[3]
    all_means, all_sems = [], []
    for class_name, color, offset, text_offset in [
        ("green", GREEN_COLOR, -0.05, -20),
        ("yellow", YELLOW_COLOR, 0.05, 20),
    ]:
        df = df_all[df_all["cell_class"] == class_name]
        means, sems, ns = [], [], []
        for col in condition_cols:
            m, _, se, n = mean_sd_sem(df[col] if col in df else np.array([]))
            means.append(m); sems.append(se); ns.append(n)
        xpos = x + offset
        ax.errorbar(xpos, means, yerr=sems, marker="o", lw=2.2, capsize=5,
                    color=color, label=class_name.capitalize())
        for xx, m, se, n in zip(xpos, means, sems, ns):
            annotate_mean_sem_point(
                ax, xx, m, se, n, color=color, digits=4,
                x_offset_points=text_offset,
                y_offset_points=7 if class_name == "green" else 22,
                fontsize=6.2,
            )
        all_means.extend(means); all_sems.extend(sems)
    ax.axhline(0, color="black", ls=":", lw=1)
    ...
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, all_means, all_sems, reference_values=[0.0])
    fig.suptitle(title + "\nNo individual cells; y-ranges use plotted means ± SEM only", fontsize=14)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)


def _plot_two_condition_mean_only(ax, df, col_a, col_b, label_a, label_b, color, panel_title):
    vals_a = df[col_a] if col_a in df else np.array([])
    vals_b = df[col_b] if col_b in df else np.array([])
    ma, _, sea, na = mean_sd_sem(vals_a)
    mb, _, seb, nb = mean_sd_sem(vals_b)
    ax.errorbar([0, 1], [ma, mb], yerr=[sea, seb], marker="o", lw=2.4,
                capsize=5, color=color)
    annotate_mean_sem_point(ax, 0, ma, sea, na, color=color, digits=4)
    annotate_mean_sem_point(ax, 1, mb, seb, nb, color=color, digits=4)
    ax.axhline(0, color="black", ls=":", lw=1)
    ax.set_xticks([0, 1], [label_a, label_b])
    ...
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, [ma, mb], [sea, seb], reference_values=[0.0])


def _plot_ratio_mean_only(ax, green_values, yellow_values, title, p_value):
    mg, _, seg, ng = mean_sd_sem(green_values)
    my, _, sey, ny = mean_sd_sem(yellow_values)
    ax.errorbar([0, 1], [mg, my], yerr=[seg, sey], marker="o", lw=0,
                capsize=6, markersize=8, color="black")
    ax.scatter([0], [mg], color=GREEN_COLOR, s=65, zorder=4)
    ax.scatter([1], [my], color=YELLOW_COLOR, edgecolor="black", linewidth=0.4, s=70, zorder=4)
    annotate_mean_sem_point(ax, 0, mg, seg, ng, color=GREEN_COLOR, digits=3)
    annotate_mean_sem_point(ax, 1, my, sey, ny, color=YELLOW_COLOR, digits=3)
    ax.axhline(1, color="black", ls=":", lw=1)
    ax.set_xticks([0, 1], ["Green", "Yellow"])
    ...
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, [mg, my], [seg, sey], reference_values=[1.0])
...
def combined_sound_response_summary_mean_only(
    ...
    """Clean quantitative companion to the detailed sound-response summary."""
    classified = cell_df[cell_df["cell_class"].isin(["green", "yellow"])].copy()
    green = classified[classified["cell_class"] == "green"]
    yellow = classified[classified["cell_class"] == "yellow"]
    fig, axes = plt.subplots(2, 4, figsize=(23, 12), constrained_layout=True)

    _plot_two_condition_mean_only(axes[0,0], green, "fade_in_response", "fade_out_response",
                                  "Fade-in", "Fade-out", GREEN_COLOR, f"Green (n={len(green)})")
    _plot_two_condition_mean_only(axes[0,1], yellow, "fade_in_response", "fade_out_response",
                                  "Fade-in", "Fade-out", YELLOW_COLOR, f"Yellow (n={len(yellow)})")
    _plot_ratio_mean_only(
        axes[0,2], green["fade_in_over_fade_out_abs_ratio"], yellow["fade_in_over_fade_out_abs_ratio"],
        "|Fade-in| / |Fade-out|", pooled_stats["fade_ratio_green_vs_yellow"].get("holm_p")
    )

    ax = axes[0,3]
    x = np.arange(len(am_rates), dtype=float)
    all_means, all_sems = [], []
    for class_name, df, color, offset, textoff in [
        ("green", green, GREEN_COLOR, -0.04, -18),
        ("yellow", yellow, YELLOW_COLOR, 0.04, 18),
    ]:
        means, sems, ns = [], [], []
        for rate in am_rates:
            col = f"am_response_{rate:g}Hz"
        ...
        for xx, m, se, n in zip(xpos, means, sems, ns):
            annotate_mean_sem_point(
                ax, xx, m, se, n, color=color, digits=4,
                x_offset_points=textoff,
                y_offset_points=7 if class_name == "green" else 21,
                fontsize=6.0,
            )
        all_means.extend(means); all_sems.extend(sems)
    ax.axhline(0, color="black", ls=":", lw=1)
    ...
    _plot_two_condition_mean_only(axes[1,0], green, "fade_in_response", "best_am_response",
                                  "Fade-in", "Best AM", GREEN_COLOR, f"Green (n={len(green)})")
    _plot_two_condition_mean_only(axes[1,1], yellow, "fade_in_response", "best_am_response",
                                  "Fade-in", "Best AM", YELLOW_COLOR, f"Yellow (n={len(yellow)})")
    _plot_ratio_mean_only(
        axes[1,2], green["fade_in_over_best_am_abs_ratio"], yellow["fade_in_over_best_am_abs_ratio"],
        "|Fade-in| / |Best AM|", pooled_stats["fade_bestam_ratio_green_vs_yellow"].get("holm_p")
    )

    ax = axes[1,3]
    mg, _, seg, ng = mean_sd_sem(green["mean_sound_response"])
    my, _, sey, ny = mean_sd_sem(yellow["mean_sound_response"])
    ax.errorbar([0,1], [mg,my], yerr=[seg,sey], marker="o", lw=0, capsize=6,
                markersize=8, color="black")
    ax.scatter([0], [mg], color=GREEN_COLOR, s=65, zorder=4)
    ax.scatter([1], [my], color=YELLOW_COLOR, edgecolor="black", linewidth=0.4, s=70, zorder=4)
    annotate_mean_sem_point(ax, 0, mg, seg, ng, color=GREEN_COLOR, digits=4)
    annotate_mean_sem_point(ax, 1, my, sey, ny, color=YELLOW_COLOR, digits=4)
    ax.axhline(0, color="black", ls=":", lw=1)
    ax.set_xticks([0,1], ["Green", "Yellow"])
    ax.set_ylabel("Mean response across all sounds")
    ax.set_title("Reliable-response magnitude\nmean ± SEM only")
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, [mg,my], [seg,sey], reference_values=[0.0])
...
def session_level_figure(
    ...
    for ax, (metric, ylabel, stat_result) in zip(axes.flat, panels):
        ...
        reference_values = [0.0] if metric == "mean_fade_in_response" else None
        apply_robust_y_limits(
            ax,
            [gv, yv],
            important_values=important,
...
def fade_in_suppression_mean_only(
    ...
    """Fade-in comparison with no individual cell/session points."""
    fig, axes = plt.subplots(1, 2, figsize=(13, 6.5), constrained_layout=True)

    ax = axes[0]
    green = finite_values(cell_df.loc[cell_df["cell_class"] == "green", "fade_in_response"])
    yellow = finite_values(cell_df.loc[cell_df["cell_class"] == "yellow", "fade_in_response"])
    mg, _, seg, ng = mean_sd_sem(green)
    my, _, sey, ny = mean_sd_sem(yellow)
    ax.errorbar([0,1], [mg,my], yerr=[seg,sey], marker="o", lw=0, capsize=6,
                markersize=8, color="black")
    ax.scatter([0], [mg], color=GREEN_COLOR, s=70, zorder=4)
    ax.scatter([1], [my], color=YELLOW_COLOR, edgecolor="black", linewidth=0.4, s=75, zorder=4)
    annotate_mean_sem_point(ax, 0, mg, seg, ng, color=GREEN_COLOR, digits=4)
    annotate_mean_sem_point(ax, 1, my, sey, ny, color=YELLOW_COLOR, digits=4)
    ax.axhline(0, color="black", ls="--", lw=1)
    p_pool = pooled_stats.get("fade_in_yellow_vs_green", {}).get("p")
    p_zero = pooled_stats.get("fade_in_yellow_less_than_zero", {}).get("p")
    ax.set_title(
        "Pooled cells: mean ± SEM only\n"
        f"yellow<0 p={('n/a' if p_zero is None else f'{p_zero:.3g}')}; "
        f"yellow vs green p={('n/a' if p_pool is None else f'{p_pool:.3g}')}"
    )
    ax.set_xticks([0,1], ["Green", "Yellow"])
    ax.set_ylabel("Mean fade-in response")
    ax.grid(axis="y", alpha=0.22)
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, [mg,my], [seg,sey], reference_values=[0.0])

    ax = axes[1]
    if len(class_summary_df):
        pivot = class_summary_df.pivot(index="session_key", columns="cell_class", values="mean_fade_in_response")
    else:
        pivot = pd.DataFrame()
    gv = pivot["green"].to_numpy(dtype=float) if "green" in pivot else np.array([])
    yv = pivot["yellow"].to_numpy(dtype=float) if "yellow" in pivot else np.array([])
    mg, _, seg, ng = mean_sd_sem(gv)
    my, _, sey, ny = mean_sd_sem(yv)
    ax.errorbar([0,1], [mg,my], yerr=[seg,sey], marker="o", lw=0, capsize=6,
                markersize=8, color="black")
    ax.scatter([0], [mg], color=GREEN_COLOR, s=70, zorder=4)
    ax.scatter([1], [my], color=YELLOW_COLOR, edgecolor="black", linewidth=0.4, s=75, zorder=4)
    ...
    annotate_mean_sem_point(ax, 1, my, sey, ny, color=YELLOW_COLOR, digits=4)
    ax.axhline(0, color="black", ls="--", lw=1)
    p_pair = session_stats.get("fade_in_green_vs_yellow", {}).get("p")
    p_y0 = session_stats.get("yellow_mean_fade_in_less_than_zero", {}).get("p")
    ax.set_title(
        "Session means: mean ± SEM only\n"
        f"yellow<0 p={('n/a' if p_y0 is None else f'{p_y0:.3g}')}; "
        f"green vs yellow p={('n/a' if p_pair is None else f'{p_pair:.3g}')}"
    )
    ax.set_xticks([0,1], ["Green", "Yellow"])
    ...
    ax.spines[["top", "right"]].set_visible(False)
    apply_mean_error_y_limits(ax, [mg,my], [seg,sey], reference_values=[0.0])
    fig.suptitle(title + "\nNo individual points; y-ranges use plotted means ± SEM only", fontsize=14)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def session_level_mean_only_figure(
    ...
    """Mean +/- SEM across sessions for the four primary session-level panels."""
    pivot = class_summary_df.pivot(index="session_key", columns="cell_class")
    fig, axes = plt.subplots(2, 2, figsize=(13, 10), constrained_layout=True)
    panels = [
        ("pct_sound_responsive_fdr", "Sound-responsive cells (%)",
         session_stats["responsive_fraction_green_vs_yellow"], None, 2),
        ("median_fade_in_over_fade_out_abs_ratio", "Median |FI| / |FO| per session",
         session_stats["fade_ratio_green_vs_yellow"], [1.0], 3),
        ("median_fade_in_over_best_am_abs_ratio", "Median |FI| / |best AM| per session",
         session_stats["fade_bestam_ratio_green_vs_yellow"], [1.0], 3),
        ("mean_fade_in_response", "Mean fade-in response per session",
         session_stats["fade_in_green_vs_yellow"], [0.0], 4),
    ]
    for ax, (metric, ylabel, stat_result, refs, digits) in zip(axes.flat, panels):
        if metric not in pivot or "green" not in pivot[metric] or "yellow" not in pivot[metric]:
            ax.axis("off"); continue
        gv = pivot[metric]["green"].to_numpy(dtype=float)
        yv = pivot[metric]["yellow"].to_numpy(dtype=float)
        mg, _, seg, ng = mean_sd_sem(gv)
        my, _, sey, ny = mean_sd_sem(yv)
        ax.errorbar([0,1], [mg,my], yerr=[seg,sey], marker="o", lw=0, capsize=6,
                    markersize=8, color="black")
        ax.scatter([0], [mg], color=GREEN_COLOR, s=70, zorder=4)
        ax.scatter([1], [my], color=YELLOW_COLOR, edgecolor="black", linewidth=0.4, s=75, zorder=4)
        annotate_mean_sem_point(ax, 0, mg, seg, ng, color=GREEN_COLOR, digits=digits)
        annotate_mean_sem_point(ax, 1, my, sey, ny, color=YELLOW_COLOR, digits=digits)
        if refs:
            for ref in refs:
                ax.axhline(ref, color="black", ls=":", lw=1)
        p = stat_result.get("p")
        ax.set_title(f"Mean ± SEM across sessions\npaired Wilcoxon p={('n/a' if p is None else f'{p:.3g}')}" )
        ax.set_xticks([0,1], ["Green", "Yellow"])
        ax.set_ylabel(ylabel)
        ax.grid(axis="y", alpha=0.22)
        ax.spines[["top", "right"]].set_visible(False)
        apply_mean_error_y_limits(ax, [mg,my], [seg,sey], reference_values=refs)
    fig.suptitle(title + "\nNo individual session points; y-ranges use plotted means ± SEM only", fontsize=14)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    if len(responsive_cell_df) and len(responsive_class_summary_df):
        ...
        responsive_pooled_stats = {
            "fade_ratio_green_vs_yellow": {"p": None, "holm_p": None},
            "fade_bestam_ratio_green_vs_yellow": {"p": None, "holm_p": None},
        ...
        responsive_session_stats = {
            ...
            "fade_ratio_green_vs_yellow": {"p": None},
            "fade_bestam_ratio_green_vs_yellow": {"p": None},
    ...
    # mean+SEM-only companions for both subsets.
    # -------------------------------------------------------------------------
    ...
    combined_selectivity_raster_preference_and_ratio(
        responsive_cell_df,
        all_am_rates,
        combined_dir / "combined_sound_selectivity_rasters_preference_and_ratio_FDR_responsive_only.png",
        title + " | FDR sound-responsive cells only",
    )
    ...
    figure_index = [
        ...
        "MEAN_SEM_ONLY files omit individual points and optimize y-limits from mean±SEM.",
        "Selectivity rasters remain cell heatmaps; FDR versions additionally require statistical responsiveness.",
        "",
    ]
