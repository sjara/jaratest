...
Example
-------
python sound_tuning_area_analysis_v6.py imag039 \
    --area A1 \
...
def signed_peak_in_window(eventlocked_window: np.ndarray) -> np.ndarray:
    """Signed value at the largest absolute excursion for every cell x trial."""
    arr = np.asarray(eventlocked_window, dtype=float)
    if arr.ndim != 3:
        raise ValueError("signed_peak_in_window expects cells x trials x time.")
    flat = arr.reshape(-1, arr.shape[-1])
    out = np.full(flat.shape[0], np.nan, dtype=float)
    finite_row = np.any(np.isfinite(flat), axis=1)
    if np.any(finite_row):
        work = np.where(np.isfinite(flat[finite_row]), np.abs(flat[finite_row]), -np.inf)
        idx = np.argmax(work, axis=1)
        selected = flat[finite_row]
        out[np.flatnonzero(finite_row)] = selected[np.arange(len(idx)), idx]
    return out.reshape(arr.shape[0], arr.shape[1])
...
def _plot_peak_condition_points(ax, values_by_condition, labels, colors, ylabel: str) -> None:
...
def save_session_selective_example_figures(
    ...
    trial_peak_response: np.ndarray,
    tvec: np.ndarray,
    ...
    """Save top FI/FO-selective GREEN/YELLOW example cells for one session."""
    root = session_output_dir / "example_selective_cells"
    ...
    for class_name, class_color in [("green", GREEN_COLOR), ("yellow", YELLOW_COLOR)]:
        ...
        cdf = cdf[np.isfinite(cdf["fade_in_vs_fade_out_selectivity"])].copy()
        if len(cdf) == 0:
            continue
        cdf["abs_fi_fo_selectivity"] = np.abs(cdf["fade_in_vs_fade_out_selectivity"])
        cdf = cdf.sort_values("abs_fi_fo_selectivity", ascending=False).head(N_EXAMPLE_CELLS_PER_CLASS)
        ...
        for rank, (_, row) in enumerate(cdf.iterrows(), start=1):
            ...
            # Full event-locked trace version. The left major panel contains FI/FO
            # columns; the right major panel contains one column per AM rate.
            ...
            # Trial-level signed peak scalar version.
            fig, axes = plt.subplots(1, 2, figsize=(14.5, 5.8))
            _plot_peak_condition_points(
                axes[0],
                [trial_peak_response[i, fade_in_mask], trial_peak_response[i, fade_out_mask]],
                ["Fade-in", "Fade-out"],
                [FADE_IN_COLOR, FADE_OUT_COLOR],
                "Signed peak dF/F in response window",
            )
            axes[0].set_title("Fade-in vs fade-out trial peaks\nlight points = trials; dark = mean ± SD")
            _plot_peak_condition_points(
                axes[1],
                [trial_peak_response[i, mask] for mask in am_masks],
                [f"{rate:g} Hz" for rate in am_rates],
                am_colors[:len(am_rates)],
                "Signed peak dF/F in response window",
            )
            axes[1].set_title("AM-rate trial peaks\nlight points = trials; dark = mean ± SD")
            fig.suptitle(
                f"{title_suffix} | {class_name.upper()} rank {rank} | ROI {roi}\n"
                f"FI/FO selectivity={fi_fo_si:.3f} | AM lifetime sparseness={am_sparse:.3f}",
                fontsize=13,
            ...
            fig.savefig(class_dir / f"rank_{rank:02d}_roi_{roi:04d}_peak_scalars.png", dpi=300, bbox_inches="tight")
            plt.close(fig)
...
def analyze_session(
    ...
    # Signed peak used only for the requested per-trial scalar example plots.
    # The peak is the sample with largest absolute excursion in the response
    # window, retaining its sign.
    trial_peak_response = signed_peak_in_window(
        eventlocked_baseline_corrected[:, :, response_mask_t]
    )
    ...
    # Requested per-session examples: top 10 GREEN and top 10 YELLOW cells
    # ranked by absolute FI/FO selectivity. Each cell gets a full-trace and a
    # signed-peak scalar figure.
    save_session_selective_example_figures(
        cell_df=cell_df,
        eventlocked_baseline_corrected=eventlocked_baseline_corrected,
        trial_peak_response=trial_peak_response,
        tvec=np.asarray(tvec, dtype=float),
        fade_in_mask=np.asarray(condition_masks[0], dtype=bool),
        fade_out_mask=np.asarray(condition_masks[1], dtype=bool),
        am_masks=[np.asarray(mask, dtype=bool) for mask in condition_masks[2:]],
        am_rates=np.asarray(possible_am_rates, dtype=float),
...
def _plot_session_paired_condition_points(ax, df, col_a, col_b, label_a, label_b, color, title):
    ...
    for session_key, sdf in df.groupby("session_key", sort=True):
        ...
        ma = float(np.mean(a)) if len(a) else np.nan
        mb = float(np.mean(b)) if len(b) else np.nan
        if not (np.isfinite(ma) and np.isfinite(mb)):
            continue
        n_sessions += 1
        ax.plot([0,1], [ma,mb], color=color, alpha=0.35, lw=1)
        ax.scatter([0,1], [ma,mb], color=color, alpha=0.75, s=28, zorder=3,
                   edgecolor="black" if color == YELLOW_COLOR else "none", linewidth=0.25)
        display.extend([ma,mb])
    ax.axhline(0, color="black", ls=":", lw=0.8)
    ax.set_xticks([0,1], [label_a,label_b])
    ax.set_ylabel("Session mean baseline-subtracted dF/F")
    ax.set_title(f"{title}\npoints = session means; n={n_sessions}")
    ax.grid(axis="y", alpha=0.2)
    ax.spines[["top", "right"]].set_visible(False)
...
def _plot_session_class_metric_points(ax, df, metric, ylabel, title, p_value=None, reference=None):
    ...
    for session_key in sorted(df["session_key"].dropna().astype(str).unique()):
        ...
        for class_name in ["green","yellow"]:
            ...
            pivot[session_key][class_name]=float(np.mean(vals)) if len(vals) else np.nan
        g=pivot[session_key]["green"]; y=pivot[session_key]["yellow"]
        if np.isfinite(g) and np.isfinite(y):
            ax.plot([0,1],[g,y],color="0.65",alpha=0.5,lw=1)
        if np.isfinite(g):
            ax.scatter(0,g,color=GREEN_COLOR,s=32,zorder=3); display.append(g)
        if np.isfinite(y):
            ax.scatter(1,y,color=YELLOW_COLOR,edgecolor="black",linewidth=0.3,s=36,zorder=3); display.append(y)
    if reference is not None:
        ax.axhline(reference,color="black",ls=":",lw=1)
    ptxt="n/a" if p_value is None else f"{p_value:.3g}"
    ax.set_xticks([0,1],["Green","Yellow"])
    ax.set_ylabel(ylabel)
    ax.set_title(title+f"\npoints = session means | paired p={ptxt}")
