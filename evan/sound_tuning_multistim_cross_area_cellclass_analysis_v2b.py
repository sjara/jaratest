...
Prerequisite: sound_tuning_multistim_fov_analysis_v2.py for A1/AAF/A2.
...
def responsive(df,b):
 base=block_data_cells(df,b) if "block_data_cells" in globals() else df
...
def plot_profile_cells(cells,b,branch,areas,cls,depth,out):
 ...
 if not cols:return
 vals=np.array([v for v,_ in cols]); x=np.log2(vals/1000) if b=='pure_tones' else vals; fig,ax=plt.subplots(figsize=(10.5,6))
 for a in areas:
 ...
 else: _readable_ticks(ax,x,[f"{v:g}" for v in vals]); ax.set_xlabel('FM slope (octaves/s)')
...
def main():
 ...
 print("="*82); print("MULTISTIM CROSS-AREA CELL-CLASS ANALYSIS v2b"); print("="*82)
 ...
 save_json(root/"run_manifest.json",{"version":"v2","subject":subject,"dates":dates,"areas":areas,"primary_replicate":"FOV","trial_reduction":{"enabled":not args.skip_trial_reduction,"fractions":TRIAL_FRACTIONS,"resamples":args.trial_reduction_resamples,"all_individual_significant_primary_YELLOW_results":True}})
