#!/usr/bin/env python3
"""
analyze_sound_red_split_full.py

Full CLI version of the notebook workflow.

Example calls
-------------

AM tuning:
python analyze_sound_red_split_full.py \
...
python analyze_sound_red_split_full.py \
  ...
  --output-dir /data/twophoton/imag025_processed/20260417/000/analysis_outputs \
  --subject imag025 --date 20260417 --session 000 --plane 0 \
  --paradigm natural_sounds --sound-type natural
"""
...
def parse_args():
    ...
    p.add_argument("--suite2p-dir", required=True, help="Path to suite2p/plane0")
    ...
    p.add_argument("--sound-type", choices=["frequency", "AM", "natural"], required=True)
    ...
    p.add_argument("--n-example-cells", type=int, default=1)
...
def get_sound_vector(data2p, sound_type):
    if sound_type in ["frequency", "AM"]:
        ...
        for candidate in ["soundID", "currentSound", "soundType", "stimID"]:
            ...
            raise KeyError(
                "Natural-sound key not found. Tried: soundID, currentSound, soundType, stimID."
            )
...
def classify_red_cells(suite2p_dir, stat, iscell_full):
    ...
    if redcell_path.exists():
        redcell_suite2p = np.load(redcell_path)[:, 0].astype(bool)
    else:
        redcell_suite2p = None

    # GMM threshold if possible, fallback to 95th percentile
    ...
    redcell_gmm = red_mean > thresh

    return redcell_suite2p, redcell_gmm, red_mean, thresh, method
...
def plot_eventlocked_qc(eventlocked, tvec, args, output_dir):
    ...
    ax0.set_title(f"Event-locked average ({args.subject} {args.date} {args.session} p{args.plane})")
    ...
    savefig(fig, output_dir, f"{args.subject}_{args.date}_{args.session}_p{args.plane}_eventlocked_qc.png", args.dpi)


def plot_all_cell_tuning(tuning, possible, args, output_dir):
    labels, xlabel = xlabels_for_stim(possible, args.sound_type)
    ...
    if args.sound_type in ["frequency", "AM"]:
        ax[0].set_xscale("log")
    ...
    savefig(fig, output_dir, f"{args.subject}_{args.date}_{args.session}_p{args.plane}_all_cell_tuning.png", args.dpi)


def plot_tuning_raster(tuning, possible, args, output_dir):
    best_idx = np.nanargmax(tuning, axis=1)
    order = np.argsort(possible[best_idx])
    ...
    im = ax.imshow(tuning_norm, aspect="auto", cmap="viridis")
...
def plot_red_diagnostics(suite2p_dir, stat, iscell_full, red_mean, thresh, method, output_dir, args):
    ops = np.load(suite2p_dir / "ops.npy", allow_pickle=True).item()
    green = ops["meanImg"]
    ...
    savefig(fig, output_dir, f"{args.subject}_{args.date}_{args.session}_p{args.plane}_red_hist.png", args.dpi)

    # threshold comparison images
...
def plot_example_cell(tuning, possible, trials_each, eventlocked, tvec, args, output_dir, kind="low"):
    best_idx = np.nanargmax(tuning, axis=1)
    ...
    if kind == "low":
        ...
        candidates = np.where((best <= threshold) & (peak > args.min_response))[0]
    else:
        threshold = np.percentile(possible, 70)
        candidates = np.where((best >= threshold) & (peak > args.min_response))[0]

    if len(candidates) == 0:
        print(f"No {kind}-stimulus example cells found.")
        return

    # random each run
    cell_idx = np.random.choice(candidates)
    print(f"{kind} example cell: {cell_idx}, best={best[cell_idx]}")
    ...
    ax[0].set_title(f"Cell {cell_idx} response raster")
...
def plot_trace_overlay(tuning, possible, trials_each, eventlocked, tvec, args, output_dir):
    best_idx = np.nanargmax(tuning, axis=1)
    peak = np.nanmax(tuning, axis=1)
    ...
    ax.axvspan(args.resp_start, args.resp_end, color="red", alpha=0.08)
    ax.set_xlabel("Time from sound onset (s)")
    ax.set_ylabel("dF/F")
    ax.set_title(f"Cell {cell_idx} response traces")
    ax.legend(frameon=False)

    savefig(fig, output_dir, f"{args.subject}_{args.date}_{args.session}_p{args.plane}_example_trace_overlay_cell{cell_idx}.png", args.dpi)


def plot_red_split_raster(tuning, possible, redcell, args, output_dir):
    labels, xlabel = xlabels_for_stim(possible, args.sound_type)
    ...
    savefig(fig, output_dir, f"{args.subject}_{args.date}_{args.session}_p{args.plane}_red_split_raster.png", args.dpi)


def plot_spatial_maps(tuning, possible, eventlocked, redcell, inds, stat, suite2p_dir, args, output_dir):
    ops = np.load(suite2p_dir / "ops.npy", allow_pickle=True).item()
    ...
    sc1 = ax[1].scatter(xc, yc, c=skewness, cmap="coolwarm", vmin=-2, vmax=2, s=60, edgecolors=edgecolors, linewidths=linewidths)
    ax[1].set_title("dF/F skewness")
    ax[1].axis("off")

    fig.colorbar(sc0, ax=ax[0], label=f"Preferred {args.sound_type}")
    fig.colorbar(sc1, ax=ax[1], label="Skewness clipped ±2")

    savefig(fig, output_dir, f"{args.subject}_{args.date}_{args.session}_p{args.plane}_spatial_maps.png", args.dpi)
...
def main():
    ...
    suite2p_dir = Path(args.suite2p_dir).expanduser().resolve()
    ...
    stim_all, stim_key = get_sound_vector(data2p, args.sound_type)
    ...
    redcell_suite2p, redcell_gmm_full, red_mean, thresh, method = classify_red_cells(suite2p_dir, stat, iscell_full)

    redcell = redcell_gmm_full[inds]

    print(f"Red classification method: {method}, threshold={thresh:.3f}")
    print(f"Total filtered cells: {eventlocked.shape[0]}")
    ...
    plot_eventlocked_qc(eventlocked, tvec, args, output_dir)
    plot_all_cell_tuning(tuning, possible, args, output_dir)
    plot_tuning_raster(tuning, possible, args, output_dir)
    plot_red_diagnostics(suite2p_dir, stat, iscell_full, red_mean, thresh, method, output_dir, args)
    plot_example_cell(tuning, possible, trials_each, eventlocked, tvec, args, output_dir, kind="low")
    plot_example_cell(tuning, possible, trials_each, eventlocked, tvec, args, output_dir, kind="high")
    plot_trace_overlay(tuning, possible, trials_each, eventlocked, tvec, args, output_dir)
    plot_red_split_raster(tuning, possible, redcell, args, output_dir)
    plot_spatial_maps(tuning, possible, eventlocked, redcell, inds, stat, suite2p_dir, args, output_dir)
