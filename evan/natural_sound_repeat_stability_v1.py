...
natural_sound_response_analysis_v1.py and asks the design question:
...
`natural_responsive_fdr` upstream. Use --condition-responsive-only if you want to
restrict to cell x condition pairs that were individually FDR-responsive.
...
All natural-sound archives for imag039 on one date:
    ...
    python natural_sound_repeat_stability_v1.py imag039 --dates 20260717
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--condition-responsive-only",action="store_true",
                   help="Use only individually FDR-responsive cell x condition pairs.")
...
def load_units_from_archive(path: Path,args: argparse.Namespace) -> tuple[list[dict[str,Any]],dict[str,Any]]:
    ...
    with np.load(path,allow_pickle=False) as z:
        ...
        if schema!="natural_sound_response_v1":
        ...
        meta={"path":str(path),"subject":subject,"date":date,"session":session,"canonical_area":area,
              "brain_area":brain,"depth":depth,"n_units":len(units),"n_cells":len(classes),"n_trials":int(np.sum(analysis_trials))}
    return units,meta
...
def main()->None:
    ...
    if not archives:
        raise RuntimeError("No matching natural_sound_trial_responses.npz archives were found. Run natural_sound_response_analysis_v1.py first.")
    print("="*80);print("NATURAL-SOUND REPEAT-NUMBER STABILITY v1");print("="*80)
    ...
    for path in archives:
        ...
        print(f"  {meta['date']} {meta['session']} {meta['canonical_area']}: {meta['n_units']} eligible cell x condition units")
    if not all_units:
        raise RuntimeError("No eligible cell x condition units after filters.")
    ...
    manifest={
        ...
        "response_source":args.response_source,"condition_responsive_only":args.condition_responsive_only,
