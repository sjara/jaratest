#!/usr/bin/env python3
"""
Cross-area sound-tuning analysis within YELLOW and GREEN cell classes (v4).
...
---------------------------------
For every individually significant YELLOW-across-area result represented by a
statistical figure, a separate Monte-Carlo robustness figure is generated. This
includes primary session-level omnibus and pairwise findings, clearly labeled
sensitivity analyses, explicitly exploratory pooled-cell findings, condition-
profile findings, and the retained area x layer comparisons. Each robustness
figure names the exact test and correction family being tracked.  Trial
...
# =============================================================================
# v4 OVERRIDES: COMPLETE ON-FIGURE STATISTICS + INDIVIDUAL TRIAL-REDUCTION TESTS
# =============================================================================

def _json_clean_v4(obj: Any) -> Any:
    if isinstance(obj, dict):
        return {str(k): _json_clean_v4(v) for k,v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [_json_clean_v4(v) for v in obj]
    if isinstance(obj, np.ndarray):
        return [_json_clean_v4(v) for v in obj.tolist()]
...
def save_json(path: Path, obj: Any) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as f:
        json.dump(_json_clean_v4(obj), f, indent=2, allow_nan=False)



def _num_text(x: Any, digits: int = 3) -> str:
    try:
        x = float(x)
    except Exception:
        return "n/a"
...
def _safe_stem(text: str, maxlen: int = 130) -> str:
    import re as _re
    t = _re.sub(r"[^A-Za-z0-9._-]+", "_", str(text)).strip("_.")
    return t[:maxlen] if t else "test"


def _cross_area_stats_lines(stats_rec: dict[str, Any], *, scope_label: str,
                            family_corrected_omnibus: bool = False) -> list[str]:
    omni = stats_rec.get("omnibus", {})
    line = f"{scope_label}: Kruskal-Wallis H={_num_text(omni.get('H'))}, raw {p_text(omni.get('p', np.nan))}"
    if family_corrected_omnibus:
        line += f", family-Holm {p_text(omni.get('p_holm_family', np.nan))}"
    ns = omni.get("group_ns", {})
    if ns:
        line += "; n=" + ", ".join(f"{a}:{ns.get(a,0)}" for a in ns)
    lines = [line, "Pairwise 2-sided Mann-Whitney U; Holm across the 3 A1/AAF/A2 pairs:"]
    for r in stats_rec.get("pairwise", {}).values():
        lines.append(
            f"  {r.get('area1','?')} vs {r.get('area2','?')}: U={_num_text(r.get('U'))}, "
            f"raw {p_text(r.get('p', np.nan))}, Holm {p_text(r.get('p_holm', np.nan))}, "
            f"n={r.get('n1',0)}/{r.get('n2',0)}, Cliff d={_num_text(r.get('cliffs_delta'))}"
        )
    return lines


def _draw_stats_axis(ax: plt.Axes, lines: list[str], title: str = "Statistics") -> None:
    ax.axis("off")
    ax.set_title(title, loc="left", fontsize=10, fontweight="bold")
    ax.text(0.0, 0.98, "\n".join(lines), transform=ax.transAxes, ha="left", va="top",
            fontsize=7.0, family="monospace", linespacing=1.30, wrap=True)


def add_sig_bars(ax: plt.Axes, pairwise: dict[str, Any], xmap: dict[str, float],
                 alpha: float = 0.05) -> None:
    """Bars mark significant pairs; complete results for all pairs are printed on the same figure."""
    sig = [r for r in pairwise.values()
           if np.isfinite(r.get("p_holm", np.nan)) and r["p_holm"] < alpha]
    if not sig:
        return
    y0, y1 = ax.get_ylim()
    span = max(y1-y0, 1e-9)
    ax.set_ylim(y0, y1 + 0.07*span*len(sig))
    y0, y1 = ax.get_ylim()
    span = y1-y0
    for i, r in enumerate(sig):
        a, b = r["area1"], r["area2"]
        if a not in xmap or b not in xmap:
            continue
        x1, x2 = xmap[a], xmap[b]
        y = y1 - (0.04 + 0.055*i)*span
        ax.plot([x1,x1,x2,x2], [y-0.006*span,y,y,y-0.006*span], color="0.2", lw=1)
        ax.text((x1+x2)/2, y+0.004*span, f"MW Holm p={r['p_holm']:.3g}",
                ha="center", va="bottom", fontsize=7)


def plot_cross_area_cell_metric(df: pd.DataFrame, metric: str, areas: list[str], title: str,
                                stats_rec: dict[str, Any], output_path: Path, sensitivity: bool = False) -> None:
    fig, (ax, axstats) = plt.subplots(1, 2, figsize=(13.5, 6.5),
    ...
    ref = _metric_reference(metric)
    if ref is not None:
        ax.axhline(ref,ls="--",lw=1,color="0.45")
    ylim = robust_ylim(display,ref,bounded=metric in {FI_FO_PRIMARY,FI_FO_SESSION_MEDIAN,FI_FO_PRESTIM60})
    if ylim:
        ax.set_ylim(*ylim)
    add_sig_bars(ax,stats_rec.get("pairwise",{}),xmap)
    ...
    scope = "Sensitivity / exploratory pooled-cell" if sensitivity else "Exploratory pooled-cell"
    ax.set_title(title + "\n" + scope)
    eq = _metric_equation(metric)
    if eq:
        ax.text(0.5,-0.13,eq,transform=ax.transAxes,ha="center",va="top",fontsize=9)
    ax.grid(axis="y",alpha=0.2)
    ax.spines[["top","right"]].set_visible(False)
    _draw_stats_axis(axstats,_cross_area_stats_lines(stats_rec,scope_label=scope),"All statistics shown")
...
def plot_cross_area_session_metric(df: pd.DataFrame, metric: str, areas: list[str], title: str,
    ...
    for a in areas:
        ...
        if len(vals):
            ax.scatter(np.full(len(vals),x)+offs,vals,s=30,alpha=0.35,color=AREA_COLORS[a],zorder=2)
            m,se,n=mean_sem(vals)
            ax.errorbar(x,m,yerr=se if np.isfinite(se) else None,fmt="o",ms=11,capsize=7,
                        elinewidth=2.7,color=AREA_COLORS[a],zorder=5)
            ax.annotate(f"n={n}",(x,m),xytext=(0,12),textcoords="offset points",ha="center",fontsize=8)
            display.extend(vals.tolist())
    ref=_metric_reference(metric)
    if ref is not None:
    ...
    if ylim:
        ax.set_ylim(*ylim)
    add_sig_bars(ax,stats_rec.get("pairwise",{}),xmap)
    scope="Session-level sensitivity" if sensitivity else "PRIMARY session-level"
    ax.set_xticks(range(len(areas)),areas)
    ax.set_ylabel(METRIC_LABELS.get(metric,metric))
    ax.set_title(title + "\n" + scope)
    eq=_metric_equation(metric)
    ...
    ax.grid(axis="y",alpha=0.2)
    ax.spines[["top","right"]].set_visible(False)
    _draw_stats_axis(axstats,_cross_area_stats_lines(stats_rec,scope_label=scope,
                    family_corrected_omnibus=not sensitivity),"All statistics shown")
    fig.subplots_adjust(bottom=0.20,wspace=0.28)
    fig.savefig(output_path,dpi=DPI,bbox_inches="tight")
    plt.close(fig)
    ...
    for ax,(metric,label) in zip(axes,RESPONSIVENESS.items()):
        ...
        for a in areas:
            ...
            if len(vals):
                ...
                display.extend(vals.tolist())
        rec=stats_out["responsiveness"][metric]["session_level_primary"]
        ylim=robust_ylim(display+[0.0],0.0)
        if ylim:
            ax.set_ylim(max(0.0,ylim[0]),min(100.0,max(ylim[1],5.0)))
        add_sig_bars(ax,rec["pairwise"],xmap)
        ax.set_xticks(range(len(areas)),areas)
        ...
        lines=_cross_area_stats_lines(rec,scope_label="PRIMARY session-level",
                                      family_corrected_omnibus=True)
        ax.text(0.01,0.01,"\n".join(lines),transform=ax.transAxes,ha="left",va="bottom",
                fontsize=5.5,family="monospace",linespacing=1.12,
                bbox=dict(boxstyle="round,pad=0.25",facecolor="white",alpha=0.80,edgecolor="0.8"))
        ax.grid(axis="y",alpha=0.2)
        ax.spines[["top","right"]].set_visible(False)
    for ax in axes[len(RESPONSIVENESS):]:
        ax.axis("off")
    fig.suptitle(f"{cell_class.upper()} responsiveness across areas — {depth_label}\n"
                 "small points=session percentages; large=area mean ± SE; every panel lists all tests",
                 fontsize=13)
    fig.tight_layout(rect=[0,0,1,0.94])
    fig.savefig(output_dir/"responsiveness_cross_area_SESSION_POINTS.png",dpi=DPI,bbox_inches="tight")
    plt.close(fig)


def _condition_family_stats_lines(fam_stats: dict[str, Any]) -> list[str]:
    lines=[]
    for col,wrap in fam_stats.items():
        rec=wrap.get("session_level_primary",{})
        label=wrap.get("label",col)
        omni=rec.get("omnibus",{})
        lines.append(f"{label}: KW H={_num_text(omni.get('H'))}, raw {p_text(omni.get('p',np.nan))}, "
                     f"family-Holm {p_text(omni.get('p_holm_family',np.nan))}")
        for r in rec.get("pairwise",{}).values():
            lines.append(f"  {r['area1']} vs {r['area2']}: MW U={_num_text(r.get('U'))}, "
                         f"raw {p_text(r.get('p',np.nan))}, Holm {p_text(r.get('p_holm',np.nan))}, "
                         f"n={r.get('n1',0)}/{r.get('n2',0)}, d={_num_text(r.get('cliffs_delta'))}")
    return lines


def _annotate_condition_stats(ax: plt.Axes, xs: list[float], cond_cols: list[str],
                              fam_stats: dict[str, Any]) -> None:
    y0,y1=ax.get_ylim()
    span=max(y1-y0,1e-9)
    for x,c in zip(xs,cond_cols):
        rec=fam_stats.get(c,{}).get("session_level_primary",{})
        omni=rec.get("omnibus",{})
        ph=omni.get("p_holm_family",np.nan)
        sigpairs=[f"{r['area1']}-{r['area2']}" for r in rec.get("pairwise",{}).values()
                  if np.isfinite(r.get("p_holm",np.nan)) and r["p_holm"]<0.05]
        mark="*" if np.isfinite(ph) and ph<0.05 else ""
        txt=f"{mark}H={_num_text(omni.get('H'),2)}\nFH {p_text(ph)}"
        if sigpairs:
            txt += "\n" + ",".join(sigpairs)
        ax.text(x,y1-0.018*span,txt,ha="center",va="top",fontsize=6.2)


def plot_condition_profile_all(cells: pd.DataFrame, stats_out: dict[str, Any], areas: list[str],
                               cell_class: str, depth_label: str, output_dir: Path) -> None:
    am_cols=_am_primary_columns(cells)
    fi_x,am_x=_condition_plot_positions(len(am_cols))
    cols=["fade_in_response","fade_out_response"]+[c for _,c in am_cols]
    labels=["Fade-in","Fade-out"]+[f"{r:g} Hz" for r,_ in am_cols]
    xs=list(fi_x)+list(am_x)
    fig,(ax,axstats)=plt.subplots(1,2,figsize=(18,7.8),gridspec_kw={"width_ratios":[1.55,1.15]})
    display=[]
    for a in areas:
        area=cells[cells["canonicalArea"]==a]
        rows=[]
        for _,sdf in area.groupby("session_key"):
            rows.append([np.nanmean(pd.to_numeric(sdf[c],errors="coerce")) for c in cols])
        if rows:
            M=np.asarray(rows,float)
            for row in M:
                ax.plot(fi_x,row[:2],color=AREA_COLORS[a],alpha=0.16,lw=0.8)
                if len(am_cols):
                    ax.plot(am_x,row[2:],color=AREA_COLORS[a],alpha=0.16,lw=0.8)
    ...
    ax.set_title(f"{cell_class.upper()} across areas — {depth_label}\nAll cells; session means + area mean ± SE")
    ax.legend(frameon=False)
    ax.grid(axis="y",alpha=0.2)
    ax.spines[["top","right"]].set_visible(False)
    _draw_stats_axis(axstats,_condition_family_stats_lines(stats_out["condition_profiles"]["all_cells"]),
                     "All condition-wise statistics")
    fig.tight_layout()
    fig.savefig(output_dir/"condition_response_profile_all_cells.png",dpi=DPI,bbox_inches="tight")
    plt.close(fig)


def _plot_profile_panel(ax: plt.Axes, df: pd.DataFrame, columns: list[tuple[str,str]],
                        areas: list[str], fam_stats: dict[str, Any], title: str) -> None:
    x=np.arange(len(columns),dtype=float)
    display=[]
    for a in areas:
        adf=df[df["canonicalArea"]==a]
        rows=[]
        for _,sdf in adf.groupby("session_key"):
            rows.append([np.nanmean(pd.to_numeric(sdf[c],errors="coerce")) for _,c in columns])
        if rows:
            M=np.asarray(rows,float)
            for row in M:
                ax.plot(x,row,color=AREA_COLORS[a],alpha=0.16,lw=0.8)
            overall=np.nanmean(M,axis=0)
            n=np.sum(np.isfinite(M),axis=0)
            sem=np.nanstd(M,axis=0,ddof=1)/np.sqrt(n)
            ax.plot(x,overall,color=AREA_COLORS[a],lw=2.7,marker="o",ms=5,label=a)
    ...
    if ylim:
        ax.set_ylim(*ylim)
    _annotate_condition_stats(ax,list(x),[c for _,c in columns],fam_stats)
    ax.set_xticks(x,[l for l,_ in columns],rotation=25,ha="right")
    ax.set_ylabel("Response (dF/F)")
    ax.set_title(title)
    ax.grid(axis="y",alpha=0.2)
    ax.spines[["top","right"]].set_visible(False)
...
def plot_condition_profile_responsive(cells: pd.DataFrame, stats_out: dict[str, Any],
                                      areas: list[str], cell_class: str, depth_label: str,
                                      output_dir: Path) -> None:
    am_cols=_am_primary_columns(cells)
    fi_df=analysis_subset(cells,"fi_fo_responsive")
    am_df=analysis_subset(cells,"am_responsive")
    fig=plt.figure(figsize=(18,11.2))
    gs=fig.add_gridspec(2,2,height_ratios=[1.25,1.0],hspace=0.22,wspace=0.20)
    ax0=fig.add_subplot(gs[0,0]); ax1=fig.add_subplot(gs[0,1])
    st0=fig.add_subplot(gs[1,0]); st1=fig.add_subplot(gs[1,1])
    _plot_profile_panel(ax0,fi_df,[("Fade-in","fade_in_response"),("Fade-out","fade_out_response")],
                        areas,stats_out["condition_profiles"]["responsive_fi_fo"],"FI-or-FO responsive")
    _plot_profile_panel(ax1,am_df,[(f"{r:g} Hz",c) for r,c in am_cols],areas,
                        stats_out["condition_profiles"]["responsive_am"],">=1-AM-rate responsive")
    ax1.legend(frameon=False)
    _draw_stats_axis(st0,_condition_family_stats_lines(stats_out["condition_profiles"]["responsive_fi_fo"]),
                     "FI/FO responsive: all statistics")
    _draw_stats_axis(st1,_condition_family_stats_lines(stats_out["condition_profiles"]["responsive_am"]),
                     "AM responsive: all statistics")
    fig.suptitle(f"{cell_class.upper()} condition responses across areas — {depth_label}\n"
                 "condition-specific responsive populations",fontsize=13)
    fig.savefig(output_dir/"condition_response_profile_FDR_responsive_only.png",dpi=DPI,bbox_inches="tight")
    plt.close(fig)


def plot_raster(df: pd.DataFrame, sort_metric: str, areas: list[str], title: str,
                output_path: Path, stats_lines: list[str] | None = None) -> None:
    nstats=1 if stats_lines else 0
    fig=plt.figure(figsize=(13.5,3.5*len(areas)+(2.2 if nstats else 0.5)))
    gs=fig.add_gridspec(len(areas)+nstats,1,
                        height_ratios=[1]*len(areas)+([0.75] if nstats else []),hspace=0.40)
    axes=[fig.add_subplot(gs[i,0]) for i in range(len(areas))]
    im=None
    for ax,a in zip(axes,areas):
        adf=df[df["canonicalArea"]==a].copy()
        matrix,labels=_condition_matrix(adf)
        if matrix.size==0:
            ax.text(0.5,0.5,"No cells",transform=ax.transAxes,ha="center")
            ax.axis("off")
            continue
        denom=np.nanmax(np.abs(matrix),axis=1)
        good=np.isfinite(denom)&(denom>0)
        matrix=matrix[good]
        adf=adf.iloc[np.flatnonzero(good)].copy()
        norm=matrix/denom[good,None]
        rank=pd.to_numeric(adf[sort_metric],errors="coerce").to_numpy(float)
        order=np.argsort(np.nan_to_num(rank,nan=-np.inf))[::-1]
        im=ax.imshow(norm[order],aspect="auto",interpolation="nearest",vmin=-1,vmax=1,cmap="coolwarm")
        ax.set_xticks(np.arange(len(labels)),labels,rotation=25,ha="right")
        ax.set_ylabel(f"{a} cells")
        ax.set_title(f"{a}: n={len(order)}")
    if stats_lines:
        _draw_stats_axis(fig.add_subplot(gs[-1,0]),stats_lines,
                         "Relevant cross-area statistics for the sorting metric")
    fig.suptitle(title,y=0.995)
    if im is not None:
        fig.colorbar(im,ax=axes,label="Normalized condition response",shrink=0.75)
    fig.subplots_adjust(top=0.93,bottom=0.06,right=0.90)
    fig.savefig(output_path,dpi=DPI,bbox_inches="tight")
    plt.close(fig)
...
def plot_selectivity_outputs(cells: pd.DataFrame, stats_out: dict[str, Any], areas: list[str],
    ...
    for metric,stem,sensitivity in specs:
        plot_cross_area_cell_metric(cells,metric,areas,
            f"{cell_class.upper()} — {depth_label}\n{METRIC_LABELS[metric]} — all cells",
            pooled[metric],output_dir/f"{stem}_across_areas_all_cells_CELL_POINTS.png",sensitivity=sensitivity)
    for metric,stem,sensitivity in specs[:-1]:
        rec=sel["sensitivity"][metric]["session_level_sensitivity"] if sensitivity else sel[metric]["session_level_primary"]
        plot_cross_area_session_metric(cells,metric,areas,
            f"{cell_class.upper()} — {depth_label}\n{METRIC_LABELS[metric]} — all cells",
            rec,output_dir/f"{stem}_across_areas_all_cells_SESSION_POINTS.png",sensitivity=sensitivity)
    fi_stats=_cross_area_stats_lines(sel[FI_FO_PRIMARY]["session_level_primary"],
             scope_label="PRIMARY session-level FI/FO",family_corrected_omnibus=True)
    fi_stats += [""] + _cross_area_stats_lines(pooled[FI_FO_PRIMARY],scope_label="Exploratory pooled-cell FI/FO")
    am_stats=_cross_area_stats_lines(pooled[AM_SPARSE],scope_label="Exploratory pooled-cell AM lifetime sparseness")
    plot_raster(cells,FI_FO_PRIMARY,areas,
                f"{cell_class.upper()} — {depth_label} — all cells\nsorted descending by signed FI/FO selectivity",
                output_dir/"selectivity_rasters_all_cells.png",fi_stats)
    plot_raster(cells,AM_SPARSE,areas,
                f"{cell_class.upper()} — {depth_label} — all cells\nsorted descending by AM-rate lifetime sparseness",
                output_dir/"am_selectivity_raster_all_cells.png",am_stats)

    pooled_r=stats_out["exploratory_pooled"]["selectivity"]["responsive_only"]
    sel_r=stats_out["selectivity"]["responsive_only"]
    fi_df=analysis_subset(cells,"fi_fo_responsive")
    fi_am_df=analysis_subset(cells,"fi_am_responsive")
    am_df=analysis_subset(cells,"am_responsive")
    rspecs=[
        (FI_FO_PRIMARY,fi_df,"fade_in_vs_fade_out_selectivity",False,"fi_fo_responsive"),
        (FI_FO_SESSION_MEDIAN,fi_df,"fade_in_vs_fade_out_selectivity_session_median_dff",True,"fi_fo_responsive"),
        (FI_FO_PRESTIM60,fi_df,"fade_in_vs_fade_out_selectivity_60s_prestim_median_dff",True,"fi_fo_responsive"),
        (FI_BEST_AM,fi_am_df,"fade_in_vs_best_am_selectivity",False,"fi_am_responsive"),
        (AM_SPARSE,am_df,"am_rate_lifetime_sparseness",False,"am_responsive"),
    ]
    for metric,df,stem,sensitivity,labelkind in rspecs:
        plot_cross_area_cell_metric(df,metric,areas,
            f"{cell_class.upper()} — {depth_label}\n{METRIC_LABELS[metric]} — {SUBSET_LABELS[labelkind]}",
            pooled_r[metric],output_dir/f"{stem}_across_areas_FDR_responsive_only_CELL_POINTS.png",sensitivity=sensitivity)
    for metric,df,stem,sensitivity,_ in rspecs[:-1]:
        rec=sel_r["sensitivity"][metric]["session_level_sensitivity"] if sensitivity else sel_r[metric]["session_level_primary"]
        plot_cross_area_session_metric(df,metric,areas,
            f"{cell_class.upper()} — {depth_label}\n{METRIC_LABELS[metric]} — responsive-only",
            rec,output_dir/f"{stem}_across_areas_FDR_responsive_only_SESSION_POINTS.png",sensitivity=sensitivity)
    fi_stats=_cross_area_stats_lines(sel_r[FI_FO_PRIMARY]["session_level_primary"],
             scope_label="PRIMARY session-level FI/FO",family_corrected_omnibus=True)
    fi_stats += [""] + _cross_area_stats_lines(pooled_r[FI_FO_PRIMARY],scope_label="Exploratory pooled-cell FI/FO")
    am_stats=_cross_area_stats_lines(pooled_r[AM_SPARSE],scope_label="Exploratory pooled-cell AM lifetime sparseness")
    plot_raster(fi_df,FI_FO_PRIMARY,areas,
                f"{cell_class.upper()} — {depth_label} — FI-or-FO responsive\nsorted descending by signed FI/FO selectivity",
                output_dir/"selectivity_rasters_FDR_responsive_only.png",fi_stats)
    plot_raster(am_df,AM_SPARSE,areas,
                f"{cell_class.upper()} — {depth_label} — >=1-AM responsive\nsorted descending by AM-rate lifetime sparseness",
                output_dir/"am_selectivity_raster_FDR_responsive_only.png",am_stats)


def plot_area_layer_metric(df: pd.DataFrame, metric: str, areas: list[str], title: str,
                           output_path: Path, session_level: bool) -> dict[str, Any]:
    df=add_layer_columns(df)
    df=df[df["layer_key"].notna()].copy()
    stats_rec=_layer_pair_stats(df,metric,areas,session_level)
    fig,(ax,axstats)=plt.subplots(1,2,figsize=(15.5,6.8),gridspec_kw={"width_ratios":[1.55,1.0]})
    display=[]; xticks=[]; xlabels=[]; x=0; positions={}
    for a in areas:
        positions[a]={}
        for lk,label in [("L23","L2/3"),("L45","L4/5")]:
            sub=df[(df["canonicalArea"]==a)&(df["layer_key"]==lk)]
            ...
            offs=np.linspace(-0.055,0.055,max(len(vals),1))
            if len(vals):
                ax.scatter(np.full(len(vals),x)+offs,vals,s=28 if session_level else 18,
                           alpha=0.35 if session_level else 0.20,color=AREA_COLORS[a])
                m,se,n=mean_sem(vals)
                ax.errorbar(x,m,yerr=se if np.isfinite(se) else None,fmt="o",ms=10,capsize=6,lw=2.3,color=AREA_COLORS[a])
                display.extend(vals.tolist())
            positions[a][lk]=x
            xticks.append(x); xlabels.append(f"{a}\n{label}"); x+=1
        x+=0.6
    ...
    if ylim:
        ax.set_ylim(*ylim)
    y0,y1=ax.get_ylim(); span=max(y1-y0,1e-9)
    sig=[(a,r) for a,r in stats_rec.items()
         if np.isfinite(r.get("p_holm_across_areas",np.nan)) and r["p_holm_across_areas"]<0.05]
    if sig:
        ax.set_ylim(y0,y1+0.07*span*len(sig)); y0,y1=ax.get_ylim(); span=y1-y0
    for i,(a,r) in enumerate(sig):
        x1,x2=positions[a]["L23"],positions[a]["L45"]
        y=y1-(0.04+0.055*i)*span
        ax.plot([x1,x1,x2,x2],[y-0.006*span,y,y,y-0.006*span],color="0.2",lw=1)
        ax.text((x1+x2)/2,y+0.004*span,f"MW Holm p={r['p_holm_across_areas']:.3g}",ha="center",fontsize=7)
    ax.set_xticks(xticks,xlabels)
    ax.set_ylabel(METRIC_LABELS.get(metric,metric))
    ax.set_title(title+"\nL2/3 vs L4/5 within each area")
    eq=_metric_equation(metric)
    if eq:
        ax.text(0.5,-0.15,eq,transform=ax.transAxes,ha="center",va="top",fontsize=9)
    ax.grid(axis="y",alpha=0.2)
    ax.spines[["top","right"]].set_visible(False)
    scope="PRIMARY session-level" if session_level else "Exploratory pooled-cell"
    lines=[f"{scope}: 2-sided Mann-Whitney U L2/3 vs L4/5; Holm across the 3 area tests:"]
    for a in areas:
        r=stats_rec[a]
        lines.append(f"  {a}: U={_num_text(r.get('U'))}, raw {p_text(r.get('p',np.nan))}, "
                     f"Holm {p_text(r.get('p_holm_across_areas',np.nan))}, n={r.get('n1',0)}/{r.get('n2',0)}, "
                     f"Cliff d={_num_text(r.get('cliffs_delta'))}")
    _draw_stats_axis(axstats,lines,"All area × layer statistics")
    fig.subplots_adjust(bottom=0.20,wspace=0.28)
    fig.savefig(output_path,dpi=DPI,bbox_inches="tight")
    plt.close(fig)
    return stats_rec
...
def reduced_yellow_cells(full_yellow_cells: pd.DataFrame, fraction: float, rng: np.random.Generator,
                         archive_cache: dict[str,dict[str,np.ndarray]], sign_cache: SignCache,
                         recompute_responsiveness: bool = True) -> tuple[pd.DataFrame,list[int]]:
    """v4: same sampled trial identities are applied to primary and both alternate-dF/F matrices."""
    rows=[]; all_counts=[]
    sessions=full_yellow_cells[["session_key","canonicalArea","depth","trial_response_archive_path"]].drop_duplicates("session_key")
    for _,srow in sessions.iterrows():
        arc=load_trial_archive(srow["trial_response_archive_path"],archive_cache)
        for required in ("trial_response_session_median_dff","trial_response_60s_prestim_median_dff"):
            if required not in arc:
                raise ValueError(f"{srow['trial_response_archive_path']} lacks v13 field {required}")
        chosen,counts=_sample_condition_trials(arc,fraction,rng)
        all_counts.extend(counts)
        R=np.asarray(arc["trial_response_primary"],float)
        Rsm=np.asarray(arc["trial_response_session_median_dff"],float)
        Rpre=np.asarray(arc["trial_response_60s_prestim_median_dff"],float)
        classified=np.asarray(arc["classified_mask"],bool)
        n_cells=R.shape[0]; n_cond=len(chosen)
        def _means(M):
            out=np.full((n_cells,n_cond),np.nan)
            for ci,take in enumerate(chosen):
                if len(take):
                    out[:,ci]=np.nanmean(M[:,take],axis=1)
            return out
        cond=_means(R); cond_sm=_means(Rsm); cond_pre=_means(Rpre)
        resp=np.zeros_like(cond,dtype=bool)
        for ci,take in enumerate(chosen):
            if recompute_responsiveness and len(take)>=TRIAL_REDUCTION_MIN_TRIALS:
                pv=signflip_pvalues_grouped(R[:,take],classified,sign_cache)
                q=bh_fdr(pv,classified)
                resp[:,ci]=classified & np.isfinite(q) & (q<FDR_ALPHA)
        fi=cond[:,0] if n_cond>0 else np.full(n_cells,np.nan)
        fo=cond[:,1] if n_cond>1 else np.full(n_cells,np.nan)
        am=cond[:,2:] if n_cond>2 else np.empty((n_cells,0))
        fi_sm=cond_sm[:,0] if n_cond>0 else np.full(n_cells,np.nan)
        fo_sm=cond_sm[:,1] if n_cond>1 else np.full(n_cells,np.nan)
        fi_pre=cond_pre[:,0] if n_cond>0 else np.full(n_cells,np.nan)
        fo_pre=cond_pre[:,1] if n_cond>1 else np.full(n_cells,np.nan)
        best=np.full(n_cells,np.nan)
        if am.shape[1]:
            validrow=np.any(np.isfinite(am),axis=1)
            idx=np.flatnonzero(validrow)
            if len(idx):
                best[idx]=am[idx,np.nanargmax(am[idx],axis=1)]
        fi_fo=bounded_fi_fo(fi,fo)
        fi_fo_sm=bounded_fi_fo(fi_sm,fo_sm)
        fi_fo_pre=bounded_fi_fo(fi_pre,fo_pre)
        fi_best=fi_best_selectivity(fi,best)
        sparse=lifetime_sparseness(am)
        if recompute_responsiveness:
            fi_resp=resp[:,0] if n_cond>0 else np.zeros(n_cells,bool)
            fo_resp=resp[:,1] if n_cond>1 else np.zeros(n_cells,bool)
            am_resp=np.any(resp[:,2:],axis=1) if n_cond>2 else np.zeros(n_cells,bool)
            sound_resp=np.any(resp,axis=1)
            excited=np.zeros(n_cells,bool); suppressed=np.zeros(n_cells,bool)
            for i in np.flatnonzero(sound_resp):
                cand=np.flatnonzero(resp[i]&np.isfinite(cond[i]))
                if len(cand):
                    b=cand[int(np.argmax(np.abs(cond[i,cand])))]
                    if cond[i,b]>0: excited[i]=True
                    elif cond[i,b]<0: suppressed[i]=True
            ...
            fi_resp=fo_resp=am_resp=sound_resp=excited=suppressed=np.zeros(n_cells,bool)
        classes=np.asarray(arc["cell_class"]).astype(str)
        yellow=(classes=="yellow") & classified
        rates=np.asarray(arc["possible_am_rates_hz"],float)
        for i in np.flatnonzero(yellow):
            row={
                "subject":str(full_yellow_cells["subject"].iloc[0]),"date":"","session":"",
                "session_key":str(np.asarray(arc["session_key"]).item()),
                "canonicalArea":str(np.asarray(arc["canonical_area"]).item()),
                "brainArea":"","depth":float(srow["depth"]),"cell_class":"yellow",
                "analysis_index":int(np.asarray(arc["analysis_index"])[i]),
                "suite2p_roi":int(np.asarray(arc["suite2p_roi"])[i]),
                "fade_in_response":float(fi[i]),"fade_out_response":float(fo[i]),
                FI_FO_PRIMARY:float(fi_fo[i]),FI_FO_SESSION_MEDIAN:float(fi_fo_sm[i]),
                FI_FO_PRESTIM60:float(fi_fo_pre[i]),FI_BEST_AM:float(fi_best[i]),AM_SPARSE:float(sparse[i]),
                "fade_in_responsive_fdr":bool(fi_resp[i]),"fade_out_responsive_fdr":bool(fo_resp[i]),
                "fi_or_fo_responsive_fdr":bool(fi_resp[i] or fo_resp[i]),
                "any_am_rate_responsive_fdr":bool(am_resp[i]),
                "fi_or_any_am_responsive_fdr":bool(fi_resp[i] or am_resp[i]),
                "sound_responsive_fdr":bool(sound_resp[i]),"sound_excited_fdr":bool(excited[i]),
                "sound_suppressed_fdr":bool(suppressed[i]),
            }
            for j,rate in enumerate(rates):
                if j<am.shape[1]:
                    row[f"am_response_{rate:g}Hz"]=float(am[i,j])
            rows.append(row)
    return pd.DataFrame(rows),all_counts


def significant_trial_reduction_targets(full_stats: dict[str,Any]) -> list[dict[str,Any]]:
    """Every individually significant depth-stratum result, including sensitivity/exploratory outputs."""
    targets=[]
    def add(label,family,branch,key,test,p,scope,raw_p=np.nan,stat_name="",stat_value=np.nan,correction=""):
        if np.isfinite(p) and p<0.05:
            targets.append({"id":f"{family}::{branch}::{key}::{test}","label":label,"family":family,
                            "branch":branch,"key":key,"test":test,"original_p_adjusted":float(p),
                            "original_p_raw":float(raw_p) if np.isfinite(raw_p) else np.nan,
                            "scope":scope,"stat_name":stat_name,
                            "stat_value":float(stat_value) if np.isfinite(stat_value) else np.nan,
                            "correction":correction})
    for metric,wrap in full_stats["responsiveness"].items():
        rec=wrap["session_level_primary"]; o=rec["omnibus"]
        add(f"{wrap['label']} — omnibus A1/AAF/A2","responsiveness","",metric,"omnibus",
            o.get("p_holm_family",np.nan),"PRIMARY session-level",o.get("p",np.nan),
            "Kruskal-Wallis H",o.get("H",np.nan),"Holm across 7 responsiveness omnibus tests")
        for pk,r in rec["pairwise"].items():
            add(f"{wrap['label']} — {r['area1']} vs {r['area2']}","responsiveness","",metric,f"pair:{pk}",
                r.get("p_holm",np.nan),"PRIMARY session-level",r.get("p",np.nan),
                "Mann-Whitney U",r.get("U",np.nan),"Holm across 3 area pairs")
    for branch in ("all_cells","responsive_only"):
        for metric in (FI_FO_PRIMARY,FI_BEST_AM):
            wrap=full_stats["selectivity"][branch][metric]; rec=wrap["session_level_primary"]; o=rec["omnibus"]
            add(f"{wrap['label']} [{branch}] — omnibus A1/AAF/A2","selectivity_primary",branch,metric,"omnibus",
                o.get("p_holm_family",np.nan),"PRIMARY session-level",o.get("p",np.nan),
                "Kruskal-Wallis H",o.get("H",np.nan),"Holm across FI/FO + FI/best-AM omnibus tests")
            for pk,r in rec["pairwise"].items():
                add(f"{wrap['label']} [{branch}] — {r['area1']} vs {r['area2']}","selectivity_primary",branch,metric,f"pair:{pk}",
                    r.get("p_holm",np.nan),"PRIMARY session-level",r.get("p",np.nan),
                    "Mann-Whitney U",r.get("U",np.nan),"Holm across 3 area pairs")
        for metric,wrap in full_stats["selectivity"][branch].get("sensitivity",{}).items():
            rec=wrap["session_level_sensitivity"]; o=rec["omnibus"]
            add(f"{wrap['label']} [{branch}] — omnibus A1/AAF/A2","selectivity_sensitivity",branch,metric,"omnibus",
                o.get("p",np.nan),"SENSITIVITY session-level",o.get("p",np.nan),
                "Kruskal-Wallis H",o.get("H",np.nan),"No across-sensitivity family correction")
            for pk,r in rec["pairwise"].items():
                add(f"{wrap['label']} [{branch}] — {r['area1']} vs {r['area2']}","selectivity_sensitivity",branch,metric,f"pair:{pk}",
                    r.get("p_holm",np.nan),"SENSITIVITY session-level",r.get("p",np.nan),
                    "Mann-Whitney U",r.get("U",np.nan),"Holm across 3 area pairs")
    for branch,fam in full_stats.get("exploratory_pooled",{}).get("selectivity",{}).items():
        for metric,rec in fam.items():
            o=rec["omnibus"]
            add(f"{METRIC_LABELS.get(metric,metric)} [{branch}] — pooled omnibus A1/AAF/A2",
                "selectivity_pooled",branch,metric,"omnibus",o.get("p",np.nan),"EXPLORATORY pooled-cell",
                o.get("p",np.nan),"Kruskal-Wallis H",o.get("H",np.nan),"No across-metric family correction")
            for pk,r in rec["pairwise"].items():
                add(f"{METRIC_LABELS.get(metric,metric)} [{branch}] — pooled {r['area1']} vs {r['area2']}",
                    "selectivity_pooled",branch,metric,f"pair:{pk}",r.get("p_holm",np.nan),"EXPLORATORY pooled-cell",
                    r.get("p",np.nan),"Mann-Whitney U",r.get("U",np.nan),"Holm across 3 area pairs")
    for branch,fam in full_stats["condition_profiles"].items():
        for col,wrap in fam.items():
            rec=wrap["session_level_primary"]; o=rec["omnibus"]
            add(f"Condition {wrap['label']} [{branch}] — omnibus A1/AAF/A2",
                "condition_profiles",branch,col,"omnibus",o.get("p_holm_family",np.nan),"PRIMARY session-level",
                o.get("p",np.nan),"Kruskal-Wallis H",o.get("H",np.nan),f"Holm across conditions within {branch}")
            for pk,r in rec["pairwise"].items():
                add(f"Condition {wrap['label']} [{branch}] — {r['area1']} vs {r['area2']}",
                    "condition_profiles",branch,col,f"pair:{pk}",r.get("p_holm",np.nan),"PRIMARY session-level",
                    r.get("p",np.nan),"Mann-Whitney U",r.get("U",np.nan),"Holm across 3 area pairs")
    return targets
...
def extract_target_p(stats_out: dict[str,Any], target: dict[str,Any]) -> float:
    fam=target["family"]; branch=target["branch"]; key=target["key"]; test=target["test"]
    if fam=="responsiveness":
        rec=stats_out["responsiveness"][key]["session_level_primary"]
    elif fam=="selectivity_primary":
        rec=stats_out["selectivity"][branch][key]["session_level_primary"]
    elif fam=="selectivity_sensitivity":
        rec=stats_out["selectivity"][branch]["sensitivity"][key]["session_level_sensitivity"]
    elif fam=="selectivity_pooled":
        rec=stats_out["exploratory_pooled"]["selectivity"][branch][key]
    elif fam=="condition_profiles":
        rec=stats_out["condition_profiles"][branch][key]["session_level_primary"]
    else:
        return np.nan
    if test=="omnibus":
        if fam in {"responsiveness","selectivity_primary","condition_profiles"}:
            return float(rec["omnibus"].get("p_holm_family",np.nan))
        return float(rec["omnibus"].get("p",np.nan))
    pk=test.split(":",1)[1]
    return float(rec["pairwise"].get(pk,{}).get("p_holm",np.nan))


def target_requires_responsiveness(targets: list[dict[str,Any]]) -> bool:
    return any(t["family"]=="responsiveness" or t.get("branch") in
               {"responsive_only","responsive_fi_fo","responsive_am"} for t in targets)


def _target_test_description(target: dict[str,Any]) -> str:
    if target["test"]=="omnibus":
        exact=f"Kruskal-Wallis A1 vs AAF vs A2 ({target['stat_name']}={_num_text(target['stat_value'])})"
    elif target["test"]=="layer_pair":
        exact=f"two-sided Mann-Whitney U, {target['area']} L2/3 vs {target['area']} L4/5 (U={_num_text(target['stat_value'])})"
    else:
        pair=target["test"].split(":",1)[1].replace("__"," vs ")
        exact=f"two-sided Mann-Whitney U, {pair} (U={_num_text(target['stat_value'])})"
    return f"{target['scope']} | {exact} | {target['correction']}"


def plot_trial_reduction_robustness(target: dict[str,Any], raw: pd.DataFrame,
                                    count_summary: dict[float,tuple[float,int,int]], depth_label: str,
                                    output_dir: Path) -> Path:
    fractions=np.asarray(TRIAL_REDUCTION_FRACTIONS,float)
    x=100*fractions; med=[]; lo=[]; hi=[]; retain=[]
    for f in fractions:
        vals=finite_values(raw.loc[raw["fraction"]==f,"p_adjusted"])
        if len(vals):
            med.append(np.median(vals)); lo.append(np.percentile(vals,10)); hi.append(np.percentile(vals,90)); retain.append(np.mean(vals<0.05))
        else:
            med.append(np.nan); lo.append(np.nan); hi.append(np.nan); retain.append(np.nan)
    med=np.asarray(med,float); lo=np.asarray(lo,float); hi=np.asarray(hi,float); retain=np.asarray(retain,float)
    fig,(ax1,ax2)=plt.subplots(2,1,figsize=(11,8.8),sharex=True,gridspec_kw={"height_ratios":[1.05,1.0]})
    ax1.plot(x,np.clip(med,1e-6,1),marker="o")
    ax1.fill_between(x,np.clip(lo,1e-6,1),np.clip(hi,1e-6,1),alpha=0.15)
    ax1.axhline(0.05,color="0.35",ls="--",lw=1)
    ax1.set_yscale("log")
    ax1.set_ylabel("Same test/correction p\nmedian (10th-90th pct)")
    ax1.grid(alpha=0.2); ax1.spines[["top","right"]].set_visible(False)
    ax2.plot(x,retain,marker="o")
    ax2.axhline(0.8,color="0.6",ls=":",lw=1); ax2.axhline(0.9,color="0.7",ls=":",lw=1)
    ax2.set_ylim(-0.02,1.02)
    ax2.set_ylabel("Fraction retaining\ncorrected p < 0.05")
    ...
    ax2.set_xlabel("Fraction of currently available repetitions retained")
    fig.suptitle(
        f"YELLOW trial-reduction robustness — {depth_label}\n"
        f"EXPLICIT SIGNIFICANT RESULT: {target['label']}\n"
        f"{_target_test_description(target)}\n"
        f"Original raw {p_text(target.get('original_p_raw',np.nan))}; "
        f"original p after stated correction {p_text(target.get('original_p_adjusted',np.nan))}",
        fontsize=10.2)
    stem=_safe_stem(target["label"]+"__"+target["scope"])
    path=output_dir/f"trial_reduction__{stem}__ROBUSTNESS.png"
...
def run_trial_reduction_robustness(full_yellow_cells: pd.DataFrame, full_stats: dict[str,Any],
                                   areas: list[str], depth_label: str, output_dir: Path,
                                   n_resamples: int, response_permutations: int, seed: int) -> dict[str,Any]:
    targets=significant_trial_reduction_targets(full_stats)
    if not targets:
        return {"performed":False,"reason":"No significant YELLOW tests represented in this depth-stratum output"}
    root=output_dir/"trial_reduction_robustness"
    root.mkdir(parents=True,exist_ok=True)
    rng=np.random.default_rng(seed)
    archive_cache={}
    sign_cache=SignCache(response_permutations,seed+991)
    need_resp=target_requires_responsiveness(targets)
    raw_rows=[]; count_summary={}; counts=[]
    for path in full_yellow_cells["trial_response_archive_path"].dropna().astype(str).unique():
        arc=load_trial_archive(path,archive_cache)
        _,cc=_sample_condition_trials(arc,1.0,rng)
        counts.extend(cc)
    count_summary[1.0]=(float(np.median(counts)) if counts else np.nan,
                        int(np.min(counts)) if counts else 0,int(np.max(counts)) if counts else 0)
    for t in targets:
        raw_rows.append({"fraction":1.0,"resample":0,"target_id":t["id"],
                         "target_label":t["label"],"p_adjusted":t["original_p_adjusted"]})
    for frac in TRIAL_REDUCTION_FRACTIONS[1:]:
        first_counts=None
        for rep in range(int(n_resamples)):
            reduced,counts=reduced_yellow_cells(full_yellow_cells,float(frac),rng,archive_cache,
                                                 sign_cache,recompute_responsiveness=need_resp)
            if first_counts is None:
                first_counts=list(counts)
            try:
                sfull=build_full_stats(reduced,areas)
            except Exception:
                sfull=None
            for t in targets:
                try:
                    pv=extract_target_p(sfull,t) if sfull is not None else np.nan
                except Exception:
                    pv=np.nan
                raw_rows.append({"fraction":float(frac),"resample":int(rep),"target_id":t["id"],
                                 "target_label":t["label"],"p_adjusted":pv})
        cc=first_counts or []
        count_summary[float(frac)]=(float(np.median(cc)) if cc else np.nan,
                                    int(np.min(cc)) if cc else 0,int(np.max(cc)) if cc else 0)
    raw=pd.DataFrame(raw_rows)
    raw.to_csv(root/"trial_reduction_all_significant_targets_RAW.csv",index=False)
    summary_rows=[]; figure_paths=[]
    for i,t in enumerate(targets,1):
        traw=raw[raw["target_id"]==t["id"]].copy()
        stem=f"{i:03d}__{_safe_stem(t['label'])}"
        traw.to_csv(root/f"{stem}__RAW.csv",index=False)
        figure_paths.append(str(plot_trial_reduction_robustness(t,traw,count_summary,depth_label,root)))
        for frac in TRIAL_REDUCTION_FRACTIONS:
            vals=finite_values(traw.loc[traw["fraction"]==frac,"p_adjusted"])
            medn,mn,mx=count_summary[float(frac)]
            summary_rows.append({
                "target_number":i,"target_id":t["id"],"target":t["label"],"scope":t["scope"],
                "exact_test":_target_test_description(t),"fraction":frac,"percent_trials":100*frac,
                "median_trials_per_condition":medn,"min_trials_per_condition":mn,"max_trials_per_condition":mx,
                "n_valid_resamples":len(vals),"median_p_adjusted":float(np.median(vals)) if len(vals) else np.nan,
                "p10":float(np.percentile(vals,10)) if len(vals) else np.nan,
                "p90":float(np.percentile(vals,90)) if len(vals) else np.nan,
                "significance_retention":float(np.mean(vals<0.05)) if len(vals) else np.nan})
    pd.DataFrame(summary_rows).to_csv(root/"trial_reduction_summary.csv",index=False)
    save_json(root/"trial_reduction_manifest.json",{
        "fractions":list(TRIAL_REDUCTION_FRACTIONS),"n_resamples_per_reduced_fraction":int(n_resamples),
        "response_signflip_permutations":int(response_permutations),"seed":int(seed),
        "sampling":"without replacement within session x stimulus condition",
        "100_percent":"original full-data result; not re-randomized","targets":targets,
        "trial_count_summary":{str(k):v for k,v in count_summary.items()},
        "responsiveness_recomputed_for_reduced_data":bool(need_resp),
        "one_robustness_figure_per_individual_significant_test":True})
    return {"performed":True,"n_targets":len(targets),"output_root":str(root),"figures":figure_paths}


def _area_layer_significant_targets(stats_all: dict[str,Any]) -> list[dict[str,Any]]:
    targets=[]
    for branch,fam in stats_all.items():
        for metric,by_area in fam.items():
            session_level=(metric!=AM_SPARSE)
            for area,r in by_area.items():
                ph=r.get("p_holm_across_areas",np.nan)
                if np.isfinite(ph) and ph<0.05:
                    targets.append({
                        "id":f"area_layer::{branch}::{metric}::{area}",
                        "label":f"{METRIC_LABELS.get(metric,metric)} [{branch}] — {area} L2/3 vs L4/5",
                        "family":"area_layer","branch":branch,"key":metric,"area":area,"test":"layer_pair",
                        "original_p_adjusted":float(ph),"original_p_raw":float(r.get("p",np.nan)),
                        "scope":("SENSITIVITY session-level" if metric in {FI_FO_SESSION_MEDIAN,FI_FO_PRESTIM60} else "PRIMARY session-level") if session_level else "EXPLORATORY pooled-cell",
                        "stat_name":"Mann-Whitney U","stat_value":float(r.get("U",np.nan)),
                        "correction":"Holm across the 3 area-specific L2/3-vs-L4/5 tests",
                        "session_level":session_level})
    return targets


def _area_layer_target_p(reduced: pd.DataFrame,target: dict[str,Any],areas: list[str]) -> float:
    metric=target["key"]
    df=reduced
    if target["branch"]=="responsive_only":
        kind="fi_fo_responsive" if metric in {FI_FO_PRIMARY,FI_FO_SESSION_MEDIAN,FI_FO_PRESTIM60} else \
             "fi_am_responsive" if metric==FI_BEST_AM else "am_responsive"
        df=analysis_subset(df,kind)
    rec=_layer_pair_stats(add_layer_columns(df),metric,areas,session_level=bool(target["session_level"]))
    return float(rec.get(target["area"],{}).get("p_holm_across_areas",np.nan))


def run_area_layer_trial_reduction_robustness(full_yellow_cells: pd.DataFrame, stats_all: dict[str,Any],
                                               areas: list[str], root: Path, n_resamples: int,
                                               response_permutations: int, seed: int) -> dict[str,Any]:
    targets=_area_layer_significant_targets(stats_all)
    if not targets:
        return {"performed":False,"reason":"No significant YELLOW area × layer tests"}
    out=root/"trial_reduction_robustness"
    out.mkdir(parents=True,exist_ok=True)
    rng=np.random.default_rng(seed); archive_cache={}; sign_cache=SignCache(response_permutations,seed+1771)
    need_resp=any(t["branch"]=="responsive_only" for t in targets)
    rows=[]; counts=[]
    for path in full_yellow_cells["trial_response_archive_path"].dropna().astype(str).unique():
        arc=load_trial_archive(path,archive_cache); _,cc=_sample_condition_trials(arc,1.0,rng); counts.extend(cc)
    count_summary={1.0:(float(np.median(counts)) if counts else np.nan,
                        int(np.min(counts)) if counts else 0,int(np.max(counts)) if counts else 0)}
    for t in targets:
        rows.append({"fraction":1.0,"resample":0,"target_id":t["id"],"target_label":t["label"],
                     "p_adjusted":t["original_p_adjusted"]})
    for frac in TRIAL_REDUCTION_FRACTIONS[1:]:
        first=None
        for rep in range(int(n_resamples)):
            red,cc=reduced_yellow_cells(full_yellow_cells,float(frac),rng,archive_cache,sign_cache,
                                        recompute_responsiveness=need_resp)
            if first is None:
                first=list(cc)
            for t in targets:
                try: pv=_area_layer_target_p(red,t,areas)
                except Exception: pv=np.nan
                rows.append({"fraction":float(frac),"resample":rep,"target_id":t["id"],
                             "target_label":t["label"],"p_adjusted":pv})
        cc=first or []
        count_summary[float(frac)]=(float(np.median(cc)) if cc else np.nan,
                                    int(np.min(cc)) if cc else 0,int(np.max(cc)) if cc else 0)
    raw=pd.DataFrame(rows)
    raw.to_csv(out/"trial_reduction_all_significant_area_layer_targets_RAW.csv",index=False)
    summary=[]
    for i,t in enumerate(targets,1):
        traw=raw[raw["target_id"]==t["id"]].copy()
        stem=f"{i:03d}__{_safe_stem(t['label'])}"
        traw.to_csv(out/f"{stem}__RAW.csv",index=False)
        plot_trial_reduction_robustness(t,traw,count_summary,"Area × layer",out)
        for frac in TRIAL_REDUCTION_FRACTIONS:
            vals=finite_values(traw.loc[traw["fraction"]==frac,"p_adjusted"])
            medn,mn,mx=count_summary[float(frac)]
            summary.append({"target_number":i,"target":t["label"],"scope":t["scope"],
                            "exact_test":_target_test_description(t),"fraction":frac,"percent_trials":100*frac,
                            "median_trials_per_condition":medn,"min_trials_per_condition":mn,"max_trials_per_condition":mx,
                            "n_valid_resamples":len(vals),"median_p_adjusted":float(np.median(vals)) if len(vals) else np.nan,
                            "significance_retention":float(np.mean(vals<0.05)) if len(vals) else np.nan})
    pd.DataFrame(summary).to_csv(out/"trial_reduction_summary.csv",index=False)
    save_json(out/"trial_reduction_manifest.json",{"targets":targets,"fractions":list(TRIAL_REDUCTION_FRACTIONS),
              "n_resamples":int(n_resamples),"response_permutations":int(response_permutations),"seed":int(seed),
              "one_figure_per_individual_significant_test":True})
    return {"performed":True,"n_targets":len(targets),"output_root":str(out)}


def make_area_layer_outputs(all_cells: pd.DataFrame, areas: list[str], out_root: Path,
                            *, trial_reduction: bool = True, n_resamples: int = 500,
                            response_permutations: int = 512, seed: int = 20260826) -> None:
    root=out_root/"area_by_layer"; root.mkdir(parents=True,exist_ok=True)
    for cell_class in CELL_CLASSES:
        cells=all_cells[all_cells["cell_class"]==cell_class].copy()
        out=root/cell_class; out.mkdir(parents=True,exist_ok=True)
        stats_all={}
        for branch in ("all_cells","responsive_only"):
            stats_all[branch]={}
            fi_df=cells if branch=="all_cells" else analysis_subset(cells,"fi_fo_responsive")
            for metric in (FI_FO_PRIMARY,FI_FO_SESSION_MEDIAN,FI_FO_PRESTIM60):
                stats_all[branch][metric]=plot_area_layer_metric(fi_df,metric,areas,
                    f"{cell_class.upper()} area x layer — {METRIC_LABELS[metric]} — {branch.replace('_',' ')}",
                    out/f"area_layer_{metric}_{branch}_SESSION_POINTS.png",session_level=True)
            fam_df=cells if branch=="all_cells" else analysis_subset(cells,"fi_am_responsive")
            stats_all[branch][FI_BEST_AM]=plot_area_layer_metric(fam_df,FI_BEST_AM,areas,
                f"{cell_class.upper()} area x layer — {METRIC_LABELS[FI_BEST_AM]} — {branch.replace('_',' ')}",
                out/f"area_layer_{FI_BEST_AM}_{branch}_SESSION_POINTS.png",session_level=True)
            ...
            stats_all[branch][AM_SPARSE]=plot_area_layer_metric(am_df,AM_SPARSE,areas,
                ...
                out/f"area_layer_{AM_SPARSE}_{branch}_CELL_POINTS.png",session_level=False)
        save_json(out/"area_layer_statistics.json",stats_all)
        if cell_class=="yellow" and trial_reduction:
            tr=run_area_layer_trial_reduction_robustness(cells,stats_all,areas,out,
                max(1,int(n_resamples)),max(64,int(response_permutations)),int(seed)+77777)
...
def write_report(path: Path, subject: str, dates: list[str], cell_class: str, depth_label: str,
    ...
    lines=["CROSS-AREA SOUND-TUNING ANALYSIS v4","="*78,f"Subject: {subject}",f"Dates: {', '.join(dates)}",
...
def main() -> None:
    ...
    print("="*82); print("SOUND_TUNING CROSS-AREA CELL-CLASS ANALYSIS v4"); print("="*82)
    ...
    manifest={
        "script_version":"v4","prerequisite":"sound_tuning_area_analysis_v13.py","subject":subject,"dates":dates,"areas":areas,
        ...
        "trial_reduction":{"enabled":not args.skip_trial_reduction,"fractions":list(TRIAL_REDUCTION_FRACTIONS),
                           ...
                           "seed":args.trial_reduction_seed,"yellow_all_individual_significant_results":True,"one_figure_per_significant_test":True},
    }
    ...
    for depth_key,(_,_,depth_label) in DEPTH_STRATA.items():
        ...
        for cell_class in CELL_CLASSES:
            ...
            if cell_class=="yellow" and not args.skip_trial_reduction:
                ...
                save_json(out/"trial_reduction_status.json",tr)
                if tr.get("performed"): print(f"    Trial reduction: {tr['n_targets']} individually significant target(s) -> {tr['output_root']}")
                else: print(f"    Trial reduction: {tr.get('reason','not performed')}")
            write_figure_index(out)

    make_area_layer_outputs(all_cells,areas,out_root,trial_reduction=not args.skip_trial_reduction,
                            n_resamples=max(1,int(args.trial_reduction_resamples)),
                            response_permutations=max(64,int(args.trial_reduction_response_permutations)),
                            seed=int(args.trial_reduction_seed))
