...
Prerequisite
------------
sound_tuning_multistim_fov_analysis_v2.py must have been run for each area.
...
def plot_cell_profiles(df,block,area,layer_label,out,branch):
    use=block_data_cells(df,block) if branch=='all_cells' else responsive_only(df,block); cols=condition_columns(use,block)
    if not cols:return
    vals=np.array([v for v,_ in cols]); x=np.log2(vals/1000) if block=='pure_tones' else vals; fig,axes=plt.subplots(1,2,figsize=(13,5.3),sharey=True)
...
def main():
    ...
    print("="*80); print("MULTISTIM AREA x LAYER GREEN/YELLOW ANALYSIS v2b"); print("="*80)
