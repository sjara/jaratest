...
This is the third-stage follow-up analysis for am_tuning_area_analysis_v8.py.
...
These are produced by am_tuning_area_analysis_v8.py.
...
CONTINUOUS_METRICS = {
    ...
    # Primary and comparison tuning-selectivity metrics from am_tuning_area_analysis_v8.py.
...
def load_mode_tables(
    ...
    for area in areas:
        ...
        for df, path in ((cdf, cp), (qdf, qp)):
            if "canonicalArea" not in df.columns:
                raise ValueError(f"{path} has no canonicalArea column; expected am_tuning_area_analysis_v8.py output.")
    ...
    if missing:
        ...
        msg += "\nRun am_tuning_area_analysis_v8.py for each missing area using the same dates first."
...
def main() -> None:
    ...
    save_json(outroot/"run_manifest.json",{
        ...
        "prerequisite_area_analysis":"am_tuning_area_analysis_v8.py",
