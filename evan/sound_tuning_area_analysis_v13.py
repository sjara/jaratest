...
* Figure/output and responsiveness revisions retained from v11:
...
* v12 behavior retained: the two alternate dF/F definitions used for eventlocked
  QC are propagated into per-cell condition-response outputs. For each alternate
  dF/F, the script exports FI, FO, every AM-rate response, best-AM response/rate,
  FI/FO selectivity, FI/best-AM selectivity, and AM selectivity metrics.
* New in v13: every successfully analyzed session also writes a compact compressed
  trial-response archive for downstream trial-count / repetition-power analyses.
  The archive contains one response value per cell per behavioral trial for the
  primary jaratoolbox dff=True analysis and both alternate dF/F definitions, plus
  trial-condition identities, valid-trial flags, cell/ROI identities, and the
  original condition-specific FDR results. This is sufficient for downstream
  Monte-Carlo subsampling of trial repetitions without rerunning Suite2p,
  Cellpose, event alignment, or eventlocked extraction.
  Responsiveness/FDR discovery in the ordinary v13 outputs remains defined only
  from the primary jaratoolbox dff=True analysis; alternate dF/F remains a
...
def fdr_sound_responsive_subset(cell_df: pd.DataFrame, kind: str = "any") -> pd.DataFrame:
    """Return an analysis-specific FDR-responsive subset.

    kind:
      any     = responsive to at least one tested sound condition
      fi      = fade-in responsive
      fo      = fade-out responsive
      fi_fo   = fade-in OR fade-out responsive
      am      = responsive to at least one individual AM rate
      fi_am   = fade-in OR at least one AM rate responsive
    """
    column_map = {
        "any": "sound_responsive_fdr",
        "fi": "fade_in_responsive_fdr",
        "fo": "fade_out_responsive_fdr",
        "fi_fo": "fi_or_fo_responsive_fdr",
        "am": "any_am_rate_responsive_fdr",
        "fi_am": "fi_or_any_am_responsive_fdr",
    }
    if kind not in column_map:
        raise ValueError(f"Unknown responsive subset kind: {kind}")
    column = column_map[kind]
    if column not in cell_df.columns:
        return cell_df.iloc[0:0].copy()
    return cell_df[
        cell_df["cell_class"].isin(["green", "yellow"])
        & cell_df[column].astype(bool)
    ].copy()


def subset_session_class_summary(cell_df: pd.DataFrame) -> pd.DataFrame:
    """Build session x class summaries for any supplied cell subset."""
    rows = []
    ...
    for (session_key, class_name), df in classified.groupby(["session_key", "cell_class"], sort=False):
        ...
        bool_cols = [
            "sound_responsive_fdr", "sound_excited_fdr", "sound_suppressed_fdr",
            "fade_in_responsive_fdr", "fade_out_responsive_fdr",
            "fi_or_fo_responsive_fdr", "any_am_rate_responsive_fdr",
            "fi_or_any_am_responsive_fdr",
        ]
        for col in bool_cols:
            if col in df.columns:
                count = int(np.sum(df[col].astype(bool)))
                row[f"n_{col}"] = count
                row[f"pct_{col}"] = 100.0 * count / n if n else np.nan
        metric_cols = [
            "fade_in_response", "fade_out_response", "fade_in_vs_fade_out_selectivity",
            "fade_in_vs_best_am_selectivity", "am_rate_lifetime_sparseness",
            "am_rate_gini", "am_rate_fano_factor", "am_rate_fwhm_hz",
        ]
        for col in metric_cols:
            if col in df.columns:
                vals = finite_values(df[col])
                row[f"mean_{col}"] = float(np.mean(vals)) if len(vals) else np.nan
                row[f"std_{col}"] = float(np.std(vals, ddof=1)) if len(vals) > 1 else np.nan
        # Compatibility names used by older plotting/statistics helpers.
        row["n_sound_responsive_fdr"] = int(np.sum(df.get("sound_responsive_fdr", False))) if n else 0
        row["pct_sound_responsive_fdr"] = 100.0 * row["n_sound_responsive_fdr"] / n if n else np.nan
        row["n_sound_excited_fdr"] = int(np.sum(df.get("sound_excited_fdr", False))) if n else 0
        row["pct_sound_excited_fdr"] = 100.0 * row["n_sound_excited_fdr"] / n if n else np.nan
        row["n_sound_suppressed_fdr"] = int(np.sum(df.get("sound_suppressed_fdr", False))) if n else 0
        row["pct_sound_suppressed_fdr"] = 100.0 * row["n_sound_suppressed_fdr"] / n if n else np.nan
...
def bounded_fi_fo_selectivity(response_fi: np.ndarray, response_fo: np.ndarray) -> np.ndarray:
    """Bounded signed FI/FO selectivity: (FI-FO)/(|FI|+|FO|)."""
    fi = np.asarray(response_fi, dtype=float)
    fo = np.asarray(response_fo, dtype=float)
    fi, fo = np.broadcast_arrays(fi, fo)
    out = np.full(fi.shape, np.nan, dtype=float)
    denom = np.abs(fi) + np.abs(fo)
    valid = np.isfinite(fi) & np.isfinite(fo) & np.isfinite(denom) & (denom > 0)
    out[valid] = (fi[valid] - fo[valid]) / denom[valid]
    return out


def gini_coefficient_rows(response_matrix: np.ndarray) -> np.ndarray:
    """Gini coefficient of the absolute AM response profile for each cell."""
    matrix = np.asarray(response_matrix, dtype=float)
    if matrix.ndim == 1:
        matrix = matrix[None, :]
    out = np.full(matrix.shape[0], np.nan)
    for i, row in enumerate(matrix):
        vals = np.abs(row[np.isfinite(row)])
        if len(vals) < 2 or np.sum(vals) <= 1e-15:
            continue
        vals = np.sort(vals)
        n = len(vals)
        ranks = np.arange(1, n + 1, dtype=float)
        out[i] = np.sum((2 * ranks - n - 1) * vals) / (n * np.sum(vals))
    return out


def fano_factor_rows(response_matrix: np.ndarray) -> np.ndarray:
    """Variance/mean of the absolute AM response profile for each cell."""
    matrix = np.asarray(response_matrix, dtype=float)
    if matrix.ndim == 1:
        matrix = matrix[None, :]
    out = np.full(matrix.shape[0], np.nan)
    for i, row in enumerate(matrix):
        ...
        if mean_val <= 1e-15:
            continue
        out[i] = float(np.var(vals, ddof=1) / mean_val)
    return out


def fwhm_bandwidth_rows(response_matrix: np.ndarray, rates_hz: np.ndarray) -> np.ndarray:
    """FWHM in linear Hz around the largest absolute AM-rate response peak.

    Returns NaN when a half-height crossing cannot be bracketed on both sides.
    """
    matrix = np.asarray(response_matrix, dtype=float)
    ...
    out = np.full(matrix.shape[0], np.nan)
    for i, row in enumerate(matrix):
        valid = np.isfinite(row) & np.isfinite(rates)
        x = rates[valid]
        y = np.abs(row[valid])
        if len(x) < 3:
            continue
        ...
        peak_y = float(y[peak])
        if peak_y <= 1e-15 or peak == 0 or peak == len(y) - 1:
            continue
        half = 0.5 * peak_y
        left_cross = np.nan
        for j in range(peak - 1, -1, -1):
            ...
            if (y0 <= half <= y1) or (y1 <= half <= y0):
                if y1 == y0:
                    left_cross = float(x[j])
                else:
                    left_cross = float(x[j] + (half - y0) * (x[j + 1] - x[j]) / (y1 - y0))
                break
        right_cross = np.nan
        for j in range(peak, len(y) - 1):
            y0, y1 = y[j], y[j + 1]
            if (y0 >= half >= y1) or (y1 >= half >= y0):
                if y1 == y0:
                    right_cross = float(x[j + 1])
                else:
                    right_cross = float(x[j] + (half - y0) * (x[j + 1] - x[j]) / (y1 - y0))
                break
        if np.isfinite(left_cross) and np.isfinite(right_cross) and right_cross >= left_cross:
            out[i] = right_cross - left_cross
    return out


def safe_friedman(matrix: np.ndarray) -> dict[str, Any]:
    matrix = np.asarray(matrix, dtype=float)
    if matrix.ndim != 2 or matrix.shape[0] < 2 or matrix.shape[1] < 2:
        return {"statistic": None, "p": None, "n_sessions": int(matrix.shape[0]) if matrix.ndim == 2 else 0}
    valid_rows = np.all(np.isfinite(matrix), axis=1)
    matrix = matrix[valid_rows]
    if matrix.shape[0] < 2:
        return {"statistic": None, "p": None, "n_sessions": int(matrix.shape[0])}
    try:
        res = stats.friedmanchisquare(*[matrix[:, j] for j in range(matrix.shape[1])])
        return {"statistic": float(res.statistic), "p": float(res.pvalue), "n_sessions": int(matrix.shape[0])}
    except ValueError:
        return {"statistic": None, "p": None, "n_sessions": int(matrix.shape[0])}
...
def _save_eventlocked_average_figure(eventlocked_data, valid_events, tvec, output_path, title, cbar_label="dF/F"):
    ...
    if not np.any(valid_events):
        return
    trialavg = np.nanmean(np.asarray(eventlocked_data)[:, valid_events, :], axis=1)
    fig = plt.figure(figsize=(10, 8), constrained_layout=True)
    gs = fig.add_gridspec(4, 1)
    ax0 = fig.add_subplot(gs[:3, 0])
    im = ax0.imshow(trialavg, interpolation="nearest", extent=[tvec[0], tvec[-1], trialavg.shape[0], 0],
...
def _session_median_dff(eventlocked_raw, full_session_raw):
    ...
    valid = np.isfinite(f0) & (np.abs(f0) > 1e-12)
    out = np.full_like(eventlocked_raw, np.nan, dtype=float)
    out[valid] = (eventlocked_raw[valid] - f0[valid, None, None]) / f0[valid, None, None]
    return out


def _local_prestim_median_dff(eventlocked_raw, event_onset_samples, srate, valid_events, tvec,
                              local_window_s=60.0, prestim_window_s=(-0.300, 0.0)):
    """Local dF/F using only prestimulus samples from stimuli in a centered 60-s window."""
    ...
    if not np.any(pre):
        return out
    half_window = local_window_s / 2.0
    for e in range(raw.shape[1]):
        neighbors = valid_events & np.isfinite(onset_s) & (np.abs(onset_s - onset_s[e]) <= half_window)
        if not np.any(neighbors):
            continue
        baseline_samples = raw[:, neighbors, :][:, :, pre]
        f0 = np.nanmedian(baseline_samples, axis=(1, 2))
        valid_f0 = np.isfinite(f0) & (np.abs(f0) > 1e-12)
        out[valid_f0, e, :] = (raw[valid_f0, e, :] - f0[valid_f0, None]) / f0[valid_f0, None]
    return out


def _trial_response_from_dff(eventlocked_dff, baseline_mask_t, response_mask_t):
    """Return trial-baseline-corrected traces and one 0-1 s response per trial.

    This deliberately mirrors the primary analysis definition: for each trial,
    subtract the mean dF/F during BASELINE_WINDOW and then average during
    RESPONSE_WINDOW.  Keeping this operation identical isolates the effect of
    the dF/F F0 definition itself.
    """
    data = np.asarray(eventlocked_dff, dtype=float)
    baseline_mask_t = np.asarray(baseline_mask_t, dtype=bool)
    response_mask_t = np.asarray(response_mask_t, dtype=bool)
    trial_baseline = np.nanmean(data[:, :, baseline_mask_t], axis=2, keepdims=True)
    baseline_corrected = data - trial_baseline
    trial_response = np.nanmean(baseline_corrected[:, :, response_mask_t], axis=2)
    return baseline_corrected, trial_response


def _condition_response_matrix_from_trials(trial_response, condition_masks):
    """Mean response for each cell and condition using precomputed trial masks."""
    trial_response = np.asarray(trial_response, dtype=float)
    ...
    for condition_index, condition_mask in enumerate(condition_masks):
        ...
        if np.any(mask):
            out[:, condition_index] = np.nanmean(trial_response[:, mask], axis=1)
    return out


def _derived_condition_response_metrics(condition_responses, possible_am_rates):
    """Derive FI/FO and AM summaries from a condition-response matrix.

    Column convention must match the main analysis: FI, FO, then AM rates in
    possible_am_rates order.  No responsiveness tests are recomputed here.
    """
    cond = np.asarray(condition_responses, dtype=float)
    n_cells = cond.shape[0]
    fi = cond[:, 0] if cond.shape[1] >= 1 else np.full(n_cells, np.nan)
    fo = cond[:, 1] if cond.shape[1] >= 2 else np.full(n_cells, np.nan)
    am = cond[:, 2:] if cond.shape[1] > 2 else np.empty((n_cells, 0), dtype=float)

    mean_am = np.nanmean(am, axis=1) if am.shape[1] else np.full(n_cells, np.nan)
    best_am_response = np.full(n_cells, np.nan)
    best_am_rate = np.full(n_cells, np.nan)
    if am.shape[1]:
        finite_rows = np.any(np.isfinite(am), axis=1)
        rows = np.flatnonzero(finite_rows)
        if len(rows):
            best = np.nanargmax(am[rows], axis=1)
            best_am_response[rows] = am[rows, best]
            rates = np.asarray(possible_am_rates, dtype=float)
            best_am_rate[rows] = rates[best]

    return {
        "fade_in_response": fi,
        "fade_out_response": fo,
        "am_condition_responses": am,
        "mean_am_response_across_rates": mean_am,
        "best_am_response": best_am_response,
        "best_am_rate": best_am_rate,
        "fade_in_vs_fade_out_selectivity": bounded_fi_fo_selectivity(fi, fo),
        "fade_in_vs_best_am_selectivity": response_selectivity_index(fi, best_am_response),
        "am_rate_lifetime_sparseness": lifetime_sparseness(am),
        "am_rate_gini": gini_coefficient_rows(am),
        "am_rate_fano_factor": fano_factor_rows(am),
        "am_rate_fwhm_hz": fwhm_bandwidth_rows(am, np.asarray(possible_am_rates, dtype=float)),
    }


def _session_paired_fi_fo_response_figure(cell_df, output_path, title):
    fig, ax = plt.subplots(figsize=(9.0, 7.0))
    rng = np.random.default_rng(9271)
    centers = {"green": (0.0, 1.0), "yellow": (3.0, 4.0)}
    display = []
    for class_name, color in [("green", GREEN_COLOR), ("yellow", YELLOW_COLOR)]:
        df = cell_df[(cell_df["cell_class"] == class_name) & cell_df["fi_or_fo_responsive_fdr"].astype(bool)]
        fi = df["fade_in_response"].to_numpy(float)
        fo = df["fade_out_response"].to_numpy(float)
        valid = np.isfinite(fi) & np.isfinite(fo)
        fi, fo = fi[valid], fo[valid]
        x0, x1 = centers[class_name]
        jit = rng.uniform(-0.07, 0.07, len(fi))
        for a, b, j in zip(fi, fo, jit):
            ax.plot([x0+j, x1+j], [a, b], color=color, alpha=0.16, lw=0.7)
        ax.scatter(np.full(len(fi), x0)+jit, fi, color=color, alpha=0.35, s=18, edgecolors="none")
        ax.scatter(np.full(len(fo), x1)+jit, fo, color=color, alpha=0.35, s=18, edgecolors="none")
        for xx, vals in [(x0, fi), (x1, fo)]:
            m, sd, _, n = mean_sd_sem(vals)
            ax.errorbar(xx, m, yerr=sd if np.isfinite(sd) else None, fmt="o", color="black", capsize=5, lw=2.2, zorder=5)
            display.extend(finite_values(vals).tolist())
        stat = safe_paired_wilcoxon(fi, fo)
        ptxt = "n/a" if stat["p"] is None else f"{stat['p']:.3g}"
        ax.text((x0+x1)/2, 1.01, f"{class_name.upper()}: paired Wilcoxon p={ptxt}", transform=ax.get_xaxis_transform(),
                ha="center", va="bottom", fontsize=8)
    ax.axhline(0, color="black", ls=":", lw=1)
    ax.set_xticks([0,1,3,4], ["GREEN\nFade-in", "GREEN\nFade-out", "YELLOW\nFade-in", "YELLOW\nFade-out"])
    ax.set_ylabel("Mean baseline-subtracted dF/F")
    ax.set_title(title + "\nFI/FO-responsive cells only; points are cells, bars are mean ± SD")
    ax.grid(axis="y", alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    if display: apply_robust_y_limits(ax, [display], reference_values=[0.0])
    fig.savefig(output_path, dpi=300, bbox_inches="tight"); plt.close(fig)


def _session_stimulus_responsiveness_figure(cell_df, responsive_col, response_col, output_path, title, label):
    ...
    for pos, class_name, color in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
        ...
        vals = cdf[response_col].to_numpy(float) if response_col in cdf else np.full(len(cdf), np.nan)
        sig = cdf[responsive_col].astype(bool).to_numpy() if responsive_col in cdf else np.zeros(len(cdf),bool)
        x = np.full(len(cdf), pos, float) + rng.uniform(-0.13,0.13,len(cdf))
        ax.scatter(x[~sig], vals[~sig], s=17, color=color, alpha=0.18, edgecolors="none")
        ax.scatter(x[sig], vals[sig], s=30, color=color, alpha=0.80, marker="^", edgecolors="black", linewidths=0.25)
    ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks([0,1],["GREEN","YELLOW"])
    ax.set_ylabel("Mean baseline-subtracted dF/F"); ax.set_title(f"{label}: cell responses\ntriangles = FDR responsive")
    ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    ax=axes[1]
    for pos, class_name, color in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
        cdf=df[df["cell_class"]==class_name]
        pct=100*np.mean(cdf[responsive_col].astype(bool)) if len(cdf) else np.nan
        ax.bar(pos,pct,width=0.62,color=color,alpha=0.75,edgecolor="black",linewidth=0.5)
        ax.text(pos,pct+2,f"{pct:.1f}%\nn={int(np.sum(cdf[responsive_col].astype(bool)))}/{len(cdf)}",ha="center",va="bottom",fontsize=8)
    ax.set_xticks([0,1],["GREEN","YELLOW"]); ax.set_ylabel("Responsive cells (%)"); ax.set_ylim(bottom=0)
    ax.set_title(f"{label}: FDR-responsive fraction"); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    fig.suptitle(title,fontsize=13); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def analyze_session(
    ...
    # Also acquire raw fluorescence eventlocked data for the two requested
    # alternative dF/F baselines.  The original dff=True output is preserved.
    ...
    eventlocked_raw = eventlocked_raw_all[cell_mask, :, :]
    valid_events = np.asarray(valid_events, dtype=bool)
    ...
    stim_type = np.asarray(bdata["current_stim_type"], dtype=float)
    current_am = np.asarray(bdata["current_AM_rate"], dtype=float)
    fade_direction = np.asarray(bdata["fade_direction"], dtype=float)
    current_intensity = np.asarray(bdata["current_intensity"], dtype=float)
    ...
    if eventlocked_raw.shape[1] < n_behavior_trials:
        raise ValueError("Raw eventlocked data contain fewer events than behavior.")
    eventlocked_raw_aligned = eventlocked_raw[:, :n_behavior_trials, :]
    ...
    # -------------------------------------------------------------------------
    # All-sounds eventlocked QC: three explicitly requested dF/F baselines.
    # -------------------------------------------------------------------------
    _save_eventlocked_average_figure(
        eventlocked_aligned, valid_events_aligned, tvec,
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_all_sounds_eventlocked_avg_CURRENT_DFF.png",
        f"Current jaratoolbox dff=True baseline\n{title_suffix}",
    )
    ...
    _save_eventlocked_average_figure(
        session_median_dff, valid_events_aligned, tvec,
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_all_sounds_eventlocked_avg_SESSION_MEDIAN_DFF.png",
        f"Whole-session median F0 dF/F\n{title_suffix}",
    )
    local_prestim_dff = _local_prestim_median_dff(
        eventlocked_raw_aligned, event_onset_aligned, float(data2p.srate), valid_events_aligned, tvec,
        local_window_s=60.0, prestim_window_s=(-0.300, 0.0),
    )
    _save_eventlocked_average_figure(
        local_prestim_dff, valid_events_aligned, tvec,
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_all_sounds_eventlocked_avg_60s_PRESTIM_MEDIAN_DFF.png",
    ...
    # ---------------------------------------------------------------------
    # v12 alternate-dF/F sensitivity outputs.  These use the exact same
    # trial response window/baseline subtraction and condition masks as the
    # primary analysis; only the F0 definition used to make dF/F differs.
    # Responsiveness/FDR is intentionally NOT recomputed for these versions.
    # ---------------------------------------------------------------------
    _, trial_response_session_median_dff = _trial_response_from_dff(
        session_median_dff, baseline_mask_t, response_mask_t
    )
    _, trial_response_60s_prestim_median_dff = _trial_response_from_dff(
        local_prestim_dff, baseline_mask_t, response_mask_t
    )
    alt_valid_trial_mask = np.asarray(valid_events_aligned, dtype=bool)
    mean_sound_response_session_median_dff = (
        np.nanmean(trial_response_session_median_dff[:, alt_valid_trial_mask], axis=1)
        if np.any(alt_valid_trial_mask) else np.full(n_analysis_cells, np.nan)
    )
    mean_sound_response_60s_prestim_median_dff = (
        np.nanmean(trial_response_60s_prestim_median_dff[:, alt_valid_trial_mask], axis=1)
        if np.any(alt_valid_trial_mask) else np.full(n_analysis_cells, np.nan)
    )
    condition_responses_session_median_dff = _condition_response_matrix_from_trials(
        trial_response_session_median_dff, condition_masks
    )
    condition_responses_60s_prestim_median_dff = _condition_response_matrix_from_trials(
        trial_response_60s_prestim_median_dff, condition_masks
    )
    alt_session_median = _derived_condition_response_metrics(
        condition_responses_session_median_dff, possible_am_rates
    )
    alt_60s_prestim_median = _derived_condition_response_metrics(
        condition_responses_60s_prestim_median_dff, possible_am_rates
    )
    ...
    # -------------------------------------------------------------------------
    # Condition-specific responsiveness. Each condition is its own BH-FDR family
    # across classified cells: FI, FO, and every individual AM rate separately.
    # -------------------------------------------------------------------------
    condition_p_values = np.full_like(condition_responses, np.nan, dtype=float)
    condition_q_values = np.full_like(condition_responses, np.nan, dtype=float)
    condition_responsive = np.zeros(condition_responses.shape, dtype=bool)
    session_rng = np.random.default_rng(seed)
    for condition_index, condition_mask in enumerate(condition_masks):
        response_matrix = trial_response[:, condition_mask]
        p_cond = signflip_pvalues_grouped(response_matrix, classified_mask, n_permutations, session_rng)
        q_cond = bh_fdr(p_cond, classified_mask)
        condition_p_values[:, condition_index] = p_cond
        condition_q_values[:, condition_index] = q_cond
        condition_responsive[:, condition_index] = classified_mask & np.isfinite(q_cond) & (q_cond < FDR_ALPHA)

    fade_in_response = condition_responses[:, 0]
    fade_out_response = condition_responses[:, 1]
    fade_in_responsive_cells = condition_responsive[:, 0]
    fade_out_responsive_cells = condition_responsive[:, 1]
    fi_or_fo_responsive_cells = fade_in_responsive_cells | fade_out_responsive_cells
    any_am_rate_responsive_cells = (
        np.any(condition_responsive[:, 2:], axis=1)
        if condition_responsive.shape[1] > 2 else np.zeros(n_analysis_cells, dtype=bool)
    )
    fi_or_any_am_responsive_cells = fade_in_responsive_cells | any_am_rate_responsive_cells
    sound_responsive_cells = np.any(condition_responsive, axis=1)

    # Preserve scalar compatibility columns as the minimum condition-specific p/q.
    p_values = np.full(n_analysis_cells, np.nan)
    q_values = np.full(n_analysis_cells, np.nan)
    for i in range(n_analysis_cells):
        fp = condition_p_values[i, np.isfinite(condition_p_values[i])]
        fq = condition_q_values[i, np.isfinite(condition_q_values[i])]
        if len(fp): p_values[i] = float(np.min(fp))
        if len(fq): q_values[i] = float(np.min(fq))

    # Excited/suppressed are exclusive generic labels based on the sign of the
    # largest-absolute statistically significant condition response.
    sound_excited_cells = np.zeros(n_analysis_cells, dtype=bool)
    sound_suppressed_cells = np.zeros(n_analysis_cells, dtype=bool)
    for i in np.flatnonzero(sound_responsive_cells):
        sig = condition_responsive[i] & np.isfinite(condition_responses[i])
        if not np.any(sig):
            continue
        candidates = np.flatnonzero(sig)
        best_sig = candidates[int(np.argmax(np.abs(condition_responses[i, candidates])))]
        if condition_responses[i, best_sig] > 0:
            sound_excited_cells[i] = True
        elif condition_responses[i, best_sig] < 0:
            sound_suppressed_cells[i] = True
    ...
    fade_selectivity = bounded_fi_fo_selectivity(fade_in_response, fade_out_response)
    ...
    am_rate_gini = gini_coefficient_rows(am_condition_responses)
    am_rate_fano_factor = fano_factor_rows(am_condition_responses)
    am_rate_fwhm_hz = fwhm_bandwidth_rows(am_condition_responses, possible_am_rates)
    # Kept as a legacy descriptive column only; FI/FO rasters no longer sort by it.
    ...
    # ---------------------------------------------------------------------
    # v13 compact per-trial response archive.
    #
    # This is intentionally saved after the ordinary full-data analysis has
    # been completed so the archive contains both the trial-level ingredients
    # needed for later Monte-Carlo trial subsampling and the original v13
    # condition-level results for a 100% reproduction check.  Arrays are kept
    # as numeric / fixed-width unicode types so downstream loading does not
    # require allow_pickle=True.
    # ---------------------------------------------------------------------
    trial_condition_index = np.full(n_behavior_trials, -1, dtype=np.int16)
    for condition_index, condition_mask in enumerate(condition_masks):
        mask = np.asarray(condition_mask, dtype=bool)
        trial_condition_index[mask] = int(condition_index)

    trial_archive_path = (
        session_output_dir
        / f"{subject}_{date}_{session}_p{plane}_trial_response_archive_v13.npz"
    )
    np.savez_compressed(
        trial_archive_path,
        archive_schema_version=np.asarray(1, dtype=np.int16),
        source_script_version=np.asarray("v13"),
        ...
        trial_index=np.arange(n_behavior_trials, dtype=np.int32),
        valid_trial_mask=np.asarray(valid_events_aligned, dtype=bool),
        trial_condition_index=trial_condition_index,
        condition_names=np.asarray(condition_names, dtype=str),
        condition_trial_counts=np.asarray(n_trials_each_condition, dtype=np.int32),
        possible_am_rates_hz=np.asarray(possible_am_rates, dtype=float),
        stim_type=np.asarray(stim_type),
        current_am_rate_hz=np.asarray(current_am, dtype=float),
        fade_direction=np.asarray(fade_direction),
        current_intensity=np.asarray(current_intensity, dtype=float),
        ...
        analysis_index=np.arange(n_analysis_cells, dtype=np.int32),
        suite2p_roi=np.asarray(cell_roi_indices, dtype=np.int32),
        cell_class=np.asarray(cell_class, dtype=str),
        classified_mask=np.asarray(classified_mask, dtype=bool),
        green_only_mask=np.asarray(green_only_mask, dtype=bool),
        yellow_cell_mask=np.asarray(yellow_cell_mask, dtype=bool),
        cellpose_red_iou=np.asarray(cellpose_red_iou, dtype=float),
        # One response value per cell x behavioral trial.  Invalid trials are
        # retained in-place and identified by valid_trial_mask / condition=-1.
        ...
        # Original full-data condition outputs for downstream validation.
        primary_condition_responses=np.asarray(condition_responses, dtype=float),
        primary_condition_p_values=np.asarray(condition_p_values, dtype=float),
        primary_condition_q_values=np.asarray(condition_q_values, dtype=float),
        primary_condition_responsive_fdr=np.asarray(condition_responsive, dtype=bool),
        primary_sound_responsive_fdr=np.asarray(sound_responsive_cells, dtype=bool),
        primary_sound_excited_fdr=np.asarray(sound_excited_cells, dtype=bool),
        primary_sound_suppressed_fdr=np.asarray(sound_suppressed_cells, dtype=bool),
        primary_fi_or_fo_responsive_fdr=np.asarray(fi_or_fo_responsive_cells, dtype=bool),
        primary_any_am_rate_responsive_fdr=np.asarray(any_am_rate_responsive_cells, dtype=bool),
        primary_fi_or_any_am_responsive_fdr=np.asarray(fi_or_any_am_responsive_cells, dtype=bool),
        primary_fi_fo_selectivity=np.asarray(fade_selectivity, dtype=float),
        primary_fi_best_am_selectivity=np.asarray(fadein_bestam_selectivity, dtype=float),
        primary_am_lifetime_sparseness=np.asarray(am_rate_lifetime_sparseness, dtype=float),
    ...
    # Fail immediately if the archive cannot be loaded without pickle or if a
    # core dimension was written incorrectly.
    with np.load(trial_archive_path, allow_pickle=False) as archive_check:
        expected_shape = (n_analysis_cells, n_behavior_trials)
        for key in (
            "trial_response_primary",
            "trial_response_session_median_dff",
            "trial_response_60s_prestim_median_dff",
        ):
            if tuple(archive_check[key].shape) != expected_shape:
                raise RuntimeError(
                    f"v13 trial archive {key} has shape {archive_check[key].shape}, "
                    f"expected {expected_shape}."
                )
        if len(archive_check["trial_condition_index"]) != n_behavior_trials:
            raise RuntimeError("v13 trial archive condition index length mismatch.")
        if len(archive_check["suite2p_roi"]) != n_analysis_cells:
            raise RuntimeError("v13 trial archive ROI index length mismatch.")
    print(
        f"Saved v13 trial-response archive: {trial_archive_path} "
    ...
    for i in range(n_analysis_cells):
        row: dict[str, Any] = {
            ...
            "fade_in_response_p": float(condition_p_values[i, 0]),
            "fade_in_response_q": float(condition_q_values[i, 0]),
            "fade_in_responsive_fdr": bool(fade_in_responsive_cells[i]),
            "fade_out_response_p": float(condition_p_values[i, 1]),
            "fade_out_response_q": float(condition_q_values[i, 1]),
            "fade_out_responsive_fdr": bool(fade_out_responsive_cells[i]),
            "fi_or_fo_responsive_fdr": bool(fi_or_fo_responsive_cells[i]),
            "any_am_rate_responsive_fdr": bool(any_am_rate_responsive_cells[i]),
            "fi_or_any_am_responsive_fdr": bool(fi_or_any_am_responsive_cells[i]),
            ...
            "am_rate_gini": float(am_rate_gini[i]),
            "am_rate_fano_factor": float(am_rate_fano_factor[i]),
            "am_rate_fwhm_hz": float(am_rate_fwhm_hz[i]),

            # v12 alternate dF/F sensitivity-analysis columns.
            # Responsiveness/FDR columns above remain defined only from the
            # primary jaratoolbox dff=True analysis.
            "mean_sound_response_session_median_dff": float(mean_sound_response_session_median_dff[i]),
            "fade_in_response_session_median_dff": float(alt_session_median["fade_in_response"][i]),
            "fade_out_response_session_median_dff": float(alt_session_median["fade_out_response"][i]),
            "mean_am_response_across_rates_session_median_dff": float(alt_session_median["mean_am_response_across_rates"][i]),
            "best_am_response_session_median_dff": float(alt_session_median["best_am_response"][i]),
            "best_am_rate_session_median_dff": float(alt_session_median["best_am_rate"][i]),
            "fade_in_vs_fade_out_selectivity_session_median_dff": float(alt_session_median["fade_in_vs_fade_out_selectivity"][i]),
            "fade_in_vs_best_am_selectivity_session_median_dff": float(alt_session_median["fade_in_vs_best_am_selectivity"][i]),
            "am_rate_lifetime_sparseness_session_median_dff": float(alt_session_median["am_rate_lifetime_sparseness"][i]),
            "am_rate_gini_session_median_dff": float(alt_session_median["am_rate_gini"][i]),
            "am_rate_fano_factor_session_median_dff": float(alt_session_median["am_rate_fano_factor"][i]),
            "am_rate_fwhm_hz_session_median_dff": float(alt_session_median["am_rate_fwhm_hz"][i]),

            "mean_sound_response_60s_prestim_median_dff": float(mean_sound_response_60s_prestim_median_dff[i]),
            "fade_in_response_60s_prestim_median_dff": float(alt_60s_prestim_median["fade_in_response"][i]),
            "fade_out_response_60s_prestim_median_dff": float(alt_60s_prestim_median["fade_out_response"][i]),
            "mean_am_response_across_rates_60s_prestim_median_dff": float(alt_60s_prestim_median["mean_am_response_across_rates"][i]),
            "best_am_response_60s_prestim_median_dff": float(alt_60s_prestim_median["best_am_response"][i]),
            "best_am_rate_60s_prestim_median_dff": float(alt_60s_prestim_median["best_am_rate"][i]),
            "fade_in_vs_fade_out_selectivity_60s_prestim_median_dff": float(alt_60s_prestim_median["fade_in_vs_fade_out_selectivity"][i]),
            "fade_in_vs_best_am_selectivity_60s_prestim_median_dff": float(alt_60s_prestim_median["fade_in_vs_best_am_selectivity"][i]),
            "am_rate_lifetime_sparseness_60s_prestim_median_dff": float(alt_60s_prestim_median["am_rate_lifetime_sparseness"][i]),
            "am_rate_gini_60s_prestim_median_dff": float(alt_60s_prestim_median["am_rate_gini"][i]),
            "am_rate_fano_factor_60s_prestim_median_dff": float(alt_60s_prestim_median["am_rate_fano_factor"][i]),
            "am_rate_fwhm_hz_60s_prestim_median_dff": float(alt_60s_prestim_median["am_rate_fwhm_hz"][i]),
        ...
        for rate_index, rate in enumerate(possible_am_rates):
            ...
            row[f"am_response_{rate:g}Hz_p"] = float(condition_p_values[i, rate_index + 2])
            row[f"am_response_{rate:g}Hz_q"] = float(condition_q_values[i, rate_index + 2])
            row[f"am_response_{rate:g}Hz_responsive_fdr"] = bool(condition_responsive[i, rate_index + 2])
            row[f"am_response_{rate:g}Hz_session_median_dff"] = float(
                alt_session_median["am_condition_responses"][i, rate_index]
            )
            row[f"am_response_{rate:g}Hz_60s_prestim_median_dff"] = float(
                alt_60s_prestim_median["am_condition_responses"][i, rate_index]
            )
        cell_rows.append(row)
    cell_df = pd.DataFrame(cell_rows)
    cell_df.to_csv(session_output_dir / "cell_results.csv", index=False)

    # Standalone per-session GREEN-vs-YELLOW selectivity figures.
    fi_fo_df = fdr_sound_responsive_subset(cell_df, "fi_fo")
    am_resp_df = fdr_sound_responsive_subset(cell_df, "am")
    standalone_cell_selectivity_figure(
        cell_df=fi_fo_df,
        metric="fade_in_vs_fade_out_selectivity",
        ylabel="FI/FO selectivity index",
        title=f"{title_suffix}\nFade-in vs fade-out selectivity",
        equation_text=r"$SI_{FI/FO}=(R_{FI}-R_{FO})/(|R_{FI}|+|R_{FO}|)$",
        output_path=session_output_dir / f"{subject}_{date}_{session}_p{plane}_fade_in_vs_fade_out_selectivity_green_yellow.png",
        reference=0.0,
    )
    am_metric_specs = [
        ("am_rate_lifetime_sparseness", "AM-rate lifetime sparseness", "LIFETIME_SPARSENESS"),
        ("am_rate_gini", "AM-rate Gini coefficient", "GINI"),
        ("am_rate_fano_factor", "AM-rate Fano factor", "FANO"),
        ("am_rate_fwhm_hz", "AM-rate FWHM bandwidth (Hz)", "FWHM"),
    ]
    for metric, ylabel, suffix in am_metric_specs:
        standalone_cell_selectivity_figure(
            cell_df=am_resp_df,
            metric=metric,
            ylabel=ylabel,
            title=f"{title_suffix}\nAM-rate selectivity: {ylabel}",
            equation_text="AM-responsive means significant at >=1 individual AM rate (BH-FDR within rate across cells).",
            output_path=session_output_dir / f"{subject}_{date}_{session}_p{plane}_am_rate_selectivity_green_yellow_{suffix}.png",
            reference=None,
        )
    _session_paired_fi_fo_response_figure(
        cell_df,
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_fade_in_vs_fade_out_paired_cell_responses.png",
        title_suffix,
    )
    _session_stimulus_responsiveness_figure(
        cell_df, "fade_in_responsive_fdr", "fade_in_response",
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_fade_in_responsive_cells_green_yellow.png",
        title_suffix, "Fade-in",
    )
    _session_stimulus_responsiveness_figure(
        cell_df, "fade_out_responsive_fdr", "fade_out_response",
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_fade_out_responsive_cells_green_yellow.png",
        title_suffix, "Fade-out",
    )
    # AM response displayed here is the largest-absolute AM-rate response per cell.
    cell_df["am_peak_abs_signed_response"] = np.nan
    if len(possible_am_rates):
        am_arr = am_condition_responses.copy()
        for ii in range(len(cell_df)):
            if np.any(np.isfinite(am_arr[ii])):
                jj = int(np.nanargmax(np.abs(am_arr[ii])))
                cell_df.loc[ii, "am_peak_abs_signed_response"] = am_arr[ii, jj]
    _session_stimulus_responsiveness_figure(
        cell_df, "any_am_rate_responsive_fdr", "am_peak_abs_signed_response",
        session_output_dir / f"{subject}_{date}_{session}_p{plane}_am_responsive_any_rate_cells_green_yellow.png",
        title_suffix, "AM (responsive at >=1 rate)",
    )
    # Rewrite CSV after adding the AM display helper column.
    cell_df.to_csv(session_output_dir / "cell_results.csv", index=False)
    ...
    for class_name, class_mask in (("green", green_only_mask), ("yellow", yellow_cell_mask)):
        ...
        n_fi_resp = int(np.sum(fade_in_responsive_cells & class_mask))
        n_fo_resp = int(np.sum(fade_out_responsive_cells & class_mask))
        n_fi_fo_resp = int(np.sum(fi_or_fo_responsive_cells & class_mask))
        n_am_resp = int(np.sum(any_am_rate_responsive_cells & class_mask))
        class_fade = fade_in_response[class_mask]
        class_fade_out = fade_out_response[class_mask]
        ...
        class_am_gini = am_rate_gini[class_mask]
        class_am_fano = am_rate_fano_factor[class_mask]
        class_am_fwhm = am_rate_fwhm_hz[class_mask]
        class_summary: dict[str, Any] = {
            ...
            "n_fade_in_responsive_fdr": n_fi_resp,
            "pct_fade_in_responsive_fdr": 100 * safe_divide(n_fi_resp, n_class),
            "n_fade_out_responsive_fdr": n_fo_resp,
            "pct_fade_out_responsive_fdr": 100 * safe_divide(n_fo_resp, n_class),
            "n_fi_or_fo_responsive_fdr": n_fi_fo_resp,
            "pct_fi_or_fo_responsive_fdr": 100 * safe_divide(n_fi_fo_resp, n_class),
            "n_any_am_rate_responsive_fdr": n_am_resp,
            "pct_any_am_rate_responsive_fdr": 100 * safe_divide(n_am_resp, n_class),
            "mean_fade_in_response": float(np.nanmean(class_fade)) if n_class else np.nan,
            "mean_fade_out_response": float(np.nanmean(class_fade_out)) if n_class else np.nan,
            ...
            "mean_am_rate_gini": float(np.nanmean(class_am_gini)) if np.any(np.isfinite(class_am_gini)) else np.nan,
            "std_am_rate_gini": float(np.nanstd(class_am_gini, ddof=1)) if np.sum(np.isfinite(class_am_gini)) > 1 else np.nan,
            "mean_am_rate_fano_factor": float(np.nanmean(class_am_fano)) if np.any(np.isfinite(class_am_fano)) else np.nan,
            "std_am_rate_fano_factor": float(np.nanstd(class_am_fano, ddof=1)) if np.sum(np.isfinite(class_am_fano)) > 1 else np.nan,
            "mean_am_rate_fwhm_hz": float(np.nanmean(class_am_fwhm)) if np.any(np.isfinite(class_am_fwhm)) else np.nan,
            "std_am_rate_fwhm_hz": float(np.nanstd(class_am_fwhm, ddof=1)) if np.sum(np.isfinite(class_am_fwhm)) > 1 else np.nan,
    ...
    session_summary_json = {
        ...
        "trial_response_archive": {
            "path": str(trial_archive_path),
            "schema_version": 1,
            "compressed_npz": True,
            "n_cells": int(n_analysis_cells),
            "n_behavior_trials": int(n_behavior_trials),
            "n_valid_trials": int(np.sum(valid_events_aligned)),
            "contents": [
                "primary per-cell x trial responses",
                "whole-session-median-dF/F per-cell x trial responses",
                "60-s prestimulus-median-dF/F per-cell x trial responses",
                "trial condition identities and valid-trial mask",
                "cell class and Suite2p ROI identities",
                "original condition-specific p/q/FDR results",
            ],
        },
        "alternate_dff_sensitivity_outputs": {
            "session_median_dff": True,
            "60s_prestim_median_dff": True,
            "trial_response_definition": "mean 0-1 s minus mean -1-0 s",
...
def sound_responsiveness_session_mean_only(
    ...
    """Session fractions with overall mean±SEM and paired GREEN/YELLOW statistics."""
    metrics=[("sound_responsive_fdr","Responsive"),("sound_excited_fdr","Excited"),("sound_suppressed_fdr","Suppressed")]
    classified=cell_df[cell_df["cell_class"].isin(["green","yellow"])].copy()
    fig,ax=plt.subplots(figsize=(11.5,7.0))
    x=np.arange(len(metrics),dtype=float); offsets={"green":-0.12,"yellow":0.12}; colors={"green":GREEN_COLOR,"yellow":YELLOW_COLOR}
    session_keys=sorted(classified["session_key"].dropna().astype(str).unique())
    stats_raw=[]; stats_records=[]; all_bounds=[]
    overall_cache={}
    for class_name in ["green","yellow"]:
        session_means_by_metric=[[] for _ in metrics]
        for session_key in session_keys:
            sdf=classified[(classified["session_key"].astype(str)==session_key)&(classified["cell_class"]==class_name)]
            for mi,(metric,_) in enumerate(metrics):
                vals=100.0*sdf[metric].astype(float).to_numpy() if len(sdf) else np.array([])
                m,sd,_=_mean_sd(vals); session_means_by_metric[mi].append(m)
                if np.isfinite(m):
                    ax.errorbar(x[mi]+offsets[class_name],m,yerr=sd if np.isfinite(sd) else None,marker="o",markersize=3.6,lw=0,
                                elinewidth=0.8,capsize=2.5,color=colors[class_name],alpha=0.25,zorder=2)
                    all_bounds.extend([m] + ([m-sd,m+sd] if np.isfinite(sd) else []))
        oms=[]; oses=[]
        for vals in session_means_by_metric:
            m,_,se,n=mean_sd_sem(vals); oms.append(m); oses.append(se)
        overall_cache[class_name]=(np.asarray(oms,float),np.asarray(oses,float),session_means_by_metric)
        ax.errorbar(x+offsets[class_name],oms,yerr=oses,marker="o",markersize=8,lw=2.8,elinewidth=2.4,capsize=5.5,
                    color=colors[class_name],label=f"{class_name.capitalize()} overall",zorder=5)
    for mi,(metric,label) in enumerate(metrics):
        g=np.asarray(overall_cache["green"][2][mi],float); y=np.asarray(overall_cache["yellow"][2][mi],float)
        st=safe_paired_wilcoxon(g,y); stats_records.append(st); stats_raw.append(np.nan if st["p"] is None else st["p"])
    holm=holm_correct(stats_raw)
    ymax=max(finite_values(all_bounds).max() if len(finite_values(all_bounds)) else 100,100)
    for mi,((_,label),st,ph) in enumerate(zip(metrics,stats_records,holm)):
        ptxt="n/a" if st["p"] is None else f"{st['p']:.3g}"; htxt="n/a" if not np.isfinite(ph) else f"{ph:.3g}"
        ax.text(x[mi],ymax*1.04,f"paired Wilcoxon\nn={st.get('n_pairs',0)}, p={ptxt}, Holm p={htxt}",ha="center",va="bottom",fontsize=8)
    ax.set_xticks(x,[name for _,name in metrics]); ax.set_ylabel("Cells (%)")
    ax.set_title("Thin/light: individual session mean ± SD; thick/dark: overall mean ± SEM")
    ax.legend(frameon=False); ax.grid(axis="y",alpha=0.22); ax.spines[["top","right"]].set_visible(False)
    ax.set_ylim(0,max(105,ymax*1.22)); fig.suptitle(title,fontsize=14); fig.subplots_adjust(top=0.80,bottom=0.12)
    fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def _plot_top_selectivity_raster(ax, trace_records, class_name: str, title: str):
    ...
    ax.set_ylabel("Cells ranked by |bounded FI/FO SI|")
...
def combined_condition_response_plot(
    cell_df: pd.DataFrame,
    am_rates: list[float],
    output_path: Path,
    title: str,
    enhanced_fdr_style: bool = False,
) -> None:
    ...
    groups=[("All plotted cells",cell_df[cell_df["cell_class"].isin(["green","yellow"])],"black"),
            ("Green",cell_df[cell_df["cell_class"]=="green"],GREEN_COLOR),
            ("Yellow",cell_df[cell_df["cell_class"]=="yellow"],YELLOW_COLOR)]
    rng=np.random.default_rng(31415); shared_display=[]; shared_important=[0.0]
    for ax,(group_name,df,color) in zip(axes[:3],groups):
        local_display=[]; local_important=[0.0]
        if enhanced_fdr_style and len(df):
            matrix=np.column_stack([df[c].to_numpy(float) if c in df else np.full(len(df),np.nan) for c in condition_cols])
            strength=np.full(len(df),np.nan); validrow=np.any(np.isfinite(matrix),axis=1)
            strength[validrow]=np.nanmax(np.abs(matrix[validrow]),axis=1)
            inds=np.flatnonzero(np.isfinite(strength)); inds=inds[np.argsort(strength[inds])[::-1]][:10]
            for ii in inds: ax.plot(x,matrix[ii],color=color,alpha=0.22,lw=0.9,zorder=1)
        for ci,col in enumerate(condition_cols):
            if col not in df: continue
            vals=finite_values(df[col])
            if not len(vals): continue
            local_display.append(vals); shared_display.append(vals)
            jitter=rng.uniform(-0.08,0.08,len(vals)); ax.scatter(np.full(len(vals),x[ci])+jitter,vals,s=10,alpha=0.22,color=color,edgecolors="none",zorder=2)
            m,se=mean_sem(vals); ax.errorbar(x[ci],m,yerr=se,fmt="o",color="black",capsize=3,zorder=5)
            local_important.append(m); shared_important.append(m)
            if np.isfinite(se): local_important.extend([m-se,m+se]); shared_important.extend([m-se,m+se])
        ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks(x,labels,rotation=45,ha="right")
        extra="; lines = top 10 by max |response|" if enhanced_fdr_style else ""
        ax.set_title(f"{group_name}\nn={len(df)} cells{extra}"); ax.set_ylabel("Mean baseline-subtracted dF/F")
        ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
        if not enhanced_fdr_style: apply_robust_y_limits(ax,local_display,important_values=local_important,reference_values=[0.0])
    if enhanced_fdr_style:
        apply_robust_y_limits(axes[0],shared_display,important_values=shared_important,reference_values=[0.0]); shared_ylim=axes[0].get_ylim()
        for ax in axes[:3]: ax.set_ylim(shared_ylim)
    ax=axes[3]; all_means=[]; all_sems=[]; comparison_display=[]; comparison_important=[0.0]
    for class_name,color,offset in [("green",GREEN_COLOR,-0.05),("yellow",YELLOW_COLOR,0.05)]:
        df=cell_df[cell_df["cell_class"]==class_name]; means=[]; sems=[]
        for col in condition_cols:
            vals=df[col] if col in df else np.array([]); finite=finite_values(vals)
            if len(finite): comparison_display.append(finite)
            m,se=mean_sem(vals); means.append(m); sems.append(se); all_means.append(m); all_sems.append(se)
            if np.isfinite(m): comparison_important.append(m)
            if np.isfinite(se): comparison_important.extend([m-se,m+se])
        ax.errorbar(x+offset,means,yerr=sems,marker="o",capsize=4,lw=2,color=color,label=f"{class_name.capitalize()} (n={len(df)})")
    ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks(x,labels,rotation=45,ha="right"); ax.set_title("Green vs yellow mean ± SEM")
    ax.set_ylabel("Mean baseline-subtracted dF/F"); ax.legend(frameon=False); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    if enhanced_fdr_style:
        apply_mean_error_y_limits(ax,all_means,all_sems,reference_values=[0.0],pad_fraction=0.15)
    else:
        apply_robust_y_limits(ax,comparison_display,important_values=comparison_important,reference_values=[0.0])
    fig.suptitle(title,fontsize=14); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def combined_selectivity_raster_preference_and_selectivity(
    ...
    fig,axes=plt.subplots(2,3,figsize=(19,11),constrained_layout=True); im=None
    for row_idx,(class_name,color_name) in enumerate([("green","GREEN"),("yellow","YELLOW")]):
        base=cell_df[(cell_df["cell_class"]==class_name)&(cell_df["raster_responsive"]==True)].copy()
        panel_specs=[("preferred","sorted by preferred condition","sound_responsive_fdr"),
                     ("fi","sorted by bounded FI/FO selectivity","fi_or_fo_responsive_fdr"),
                     ("am","sorted by AM lifetime sparseness","any_am_rate_responsive_fdr")]
        for ax,(kind,sort_title,resp_col) in zip(axes[row_idx],panel_specs):
            df=base.copy()
            if responsive_only and resp_col in df: df=df[df[resp_col].astype(bool)].copy()
            if len(df)==0: ax.axis("off"); continue
            matrix=np.column_stack([df[c].to_numpy(float) if c in df else np.full(len(df),np.nan) for c in condition_cols])
            norm=np.nanmax(np.abs(matrix),axis=1,keepdims=True); norm[(~np.isfinite(norm))|(norm<=0)]=np.nan; normalized=matrix/norm
            if kind=="preferred":
                pref=np.nanargmax(np.where(np.isfinite(matrix),matrix,-np.inf),axis=1); order=np.argsort(pref)
            elif kind=="fi":
                si=df["fade_in_vs_fade_out_selectivity"].to_numpy(float); order=np.argsort(np.where(np.isfinite(si),si,-np.inf))[::-1]
            else:
                sp=df["am_rate_lifetime_sparseness"].to_numpy(float); order=np.argsort(np.where(np.isfinite(sp),sp,-np.inf))[::-1]
            im=ax.imshow(normalized[order],aspect="auto",interpolation="nearest",cmap="coolwarm",vmin=-1,vmax=1)
            ax.set_xticks(np.arange(len(labels)),labels,rotation=45,ha="right"); ax.set_ylabel("Cells")
    ...
    fig.suptitle(title+"\nFI/FO SI = (FI-FO)/(|FI|+|FO|)",fontsize=14)
    fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)


def combined_selectivity_raster(cell_df: pd.DataFrame, am_rates: list[float], output_path: Path, title: str, responsive_only: bool=False) -> None:
    condition_cols=["fade_in_response","fade_out_response"]+[f"am_response_{r:g}Hz" for r in am_rates]
    labels=["FI","FO"]+[f"AM {r:g}" for r in am_rates]
    fig,axes=plt.subplots(1,2,figsize=(13,9),constrained_layout=True); im=None
    for ax,class_name,color_name in zip(axes,["green","yellow"],["GREEN","YELLOW"]):
        df=cell_df[(cell_df["cell_class"]==class_name)&(cell_df["raster_responsive"]==True)].copy()
        if responsive_only and "fi_or_fo_responsive_fdr" in df: df=df[df["fi_or_fo_responsive_fdr"].astype(bool)]
        if len(df)==0:
            ax.text(0.5,0.5,f"No eligible {class_name} cells",transform=ax.transAxes,ha="center",va="center"); ax.set_title(color_name); continue
        matrix=np.column_stack([df[c].to_numpy(float) if c in df else np.full(len(df),np.nan) for c in condition_cols])
        norm=np.nanmax(np.abs(matrix),axis=1,keepdims=True); norm[(~np.isfinite(norm))|(norm<=0)]=np.nan; normalized=matrix/norm
        si=df["fade_in_vs_fade_out_selectivity"].to_numpy(float); order=np.argsort(np.where(np.isfinite(si),si,-np.inf))[::-1]
        im=ax.imshow(normalized[order],aspect="auto",interpolation="nearest",cmap="coolwarm",vmin=-1,vmax=1)
        ax.set_title(f"{color_name} cells\nn={len(df)} | sorted high-to-low bounded FI/FO SI")
        ax.set_xticks(np.arange(len(labels)),labels,rotation=45,ha="right"); ax.set_ylabel("Cells"); ax.set_xlabel("Sound condition")
    if im is not None:
        cbar=fig.colorbar(im,ax=axes,shrink=0.85); cbar.set_label("Response / max absolute response per cell")
    fig.suptitle(title+"\nFI/FO SI = (FI-FO)/(|FI|+|FO|)",fontsize=14)
    fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def session_level_figure(
    ...
    """Primary paired session comparisons with overall class mean±SEM overlays."""
    ...
    for ax,(metric,ylabel,stat_key,ref) in zip(axes.flat,panels):
        if metric not in pivot or "green" not in pivot[metric] or "yellow" not in pivot[metric]: ax.axis("off"); continue
        g=pivot[metric]["green"]; y=pivot[metric]["yellow"]; common=g.index.intersection(y.index)
        gv=g.loc[common].to_numpy(float); yv=y.loc[common].to_numpy(float); valid=np.isfinite(gv)&np.isfinite(yv); gv=gv[valid]; yv=yv[valid]
        for gg,yy in zip(gv,yv): ax.plot([0,1],[gg,yy],color="0.65",alpha=0.45,lw=0.9)
        ax.scatter(np.zeros(len(gv)),gv,color=GREEN_COLOR,s=32,zorder=3); ax.scatter(np.ones(len(yv)),yv,color=YELLOW_COLOR,edgecolor="black",linewidth=0.3,s=38,zorder=3)
        mg,seg=mean_sem(gv); my,sey=mean_sem(yv)
        ax.errorbar(0,mg,yerr=seg if np.isfinite(seg) else None,fmt="D",markersize=10,color=GREEN_COLOR,markeredgecolor="black",elinewidth=2.8,capsize=7,zorder=6)
        ax.errorbar(1,my,yerr=sey if np.isfinite(sey) else None,fmt="D",markersize=10,color=YELLOW_COLOR,markeredgecolor="black",elinewidth=2.8,capsize=7,zorder=6)
        annotate_mean_sem_point(ax,0,mg,seg,len(gv),color=GREEN_COLOR,digits=3,fontsize=7.5)
        annotate_mean_sem_point(ax,1,my,sey,len(yv),color=YELLOW_COLOR,digits=3,fontsize=7.5)
        ...
        ax.set_title(f"paired sessions n={len(gv)} | Wilcoxon p={ptxt}\ndiamonds = overall mean ± SEM")
        ax.set_xticks([0,1],["Green","Yellow"]); ax.set_ylabel(ylabel); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
        imp=[mg,my]
        if np.isfinite(seg): imp.extend([mg-seg,mg+seg])
        if np.isfinite(sey): imp.extend([my-sey,my+sey])
        apply_robust_y_limits(ax,[gv,yv],important_values=imp,reference_values=None if ref is None else [ref])
    axes.flat[-1].axis("off"); fig.suptitle(title+"\nSession-level paired summaries",fontsize=14)
    fig.subplots_adjust(top=0.88,hspace=0.40,wspace=0.30); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)


def _fmt_p(p):
    return "n/a" if p is None or not np.isfinite(p) else f"{p:.3g}"


def _combined_condition_suppression_plot(cell_df, output_path, title, response_col, condition_label):
    df=cell_df[cell_df["cell_class"].isin(["green","yellow"])].copy()
    fig,axes=plt.subplots(1,2,figsize=(13.5,6),constrained_layout=True); rng=np.random.default_rng(42)
    ax=axes[0]; vals_by={}
    for pos,class_name,color in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
        vals=finite_values(df.loc[df["cell_class"]==class_name,response_col]); vals_by[class_name]=vals
        jitter=rng.uniform(-0.12,0.12,len(vals)); ax.scatter(np.full(len(vals),pos)+jitter,vals,color=color,alpha=0.35,s=18,edgecolor="black" if class_name=="yellow" else "none",linewidth=0.2,zorder=2)
        if len(vals):
            ax.boxplot([vals],positions=[pos],widths=0.42,showfliers=False,patch_artist=False,
                       medianprops=dict(linewidth=2.2),boxprops=dict(linewidth=1.2),whiskerprops=dict(linewidth=1.0),capprops=dict(linewidth=1.0))
    mw=safe_mannwhitney(vals_by.get("green",[]),vals_by.get("yellow",[])); yw=safe_wilcoxon(vals_by.get("yellow",[]),alternative="less")
    ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks([0,1],["Green","Yellow"]); ax.set_ylabel(f"Mean {condition_label.lower()} response (baseline-subtracted dF/F)")
    ax.set_title("Cells (exploratory)\n"+f"Mann-Whitney GREEN vs YELLOW p={_fmt_p(mw['p'])}; YELLOW<0 Wilcoxon p={_fmt_p(yw['p'])}")
    ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False); apply_robust_y_limits(ax,[vals_by.get("green",[]),vals_by.get("yellow",[])],reference_values=[0.0])
    ax=axes[1]; session_means={"green":[],"yellow":[]}; keys=sorted(df["session_key"].dropna().astype(str).unique())
    for key in keys:
        pair={}
        for pos,class_name,color in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
            vals=finite_values(df.loc[(df["session_key"].astype(str)==key)&(df["cell_class"]==class_name),response_col]); m=float(np.mean(vals)) if len(vals) else np.nan
            pair[class_name]=m; session_means[class_name].append(m)
            if np.isfinite(m): ax.scatter(pos,m,color=color,s=35,edgecolor="black" if class_name=="yellow" else "none",linewidth=0.25,zorder=3)
        if np.isfinite(pair.get("green",np.nan)) and np.isfinite(pair.get("yellow",np.nan)): ax.plot([0,1],[pair["green"],pair["yellow"]],color="0.65",alpha=0.55,lw=0.9)
    g=np.asarray(session_means["green"],float); y=np.asarray(session_means["yellow"],float); valid=np.isfinite(g)&np.isfinite(y); g=g[valid]; y=y[valid]
    mg,seg=mean_sem(g); my,sey=mean_sem(y)
    ax.errorbar(0,mg,yerr=seg if np.isfinite(seg) else None,fmt="D",markersize=11,color=GREEN_COLOR,markeredgecolor="black",elinewidth=3,capsize=8,zorder=6)
    ax.errorbar(1,my,yerr=sey if np.isfinite(sey) else None,fmt="D",markersize=11,color=YELLOW_COLOR,markeredgecolor="black",elinewidth=3,capsize=8,zorder=6)
    pairtest=safe_paired_wilcoxon(g,y); yzero=safe_wilcoxon(y,alternative="less")
    ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks([0,1],["Green","Yellow"]); ax.set_ylabel(f"Session mean {condition_label.lower()} response")
    ax.set_title("Sessions; diamonds = overall mean ± SEM\n"+f"paired Wilcoxon p={_fmt_p(pairtest['p'])}; YELLOW<0 p={_fmt_p(yzero['p'])}")
    ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    important=[mg,my]
    if np.isfinite(seg): important.extend([mg-seg,mg+seg])
    if np.isfinite(sey): important.extend([my-sey,my+sey])
    apply_robust_y_limits(ax,[g,y],important_values=important,reference_values=[0.0])
    fig.suptitle(title,fontsize=14); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)


def combined_fade_in_suppression_plot(cell_df, class_summary_df, output_path, title, pooled_stats=None, session_stats=None):
    _combined_condition_suppression_plot(cell_df,output_path,title,"fade_in_response","Fade-in")


def combined_fade_out_suppression_plot(cell_df, output_path, title):
    _combined_condition_suppression_plot(cell_df,output_path,title,"fade_out_response","Fade-out")


def _condition_suppression_mean_only(cell_df, output_path, title, response_col, condition_label):
    df=cell_df[cell_df["cell_class"].isin(["green","yellow"])].copy(); fig,axes=plt.subplots(1,2,figsize=(15,6.5))
    keys=sorted(df["session_key"].dropna().astype(str).unique()); x=np.arange(len(keys),dtype=float); session_by={"green":[],"yellow":[]}
    for class_name,color,offset in [("green",GREEN_COLOR,-0.06),("yellow",YELLOW_COLOR,0.06)]:
        means=[]; sds=[]
        for key in keys:
            vals=finite_values(df.loc[(df["session_key"].astype(str)==key)&(df["cell_class"]==class_name),response_col]); m,sd,_=_mean_sd(vals); means.append(m); sds.append(sd)
        session_by[class_name]=means; axes[0].errorbar(x+offset,means,yerr=sds,marker="o",markersize=4,lw=0.9,elinewidth=0.8,capsize=2.5,color=color,alpha=0.30,label=f"{class_name.capitalize()} sessions")
        om,_,ose,n=mean_sd_sem(means); axes[0].axhline(om,color=color,lw=3.0)
        if np.isfinite(ose): axes[0].fill_between([-0.5,len(keys)-0.5],[om-ose]*2,[om+ose]*2,color=color,alpha=0.10)
    axes[0].axhline(0,color="black",ls=":",lw=1); axes[0].set_xticks(x,[k.replace("_","\n") for k in keys],rotation=45,ha="right")
    axes[0].set_ylabel(f"{condition_label} response"); axes[0].set_title("By session\npoints/error = session mean ± SD; horizontal = overall mean ± SEM")
    axes[0].legend(frameon=False,fontsize=8); axes[0].grid(axis="y",alpha=0.2); axes[0].spines[["top","right"]].set_visible(False)
    g=np.asarray(session_by["green"],float); y=np.asarray(session_by["yellow"],float); valid=np.isfinite(g)&np.isfinite(y); g=g[valid]; y=y[valid]
    for gg,yy in zip(g,y): axes[1].plot([0,1],[gg,yy],color="0.65",alpha=0.45,lw=0.9)
    mg,seg=mean_sem(g); my,sey=mean_sem(y)
    axes[1].errorbar(0,mg,yerr=seg if np.isfinite(seg) else None,fmt="D",markersize=11,color=GREEN_COLOR,markeredgecolor="black",elinewidth=3,capsize=8)
    axes[1].errorbar(1,my,yerr=sey if np.isfinite(sey) else None,fmt="D",markersize=11,color=YELLOW_COLOR,markeredgecolor="black",elinewidth=3,capsize=8)
    st=safe_paired_wilcoxon(g,y); axes[1].axhline(0,color="black",ls=":",lw=1); axes[1].set_xticks([0,1],["Green","Yellow"]); axes[1].set_ylabel(f"Session mean {condition_label.lower()} response")
    axes[1].set_title(f"Overall mean ± SEM\npaired Wilcoxon p={_fmt_p(st['p'])}"); axes[1].grid(axis="y",alpha=0.2); axes[1].spines[["top","right"]].set_visible(False)
    fig.suptitle(title,fontsize=14); fig.subplots_adjust(top=0.84,bottom=0.20,wspace=0.32); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def fade_out_suppression_mean_only(
    cell_df: pd.DataFrame,
    output_path: Path,
    title: str,
) -> None:
    """Fade-out analogue of the unchanged fade-in MEAN_SEM_ONLY layout."""
    df=cell_df[cell_df["cell_class"].isin(["green","yellow"])].copy()
    fig,axes=plt.subplots(1,2,figsize=(15,6.5))
    ax=axes[0]
    session_keys=sorted(df["session_key"].dropna().astype(str).unique()); x=np.arange(len(session_keys),dtype=float)
    ...
    for class_name,color,offset in [("green",GREEN_COLOR,-0.06),("yellow",YELLOW_COLOR,0.06)]:
        ...
        for key in session_keys:
            sdf=df[(df["session_key"].astype(str)==key)&(df["cell_class"]==class_name)]
            m,sd,_=_mean_sd(sdf["fade_out_response"].to_numpy(float)); means.append(m); sds.append(sd)
        session_means[class_name]=means
        ax.errorbar(x+offset,means,yerr=sds,marker="o",markersize=4,lw=0.9,elinewidth=0.8,capsize=2.5,color=color,alpha=0.30,label=f"{class_name.capitalize()} sessions")
        om,_,ose,n=mean_sd_sem(means); ax.axhline(om,color=color,lw=3.0,alpha=1.0)
        if np.isfinite(ose): ax.fill_between([-0.5,len(session_keys)-0.5],[om-ose]*2,[om+ose]*2,color=color,alpha=0.10)
    ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks(x,[k.replace("_","\n") for k in session_keys],rotation=45,ha="right")
    ax.set_ylabel("Fade-out response"); ax.set_title("By session\npoints/error = session mean ± SD; thick horizontal = overall mean ± SEM")
    ax.legend(frameon=False,fontsize=8); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    g=np.asarray(session_means["green"],float); y=np.asarray(session_means["yellow"],float); st=safe_paired_wilcoxon(g,y)
    # Reuse the standard class session-summary renderer, with the actual paired p-value.
    _plot_session_class_metric_mean_sd(axes[1],df,"fade_out_response","Fade-out response","Collapsed GREEN vs YELLOW",st.get("p"),reference=0.0)
    y0=safe_wilcoxon(y,alternative="less"); axes[1].text(0.5,0.02,f"Yellow session means < 0: p={_fmt_p(y0.get('p'))}",transform=axes[1].transAxes,ha="center",va="bottom",fontsize=8)
    fig.suptitle(title,fontsize=14); fig.subplots_adjust(top=0.84,bottom=0.20,wspace=0.32); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def standalone_cell_selectivity_figure(
    cell_df: pd.DataFrame,
    metric: str,
    ylabel: str,
    title: str,
    ...
    """Per-session GREEN/YELLOW cell metric with Mann-Whitney test annotation."""
    df = cell_df[cell_df["cell_class"].isin(["green", "yellow"])].copy()
    fig, ax = plt.subplots(figsize=(7.2, 7.0))
    rng = np.random.default_rng(73211)
    display = []
    values_by_class = {}
    for pos, class_name, color in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
        vals = finite_values(df.loc[df["cell_class"]==class_name, metric]) if metric in df else np.array([])
        values_by_class[class_name] = vals
        if len(vals):
            jitter=rng.uniform(-0.13,0.13,len(vals))
            ax.scatter(np.full(len(vals),pos)+jitter,vals,s=30,color=color,alpha=0.34,
                       edgecolors="black" if class_name=="yellow" else "none",linewidths=0.25,zorder=2)
            m,_,se,n=mean_sd_sem(vals)
            ax.errorbar(pos,m,yerr=se if np.isfinite(se) else None,fmt="o",markersize=11,color=color,
                        markeredgecolor="black",markeredgewidth=0.7,elinewidth=2.6,capsize=7,zorder=5)
            annotate_mean_sem_point(ax,pos,m,se,n,color=color,digits=3,fontsize=8.5,y_offset_points=10)
            display.extend(vals.tolist())
    test=safe_mannwhitney(values_by_class.get("green",[]),values_by_class.get("yellow",[]))
    ptxt="n/a" if test["p"] is None else f"{test['p']:.3g}"
    if reference is not None: ax.axhline(reference,color="black",ls=":",lw=1.2,alpha=0.8)
    ax.set_xticks([0,1],["GREEN","YELLOW"]); ax.set_xlim(-0.48,1.48); ax.set_ylabel(ylabel,fontsize=12)
    ax.set_title(title+f"\nIndividual cells; large point = mean ± SEM\nMann-Whitney U (two-sided) p={ptxt}",fontsize=12.5,pad=12)
    ax.grid(axis="y",alpha=0.22); ax.spines[["top","right"]].set_visible(False)
    if display: apply_robust_y_limits(ax,[display],reference_values=None if reference is None else [reference],annotate=True)
    fig.text(0.5,0.025,equation_text,ha="center",va="bottom",fontsize=9.5)
    fig.subplots_adjust(bottom=0.17,top=0.84,left=0.15,right=0.97)
    fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)


def standalone_session_selectivity_figure(
    cell_df: pd.DataFrame,
    metric: str,
    ylabel: str,
    title: str,
    ...
    """Readable combined GREEN/YELLOW selectivity plot with sessions as points."""
    df = cell_df[cell_df["cell_class"].isin(["green", "yellow"])].copy()
    session_keys = sorted(df["session_key"].dropna().astype(str).unique())
    fig, ax = plt.subplots(figsize=(7.8, 7.2))
    offsets = np.linspace(-0.075, 0.075, max(len(session_keys), 1))
    display = []
    session_means = {"green": [], "yellow": []}

    for s_idx, session_key in enumerate(session_keys):
        offset = offsets[min(s_idx, len(offsets)-1)]
        pair = {}
        for pos, class_name, color in [(0, "green", GREEN_COLOR), (1, "yellow", YELLOW_COLOR)]:
            vals = finite_values(
                df.loc[
                    (df["session_key"].astype(str) == session_key)
                    & (df["cell_class"] == class_name),
                    metric,
                ]
            )
            mean_value, sd_value, _, n = mean_sd_sem(vals)
            pair[class_name] = mean_value
            session_means[class_name].append(mean_value)
            if np.isfinite(mean_value):
                xx = pos + offset
                ax.errorbar(
                    xx,
                    mean_value,
                    yerr=sd_value if np.isfinite(sd_value) else None,
                    fmt="o",
                    markersize=5.5,
                    color=color,
                    alpha=0.42,
                    markeredgecolor="black" if class_name == "yellow" else "none",
                    markeredgewidth=0.25,
                    elinewidth=0.9,
                    capsize=2.5,
                    zorder=2,
                )
                display.append(mean_value)
                if np.isfinite(sd_value):
                    display.extend([mean_value-sd_value, mean_value+sd_value])
        if np.isfinite(pair.get("green", np.nan)) and np.isfinite(pair.get("yellow", np.nan)):
            ax.plot(
                [0 + offset, 1 + offset],
                [pair["green"], pair["yellow"]],
                color="0.55",
                alpha=0.28,
                lw=0.9,
                zorder=1,
    ...
    overall = {}
    for pos, class_name, color in [(0, "green", GREEN_COLOR), (1, "yellow", YELLOW_COLOR)]:
        mean_value, _, sem_value, n = mean_sd_sem(session_means[class_name])
        overall[class_name] = (mean_value, sem_value, n)
        if np.isfinite(mean_value):
            ax.errorbar(
                pos,
                mean_value,
    ...
    if np.isfinite(overall["green"][0]) and np.isfinite(overall["yellow"][0]):
        ax.plot(
            [0, 1],
            [overall["green"][0], overall["yellow"][0]],
            color="black",
            lw=2.2,
            alpha=0.72,
            zorder=4,
    ...
    if reference is not None:
        ax.axhline(reference, color="black", ls=":", lw=1.2, alpha=0.8)
    p_value = session_stats.get(stat_key, {}).get("p")
    ...
    ax.set_title(
        title
        + f"\nLight points = session mean ± SD; dark = overall mean ± SEM"
        + f"\nPaired-session Wilcoxon p={p_text}",
        fontsize=12.5,
        pad=12,
    )
    ax.grid(axis="y", alpha=0.22)
    ...
    if display:
        important = []
        for mean_value, sem_value, _ in overall.values():
            if np.isfinite(mean_value):
                important.append(mean_value)
                if np.isfinite(sem_value):
                    important.extend([mean_value-sem_value, mean_value+sem_value])
        apply_robust_y_limits(
            ax,
            [display],
            important_values=important,
            reference_values=None if reference is None else [reference],
            annotate=True,
        )
    fig.text(0.5, 0.025, equation_text, ha="center", va="bottom", fontsize=10.5)
    fig.subplots_adjust(bottom=0.16, top=0.84, left=0.15, right=0.97)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)


def _analysis_df(cell_df, panel_kind: str, responsive_only: bool):
    if not responsive_only:
        return cell_df[cell_df["cell_class"].isin(["green","yellow"])].copy()
    if panel_kind == "fi_fo": return fdr_sound_responsive_subset(cell_df,"fi_fo")
    if panel_kind == "am": return fdr_sound_responsive_subset(cell_df,"am")
    if panel_kind == "fi_am": return fdr_sound_responsive_subset(cell_df,"fi_am")
    return fdr_sound_responsive_subset(cell_df,"any")


def _paired_cell_panel(ax, df, class_name, col_a, col_b, labels, color, title):
    sdf=df[df["cell_class"]==class_name]; a=sdf[col_a].to_numpy(float); b=sdf[col_b].to_numpy(float); valid=np.isfinite(a)&np.isfinite(b); a=a[valid]; b=b[valid]
    rng=np.random.default_rng(1203); jit=rng.uniform(-0.06,0.06,len(a))
    for aa,bb,j in zip(a,b,jit): ax.plot([0+j,1+j],[aa,bb],color=color,alpha=0.12,lw=0.6,zorder=1)
    ax.scatter(jit,a,color=color,alpha=0.25,s=15,edgecolors="none",zorder=2); ax.scatter(1+jit,b,color=color,alpha=0.25,s=15,edgecolors="none",zorder=2)
    ma,sea=mean_sem(a); mb,seb=mean_sem(b); ax.errorbar([0,1],[ma,mb],yerr=[sea,seb],fmt="o-",color="black",lw=2,capsize=5,zorder=5)
    st=safe_paired_wilcoxon(a,b); ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks([0,1],labels); ax.set_ylabel("Baseline-subtracted dF/F")
    ax.set_title(title+f"\\npaired-cell Wilcoxon p={_fmt_p(st['p'])}, n={st.get('n_pairs',0)}"); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    apply_robust_y_limits(ax,[a,b],important_values=[ma,mb,ma-sea if np.isfinite(sea) else ma,ma+sea if np.isfinite(sea) else ma,mb-seb if np.isfinite(seb) else mb,mb+seb if np.isfinite(seb) else mb],reference_values=[0.0])


def _session_paired_panel(ax, df, class_name, col_a, col_b, labels, color, title):
    aa=[]; bb=[]
    for _,sdf in df[df["cell_class"]==class_name].groupby("session_key",sort=True):
        va=finite_values(sdf[col_a]); vb=finite_values(sdf[col_b]); ma=float(np.mean(va)) if len(va) else np.nan; mb=float(np.mean(vb)) if len(vb) else np.nan
        if np.isfinite(ma) and np.isfinite(mb): aa.append(ma); bb.append(mb); ax.plot([0,1],[ma,mb],color=color,alpha=0.28,lw=0.9)
    aa=np.asarray(aa,float); bb=np.asarray(bb,float); ma,sea=mean_sem(aa); mb,seb=mean_sem(bb)
    ax.errorbar([0,1],[ma,mb],yerr=[sea,seb],fmt="o-",color=color,lw=3,elinewidth=2.5,capsize=6,markersize=8,zorder=5)
    st=safe_paired_wilcoxon(aa,bb); ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks([0,1],labels); ax.set_ylabel("Session mean baseline-subtracted dF/F")
    ax.set_title(title+f"\\npaired-session Wilcoxon p={_fmt_p(st['p'])}, n={st.get('n_pairs',0)}"); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    apply_robust_y_limits(ax,[aa,bb],important_values=[ma,mb,ma-sea if np.isfinite(sea) else ma,ma+sea if np.isfinite(sea) else ma,mb-seb if np.isfinite(seb) else mb,mb+seb if np.isfinite(seb) else mb],reference_values=[0.0])


def _session_am_profile_panel(ax, df, class_name, am_rates, color, title):
    cols=[f"am_response_{r:g}Hz" for r in am_rates]; rows=[]
    for _,sdf in df[df["cell_class"]==class_name].groupby("session_key",sort=True):
        means=np.array([np.nanmean(sdf[c].to_numpy(float)) if c in sdf and np.any(np.isfinite(sdf[c])) else np.nan for c in cols]); rows.append(means); ax.plot(np.arange(len(cols)),means,color=color,alpha=0.23,lw=0.8)
    mat=np.vstack(rows) if rows else np.empty((0,len(cols))); om=[]; ose=[]
    for j in range(len(cols)):
        m,se=mean_sem(mat[:,j] if len(mat) else []); om.append(m); ose.append(se)
    ax.errorbar(np.arange(len(cols)),om,yerr=ose,marker="o",color=color,lw=3,elinewidth=2.2,capsize=4,zorder=5)
    fr=safe_friedman(mat); ax.axhline(0,color="black",ls=":",lw=1); ax.set_xticks(np.arange(len(cols)),[f"{r:g}" for r in am_rates],rotation=45,ha="right")
    ax.set_xlabel("AM rate (Hz)"); ax.set_ylabel("Session mean baseline-subtracted dF/F"); ax.set_title(title+f"\\nFriedman across AM rates p={_fmt_p(fr['p'])}, n={fr['n_sessions']}")
    ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False); apply_robust_y_limits(ax,[mat],important_values=np.r_[om,np.asarray(om)-np.asarray(ose),np.asarray(om)+np.asarray(ose)],reference_values=[0.0])


def _class_metric_session_panel(ax, df, metric, ylabel, title, reference=None):
    session={"green":{},"yellow":{}}
    for class_name in ["green","yellow"]:
        for key,sdf in df[df["cell_class"]==class_name].groupby("session_key",sort=True):
            vals=finite_values(sdf[metric]) if metric in sdf else np.array([]); session[class_name][str(key)]=float(np.mean(vals)) if len(vals) else np.nan
    common=sorted(set(session["green"])&set(session["yellow"])); g=np.array([session["green"][k] for k in common],float); y=np.array([session["yellow"][k] for k in common],float); valid=np.isfinite(g)&np.isfinite(y); g=g[valid]; y=y[valid]
    for gg,yy in zip(g,y): ax.plot([0,1],[gg,yy],color="0.65",alpha=0.35,lw=0.8)
    ax.scatter(np.zeros(len(g)),g,color=GREEN_COLOR,s=28); ax.scatter(np.ones(len(y)),y,color=YELLOW_COLOR,s=32,edgecolor="black",linewidth=0.3)
    mg,seg=mean_sem(g); my,sey=mean_sem(y); ax.errorbar(0,mg,yerr=seg if np.isfinite(seg) else None,fmt="D",markersize=9,color=GREEN_COLOR,markeredgecolor="black",elinewidth=2.6,capsize=6,zorder=5); ax.errorbar(1,my,yerr=sey if np.isfinite(sey) else None,fmt="D",markersize=9,color=YELLOW_COLOR,markeredgecolor="black",elinewidth=2.6,capsize=6,zorder=5)
    st=safe_paired_wilcoxon(g,y); 
    if reference is not None: ax.axhline(reference,color="black",ls=":",lw=1)
    ax.set_xticks([0,1],["GREEN","YELLOW"]); ax.set_ylabel(ylabel); ax.set_title(title+f"\\npaired-session Wilcoxon p={_fmt_p(st['p'])}, n={st.get('n_pairs',0)}"); ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    apply_robust_y_limits(ax,[g,y],important_values=[mg,my,mg-seg if np.isfinite(seg) else mg,mg+seg if np.isfinite(seg) else mg,my-sey if np.isfinite(sey) else my,my+sey if np.isfinite(sey) else my],reference_values=None if reference is None else [reference])


def save_split_mean_sem_summary_figures(cell_df, am_rates, output_dir, prefix, title, responsive_only=False):
    fi_fo=_analysis_df(cell_df,"fi_fo",responsive_only); fi_am=_analysis_df(cell_df,"fi_am",responsive_only); am=_analysis_df(cell_df,"am",responsive_only)
    for class_name,color in [("green",GREEN_COLOR),("yellow",YELLOW_COLOR)]:
        fig,axes=plt.subplots(1,3,figsize=(18,5.8),constrained_layout=True)
        _session_paired_panel(axes[0],fi_fo,class_name,"fade_in_response","fade_out_response",["Fade-in","Fade-out"],color,f"{class_name.upper()} FI vs FO")
        _session_paired_panel(axes[1],fi_am,class_name,"fade_in_response","best_am_response",["Fade-in","Best AM"],color,f"{class_name.upper()} FI vs best AM")
        _session_am_profile_panel(axes[2],am,class_name,am_rates,color,f"{class_name.upper()} AM-rate profile")
        fig.suptitle(title+f" | {class_name.upper()} | session means",fontsize=14); fig.savefig(output_dir/f"{prefix}_{class_name.upper()}_MEAN_SEM_ONLY.png",dpi=300,bbox_inches="tight"); plt.close(fig)
    fig,axes=plt.subplots(2,3,figsize=(17,10.5),constrained_layout=True)
    specs=[("fade_in_vs_fade_out_selectivity","FI/FO selectivity","FI/FO bounded selectivity",0.0,fi_fo),
           ("fade_in_vs_best_am_selectivity","FI/best-AM selectivity","FI vs best-AM selectivity",0.0,fi_am),
           ("am_rate_lifetime_sparseness","AM lifetime sparseness","AM lifetime sparseness",None,am),
           ("am_rate_gini","AM Gini coefficient","AM Gini coefficient",None,am),
           ("am_rate_fano_factor","AM Fano factor","AM Fano factor",None,am),
           ("am_rate_fwhm_hz","AM FWHM (Hz)","AM FWHM bandwidth",None,am)]
    for ax,(metric,ylabel,ptitle,ref,df) in zip(axes.flat,specs): _class_metric_session_panel(ax,df,metric,ylabel,ptitle,ref)
    fig.suptitle(title+" | GREEN vs YELLOW | session means ± SEM",fontsize=14); fig.savefig(output_dir/f"{prefix}_GREEN_YELLOW_MEAN_SEM_ONLY.png",dpi=300,bbox_inches="tight"); plt.close(fig)


def save_split_individual_cell_figures(cell_df, trace_records, am_rates, output_dir, prefix, title, responsive_only=False):
    fi_fo=_analysis_df(cell_df,"fi_fo",responsive_only); fi_am=_analysis_df(cell_df,"fi_am",responsive_only); am=_analysis_df(cell_df,"am",responsive_only)
    for class_name,color in [("green",GREEN_COLOR),("yellow",YELLOW_COLOR)]:
        fig,axes=plt.subplots(1,2,figsize=(12,5.6),constrained_layout=True)
        _paired_cell_panel(axes[0],fi_fo,class_name,"fade_in_response","fade_out_response",["Fade-in","Fade-out"],color,f"{class_name.upper()} FI vs FO")
        _paired_cell_panel(axes[1],fi_am,class_name,"fade_in_response","best_am_response",["Fade-in","Best AM"],color,f"{class_name.upper()} FI vs best AM")
        fig.suptitle(title+f" | {class_name.upper()} | individual cells",fontsize=14); fig.savefig(output_dir/f"{prefix}_{class_name.upper()}_INDIVIDUAL_CELLS.png",dpi=300,bbox_inches="tight"); plt.close(fig)
    fig,axes=plt.subplots(2,3,figsize=(17,10.5),constrained_layout=True)
    specs=[("fade_in_vs_fade_out_selectivity","FI/FO selectivity",0.0,fi_fo),
           ("fade_in_vs_best_am_selectivity","FI/best-AM selectivity",0.0,fi_am),
           ("am_rate_lifetime_sparseness","AM lifetime sparseness",None,am),
           ("am_rate_gini","AM Gini coefficient",None,am),
           ("am_rate_fano_factor","AM Fano factor",None,am),
           ("am_rate_fwhm_hz","AM FWHM (Hz)",None,am)]
    for ax,(metric,ylabel,ref,df) in zip(axes.flat,specs):
        vals_g=finite_values(df.loc[df["cell_class"]=="green",metric]); vals_y=finite_values(df.loc[df["cell_class"]=="yellow",metric]); st=safe_mannwhitney(vals_g,vals_y)
        _plot_cell_class_metric_points(ax,df,metric,ylabel,ylabel,st.get("p"),reference=ref); ax.set_title(ylabel+f"\\nMann-Whitney U p={_fmt_p(st.get('p'))}")
    fig.suptitle(title+" | GREEN vs YELLOW | individual cells",fontsize=14); fig.savefig(output_dir/f"{prefix}_GREEN_YELLOW_INDIVIDUAL_CELLS.png",dpi=300,bbox_inches="tight"); plt.close(fig)
    # Rasters in their own figure; FI/FO selectivity determines ranking.
    trs=trace_records
    if responsive_only: trs=[r for r in trs if bool(r.get("fi_or_fo_responsive_fdr",False))]
    fig,axes=plt.subplots(1,2,figsize=(14,8),constrained_layout=True); img=_plot_top_selectivity_raster(axes[0],trs,"green","GREEN FI/FO raster"); imy=_plot_top_selectivity_raster(axes[1],trs,"yellow","YELLOW FI/FO raster"); im=img if img is not None else imy
    if im is not None: fig.colorbar(im,ax=axes,shrink=0.85,label="Response / max absolute response per cell")
    fig.suptitle(title+" | FI/FO rasters ranked by |bounded FI/FO SI|",fontsize=14); fig.savefig(output_dir/f"{prefix}_FI_FO_RASTERS.png",dpi=300,bbox_inches="tight"); plt.close(fig)


def standalone_am_session_metric_figure(cell_df, metric, ylabel, output_path, title):
    df=fdr_sound_responsive_subset(cell_df,"am"); keys=sorted(df["session_key"].dropna().astype(str).unique())
    # Chronological session order determines viridis fill; class determines outline.
    cmap=plt.cm.viridis; norm=matplotlib.colors.Normalize(vmin=0,vmax=max(len(keys)-1,1)); fig,ax=plt.subplots(figsize=(8.6,7.4)); counts=[]; session_values={"green":{},"yellow":{}}
    for si,key in enumerate(keys):
        fill=cmap(norm(si)); pair={}
        for pos,class_name,edge in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
            sdf=df[(df["session_key"].astype(str)==key)&(df["cell_class"]==class_name)]; vals=finite_values(sdf[metric]) if metric in sdf else np.array([]); n=len(vals); m=float(np.mean(vals)) if n else np.nan; pair[class_name]=m; session_values[class_name][key]=m; counts.append(n)
            if np.isfinite(m):
                # marker radius in points is proportional to contributing-cell count.
                pass
        if np.isfinite(pair.get("green",np.nan)) and np.isfinite(pair.get("yellow",np.nan)): ax.plot([0,1],[pair["green"],pair["yellow"]],color="0.70",alpha=0.45,lw=0.9,zorder=1)
    maxn=max(counts) if counts else 1
    for si,key in enumerate(keys):
        fill=cmap(norm(si))
        for pos,class_name,edge in [(0,"green",GREEN_COLOR),(1,"yellow",YELLOW_COLOR)]:
            sdf=df[(df["session_key"].astype(str)==key)&(df["cell_class"]==class_name)]; vals=finite_values(sdf[metric]) if metric in sdf else np.array([]); n=len(vals); m=float(np.mean(vals)) if n else np.nan
            if np.isfinite(m):
                radius=3.0+8.0*(n/maxn); size=(2*radius)**2
                ax.scatter(pos,m,s=size,facecolor=fill,edgecolor=edge,linewidth=2.0,zorder=3)
    common=[k for k in keys if np.isfinite(session_values["green"].get(k,np.nan)) and np.isfinite(session_values["yellow"].get(k,np.nan))]
    g=np.array([session_values["green"][k] for k in common],float); y=np.array([session_values["yellow"][k] for k in common],float); mg,seg=mean_sem(g); my,sey=mean_sem(y)
    ax.errorbar(0,mg,yerr=seg if np.isfinite(seg) else None,fmt="D",markersize=12,mfc="white",mec=GREEN_COLOR,mew=2.5,ecolor=GREEN_COLOR,elinewidth=3,capsize=7,zorder=6)
    ax.errorbar(1,my,yerr=sey if np.isfinite(sey) else None,fmt="D",markersize=12,mfc="white",mec=YELLOW_COLOR,mew=2.5,ecolor=YELLOW_COLOR,elinewidth=3,capsize=7,zorder=6)
    st=safe_paired_wilcoxon(g,y); ax.set_xticks([0,1],["GREEN","YELLOW"]); ax.set_ylabel(ylabel); ax.set_title(title+f"\\npaired-session Wilcoxon p={_fmt_p(st['p'])}, n={st.get('n_pairs',0)}")
    ax.grid(axis="y",alpha=0.2); ax.spines[["top","right"]].set_visible(False)
    sm=plt.cm.ScalarMappable(norm=norm,cmap=cmap); cbar=fig.colorbar(sm,ax=ax,pad=0.02); cbar.set_label("Chronological session order")
    legend_ns=sorted(set([1,max(1,maxn//2),maxn])); handles=[]
    for n in legend_ns:
        radius=3.0+8.0*(n/maxn); handles.append(plt.scatter([],[],s=(2*radius)**2,facecolor="white",edgecolor="0.3",label=f"n={n} cells"))
    ax.legend(handles=handles,title="Marker size",frameon=False,loc="best"); fig.savefig(output_path,dpi=300,bbox_inches="tight"); plt.close(fig)
...
def compute_combined_statistics(
    ...
    selectivity_metrics=[
        ...
        ("am_rate_gini_green_vs_yellow","am_rate_gini"),
        ("am_rate_fano_factor_green_vs_yellow","am_rate_fano_factor"),
        ("am_rate_fwhm_hz_green_vs_yellow","am_rate_fwhm_hz"),
    ]
    pooled_tests={}
    raw_p=[]
    ...
    pooled={
        ...
        **pooled_tests,
        ...
        "fade_out_yellow_vs_green":safe_mannwhitney(yellow.get("fade_out_response", []), green.get("fade_out_response", [])),
    }
    try:
    ...
    session_level={
        ...
        "am_rate_gini_green_vs_yellow":paired_metric("mean_am_rate_gini"),
        "am_rate_fano_factor_green_vs_yellow":paired_metric("mean_am_rate_fano_factor"),
        "am_rate_fwhm_hz_green_vs_yellow":paired_metric("mean_am_rate_fwhm_hz"),
        "fade_in_green_vs_yellow":paired_metric("mean_fade_in_response"),
        "fade_out_green_vs_yellow":paired_metric("mean_fade_out_response"),
    }
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "analysis_parameters": {
            ...
            "seed": args.seed,
            "responsiveness_fdr_family": "BH-FDR separately across cells within FI, FO, and each individual AM rate",
            "fi_fo_selectivity": "(FI-FO)/(|FI|+|FO|)",
            "alternate_dff_sensitivity_outputs": {
                "session_median_dff": "F0 = whole-session median fluorescence per cell",
                "60s_prestim_median_dff": "F0 = median of -300 to 0 ms prestimulus samples from valid stimuli within +/-30 s of each event",
                "trial_response_definition": "mean dF/F 0-1 s minus mean dF/F -1-0 s",
                "responsiveness_recomputed": False,
                "responsiveness_note": "Primary jaratoolbox dff=True condition-specific FDR labels are reused for sensitivity analyses",
            },
            "trial_response_archive_v13": {
                "enabled": True,
                "schema_version": 1,
                "format": "compressed NumPy NPZ",
                "location": "session_outputs/<date>_<session>/<subject>_<date>_<session>_p<plane>_trial_response_archive_v13.npz",
                "purpose": "downstream Monte-Carlo trial-repetition subsampling without rerunning imaging preprocessing",
                "response_arrays": [
                    "trial_response_primary",
                    "trial_response_session_median_dff",
                    "trial_response_60s_prestim_median_dff",
                ...
                "trial_response_definition": "mean dF/F 0-1 s minus mean dF/F -1-0 s",
                "contains_original_fdr_results": True,
            },
        },
            ...
            manifest["successful_sessions"].append(
                {
                    ...
                    "trial_response_archive": str(result["trial_response_archive_path"]),
                }
            )
    ...
    trial_archive_index_rows = []
    for r in session_results:
        rmeta = r["meta"]
        trial_archive_index_rows.append({
            ...
            "n_behavior_trials": int(r["n_behavior_trials"]),
            "n_valid_trials": int(r["n_valid_trials"]),
            "trial_response_archive_path": str(r["trial_response_archive_path"]),
            "trial_response_archive_schema_version": 1,
        })
    trial_archive_index_df = pd.DataFrame(trial_archive_index_rows)
    trial_archive_index_df.to_csv(
    ...
    # Analysis-specific responsive subsets.
    responsive_cell_df = fdr_sound_responsive_subset(cell_df, "any")
    fi_fo_responsive_df = fdr_sound_responsive_subset(cell_df, "fi_fo")
    am_responsive_df = fdr_sound_responsive_subset(cell_df, "am")
    fade_in_responsive_df = fdr_sound_responsive_subset(cell_df, "fi")
    fade_out_responsive_df = fdr_sound_responsive_subset(cell_df, "fo")
    fade_in_class_summary_df = subset_session_class_summary(fade_in_responsive_df)
    fade_out_class_summary_df = subset_session_class_summary(fade_out_responsive_df)
    if len(fade_in_responsive_df) and len(fade_in_class_summary_df):
        fade_in_pooled_stats, fade_in_session_stats = compute_combined_statistics(fade_in_responsive_df, fade_in_class_summary_df)
    else:
        fade_in_pooled_stats, fade_in_session_stats = {}, {}
    ...
    if len(responsive_cell_df) and len(responsive_class_summary_df):
        responsive_pooled_stats, responsive_session_stats = compute_combined_statistics(responsive_cell_df, responsive_class_summary_df)
    else:
        responsive_pooled_stats, responsive_session_stats = {}, {}

    stats_payload = {
        "pooled_cell_exploratory": pooled_stats,
        "session_level_primary": session_stats,
        "fdr_any_sound_responsive": {"pooled_cell_exploratory": responsive_pooled_stats, "session_level": responsive_session_stats},
        "n_successful_sessions": len(session_results), "n_failed_sessions": len(failures),
        ...
        "n_fdr_any_sound_responsive": int(len(responsive_cell_df)),
        "n_fdr_fi_or_fo_responsive": int(len(fi_fo_responsive_df)),
        "n_fdr_any_am_rate_responsive": int(len(am_responsive_df)),
        "am_rates": all_am_rates,
    }
    save_json(combined_dir / "combined_statistics.json", stats_payload)

    title = f"{subject} | {area} | {', '.join(dates)} | {len(session_results)} successful sound_tuning sessions"

    # Responsiveness summaries.
    sound_responsiveness_figure(cell_df, combined_dir / "combined_sound_responsive_cells_green_yellow.png", title + " | pooled cells")
    sound_responsiveness_session_mean_only(cell_df, combined_dir / "combined_sound_responsiveness_session_mean_sem_only.png", title)
    _session_stimulus_responsiveness_figure(cell_df,"fade_in_responsive_fdr","fade_in_response",combined_dir/"combined_fade_in_responsive_cells_green_yellow.png",title,"Fade-in")
    _session_stimulus_responsiveness_figure(cell_df,"fade_out_responsive_fdr","fade_out_response",combined_dir/"combined_fade_out_responsive_cells_green_yellow.png",title,"Fade-out")
    _session_stimulus_responsiveness_figure(cell_df,"any_am_rate_responsive_fdr","am_peak_abs_signed_response",combined_dir/"combined_am_responsive_any_rate_cells_green_yellow.png",title,"AM (responsive at >=1 rate)")

    # Sound-condition response curves. The requested MEAN_SEM_ONLY variants are intentionally removed.
    combined_condition_response_plot(cell_df, all_am_rates, combined_dir / "combined_sound_condition_responses_green_yellow.png", title + " | all classified cells")
    combined_condition_response_plot(responsive_cell_df, all_am_rates, combined_dir / "combined_sound_condition_responses_green_yellow_FDR_responsive_only.png", title + " | any-condition FDR responsive", enhanced_fdr_style=True)

    # Selectivity rasters. FDR panels use the analysis-specific responsive mask appropriate to each sorting panel.
    combined_selectivity_raster_preference_and_selectivity(cell_df, all_am_rates, combined_dir / "combined_sound_selectivity_rasters_preference_and_selectivity.png", title, responsive_only=False)
    combined_selectivity_raster_preference_and_selectivity(cell_df, all_am_rates, combined_dir / "combined_sound_selectivity_rasters_preference_and_selectivity_FDR_responsive_only.png", title + " | analysis-specific FDR", responsive_only=True)
    combined_selectivity_raster(cell_df, all_am_rates, combined_dir / "combined_sound_selectivity_raster_FI_FO_selectivity.png", title, responsive_only=False)
    combined_selectivity_raster(cell_df, all_am_rates, combined_dir / "combined_sound_selectivity_raster_FI_FO_selectivity_FDR_responsive_only.png", title + " | FI-or-FO FDR responsive", responsive_only=True)

    # The old large combined sound-response summary figures are replaced by split, readable outputs.
    save_split_individual_cell_figures(cell_df, trace_records, all_am_rates, combined_dir,
        "combined_green_yellow_sound_response_summary", title + " | all classified cells", responsive_only=False)
    save_split_individual_cell_figures(cell_df, trace_records, all_am_rates, combined_dir,
        "combined_green_yellow_sound_response_summary_FDR_responsive_only", title + " | analysis-specific FDR", responsive_only=True)
    save_split_mean_sem_summary_figures(cell_df, all_am_rates, combined_dir,
        "combined_green_yellow_sound_response_summary", title + " | all classified cells", responsive_only=False)
    save_split_mean_sem_summary_figures(cell_df, all_am_rates, combined_dir,
        "combined_green_yellow_sound_response_summary_FDR_responsive_only", title + " | analysis-specific FDR", responsive_only=True)

    # Standalone FI/FO session comparison (all cells) with the new bounded index.
    standalone_session_selectivity_figure(
        cell_df=cell_df, metric="fade_in_vs_fade_out_selectivity", ylabel="FI/FO selectivity index",
        title=title + "\nFade-in vs fade-out selectivity",
        equation_text=r"$SI_{FI/FO}=(R_{FI}-R_{FO})/(|R_{FI}|+|R_{FO}|)$",
        output_path=combined_dir / "combined_fade_in_vs_fade_out_selectivity_green_yellow_SESSION_POINTS.png",
        session_stats=session_stats, stat_key="fade_selectivity_green_vs_yellow", reference=0.0,
    )

    # AM selectivity session-point figures: only cells responsive to >=1 individual AM rate.
    am_specs=[
        ("am_rate_lifetime_sparseness","AM-rate lifetime sparseness","combined_am_rate_selectivity_green_yellow_SESSION_POINTS.png"),
        ("am_rate_gini","AM-rate Gini coefficient","combined_am_rate_selectivity_green_yellow_SESSION_POINTS_GINI.png"),
        ("am_rate_fano_factor","AM-rate Fano factor","combined_am_rate_selectivity_green_yellow_SESSION_POINTS_FANO.png"),
        ("am_rate_fwhm_hz","AM-rate FWHM bandwidth (Hz)","combined_am_rate_selectivity_green_yellow_SESSION_POINTS_FWHM.png"),
    ]
    for metric,ylabel,filename in am_specs:
        standalone_am_session_metric_figure(cell_df,metric,ylabel,combined_dir/filename,title+"\nAM-responsive cells only")

    # Primary session-level comparisons with overall mean±SEM overlaid.
    session_level_figure(class_summary_df, combined_dir / "combined_session_level_green_yellow_comparisons.png", title, session_stats)
    session_level_mean_only_figure(cell_df, combined_dir / "combined_session_level_green_yellow_comparisons_MEAN_SEM_ONLY.png", title, session_stats)

    # Fade-in and fade-out response/suppression figures.
    combined_fade_in_suppression_plot(cell_df, class_summary_df, combined_dir / "combined_fade_in_suppression_green_yellow.png", title + " | all classified cells", pooled_stats, session_stats)
    combined_fade_in_suppression_plot(fade_in_responsive_df, subset_session_class_summary(fade_in_responsive_df), combined_dir / "combined_fade_in_suppression_green_yellow_FDR_responsive_only.png", title + " | fade-in FDR-responsive cells")
    fade_in_suppression_mean_only(cell_df, class_summary_df, combined_dir / "combined_fade_in_suppression_green_yellow_MEAN_SEM_ONLY.png", title + " | all classified cells", pooled_stats, session_stats)
    fade_in_suppression_mean_only(fade_in_responsive_df, fade_in_class_summary_df, combined_dir / "combined_fade_in_suppression_green_yellow_FDR_responsive_only_MEAN_SEM_ONLY.png", title + " | fade-in FDR-responsive cells", fade_in_pooled_stats, fade_in_session_stats)

    combined_fade_out_suppression_plot(cell_df, combined_dir / "combined_fade_out_suppression_green_yellow.png", title + " | all classified cells")
    combined_fade_out_suppression_plot(fade_out_responsive_df, combined_dir / "combined_fade_out_suppression_green_yellow_FDR_responsive_only.png", title + " | fade-out FDR-responsive cells")
    fade_out_suppression_mean_only(cell_df, combined_dir / "combined_fade_out_suppression_green_yellow_MEAN_SEM_ONLY.png", title + " | all classified cells")
    fade_out_suppression_mean_only(fade_out_responsive_df, combined_dir / "combined_fade_out_suppression_green_yellow_FDR_responsive_only_MEAN_SEM_ONLY.png", title + " | fade-out FDR-responsive cells")

    # Figure index for quick navigation.
    figure_index = [
        "FIGURE VARIANTS",
        "=" * 70,
        "FDR responsiveness is tested separately for FI, FO, and each AM rate; BH-FDR is applied across cells within each condition.",
        "MEAN_SEM_ONLY files show individual-session mean ± SD (light/thin) and overall mean ± SEM across sessions (dark/thick).",
        "FI/FO rasters use bounded SI=(FI-FO)/(|FI|+|FO|); FDR panels use condition-appropriate responsiveness.",
        "",
    ]
    ...
    manifest["combined_outputs"] = {
        ...
        "combined_trial_response_archive_index": str(
            combined_dir / "combined_trial_response_archive_index.csv"
        ),
        "trial_response_archives": [
            str(r["trial_response_archive_path"]) for r in session_results
        ],
    }
    save_json(output_dirs["root"] / "run_manifest.json", manifest)
