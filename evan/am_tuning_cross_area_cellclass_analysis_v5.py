...
V5 adds dedicated area x layer tests of sound-responsive fractions and best-frequency distributions.
...
# =============================================================================
# V5 EXPLICIT AREA x LAYER QUESTIONS
# =============================================================================
#
# These outputs directly answer two prespecified questions, separately for
# GREEN and YELLOW cells (there is never a GREEN-vs-YELLOW test here):
#
#   1) Does the fraction of sound-responsive cells differ across cortical
#      areas within each layer, and between L2/3 and L4/5 within each area?
#
#   2) For frequency-tuning sessions, how is best frequency distributed across
#      cells in each area x layer stratum, and do the session-normalized
#      best-frequency-bin fractions differ across areas/layers?
#
# Session-level percentages are the primary biological replicates. For best
# frequency, the primary inferential figures use FDR-responsive cells only,
# because the argmax frequency of a nonresponsive cell is not biologically
# interpretable as a best frequency. An all-cell distribution is retained as a
# descriptive sensitivity view.

V5_LAYER_ORDER = (("L23", "L2/3"), ("L45", "L4/5"))


def v5_fmt_p(p: float) -> str:
    return "n/a" if not np.isfinite(p) else ("<1e-4" if p < 1e-4 else f"{p:.4g}")


def v5_session_responsive_area_layer(cells: pd.DataFrame) -> pd.DataFrame:
    d = add_layer_columns(cells)
    d = d[d.layer_key.notna()].copy()
    rows = []
    for (sk, area, layer), sdf in d.groupby(["session_key", "canonicalArea", "layer_key"], dropna=False):
        vals = v4_bool_series(sdf["stim_responsive_fdr"]).to_numpy(float)
        rows.append({
            "session_key": str(sk), "canonicalArea": str(area), "layer_key": str(layer),
            "pct_sound_responsive": 100.0 * float(np.mean(vals)) if len(vals) else np.nan,
            "n_cells": int(len(vals)), "n_responsive": int(np.sum(vals)) if len(vals) else 0,
        })
    return pd.DataFrame(rows)


def v5_responsive_area_layer_stats(cells: pd.DataFrame, areas: list[str]) -> dict[str, Any]:
    tab = v5_session_responsive_area_layer(cells)
    out: dict[str, Any] = {"session_level_primary": {"six_group_omnibus": {}, "area_within_layer": {}, "layer_within_area": {}}}

    six_groups = {}
    for area in areas:
        for lk, _ in V5_LAYER_ORDER:
            six_groups[f"{area}-{lk}"] = tab.loc[(tab.canonicalArea == area) & (tab.layer_key == lk), "pct_sound_responsive"]
    out["session_level_primary"]["six_group_omnibus"] = safe_kruskal(six_groups)

    layer_omnibus_ps = []
    for lk, ll in V5_LAYER_ORDER:
        groups = {a: tab.loc[(tab.canonicalArea == a) & (tab.layer_key == lk), "pct_sound_responsive"] for a in areas}
        rec = {"layer_key": lk, "layer_label": ll, "omnibus": safe_kruskal(groups), "pairwise": pairwise_area_tests(groups, areas)}
        out["session_level_primary"]["area_within_layer"][lk] = rec
        layer_omnibus_ps.append(rec["omnibus"].get("p", np.nan))
    for (lk, _), ph in zip(V5_LAYER_ORDER, holm_adjust(layer_omnibus_ps)):
        out["session_level_primary"]["area_within_layer"][lk]["omnibus"]["p_holm_across_layers"] = ph

    layer_tests = []
    for area in areas:
        x = tab.loc[(tab.canonicalArea == area) & (tab.layer_key == "L23"), "pct_sound_responsive"]
        y = tab.loc[(tab.canonicalArea == area) & (tab.layer_key == "L45"), "pct_sound_responsive"]
        rec = safe_mannwhitney(x, y)
        rec.update({"area": area, "group1": "L2/3", "group2": "L4/5"})
        layer_tests.append(rec)
    for rec, ph in zip(layer_tests, holm_adjust([r.get("p", np.nan) for r in layer_tests])):
        rec["p_holm_across_areas"] = ph
        out["session_level_primary"]["layer_within_area"][rec["area"]] = rec
    return out


def v5_responsive_stats_lines(stats_out: dict[str, Any], areas: list[str]) -> list[str]:
    s = stats_out["session_level_primary"]
    lines = [
        "PRIMARY: session-level % responsive; GREEN and YELLOW are analyzed separately.",
        "Area tests: Kruskal-Wallis within each layer; pairwise 2-sided Mann-Whitney U, Holm across 3 area pairs.",
        "Layer tests: 2-sided Mann-Whitney U (L2/3 vs L4/5) within each area; Holm across the 3 areas.",
    ]
    six = s["six_group_omnibus"]
    lines.append(f"Six-stratum omnibus: H={six.get('H',np.nan):.3g}, p={v5_fmt_p(six.get('p',np.nan))}")
    for lk, ll in V5_LAYER_ORDER:
        rec = s["area_within_layer"][lk]; o = rec["omnibus"]
        lines.append(f"{ll} area omnibus: H={o.get('H',np.nan):.3g}, p={v5_fmt_p(o.get('p',np.nan))}, Holm(2 layers)={v5_fmt_p(o.get('p_holm_across_layers',np.nan))}")
        for pr in rec["pairwise"].values():
            lines.append(f"  {pr['area1']} vs {pr['area2']}: U={pr['U']:.3g}, p={v5_fmt_p(pr['p'])}, Holm(3 pairs)={v5_fmt_p(pr['p_holm'])}, delta={pr['cliffs_delta']:.3g}")
    for area in areas:
        r = s["layer_within_area"][area]
        lines.append(f"{area} L2/3 vs L4/5: U={r['U']:.3g}, p={v5_fmt_p(r['p'])}, Holm(3 areas)={v5_fmt_p(r['p_holm_across_areas'])}, delta={r['cliffs_delta']:.3g}")
    return lines


def v5_plot_responsive_fraction_area_layer(cells: pd.DataFrame, areas: list[str], cell_class: str,
                                           mode: str, outdir: Path) -> dict[str, Any]:
    stats_out = v5_responsive_area_layer_stats(cells, areas)
    tab = v5_session_responsive_area_layer(cells)
    outdir.mkdir(parents=True, exist_ok=True)
    fig = plt.figure(figsize=(13.5, 9.2))
    gs = fig.add_gridspec(2, 1, height_ratios=[1.75, 1.0], hspace=.18)
    ax = fig.add_subplot(gs[0, 0]); axstat = fig.add_subplot(gs[1, 0]); axstat.axis("off")
    ...
    for area in areas:
        ...
        for lk, ll in V5_LAYER_ORDER:
            xpos[area][lk] = x
            vals = finite_values(tab.loc[(tab.canonicalArea == area) & (tab.layer_key == lk), "pct_sound_responsive"])
            if len(vals):
                marker = "o" if lk == "L23" else "s"
                ax.scatter(np.full(len(vals), x) + rng.uniform(-.08, .08, len(vals)), vals, s=30, alpha=.35,
                           color=AREA_COLORS[area], marker=marker, edgecolors="none")
                m, se, n = mean_sem(vals)
                ax.errorbar(x, m, yerr=se if np.isfinite(se) else None, fmt=marker, ms=10,
                            capsize=6, elinewidth=2.4, color=AREA_COLORS[area], zorder=5)
                ax.text(x, min(98, m + (se if np.isfinite(se) else 0) + 5), f"n={n}", ha="center", fontsize=8)
            xt.append(x); xlabs.append(f"{area}\n{ll}"); x += .82
        x += .55
    ax.set_xticks(xt, xlabs); ax.set_ylim(0, 100); ax.set_ylabel("Sound-responsive cells per session (%)")
    ax.grid(axis="y", alpha=.22); ax.spines[["top", "right"]].set_visible(False)
    ax.set_title(f"{cell_class.upper()} only — {mode_title(mode)}\nFraction of sound-responsive cells across areas and layers\nresponsive = union of v9 tuning-value-specific BH-FDR discoveries")

    ybase = 97.0
    for ai, area in enumerate(areas):
        r = stats_out["session_level_primary"]["layer_within_area"][area]
        ph = r.get("p_holm_across_areas", np.nan)
        if np.isfinite(ph) and ph < .05:
            x1, x2 = xpos[area]["L23"], xpos[area]["L45"]; y = ybase - ai * 4.0
            ax.plot([x1,x1,x2,x2], [y-1,y,y,y-1], color=".2", lw=1)
            ax.text((x1+x2)/2, y+.8, f"layer Holm p={ph:.3g}", ha="center", va="bottom", fontsize=7.5)
    axstat.text(.01, .98, "\n".join(v5_responsive_stats_lines(stats_out, areas)), ha="left", va="top",
                family="monospace", fontsize=8.3, transform=axstat.transAxes)
    fig.savefig(outdir/"sound_responsive_fraction_across_area_and_layer_SESSION_POINTS.png", dpi=DPI, bbox_inches="tight")
    plt.close(fig)

    rows=[]; s=stats_out["session_level_primary"]
    for lk,ll in V5_LAYER_ORDER:
        rec=s["area_within_layer"][lk]
        for pr in rec["pairwise"].values():
            rows.append({"question":"responsive_fraction","test_family":"area_within_layer","layer":ll,
                         "area1":pr["area1"],"area2":pr["area2"],"U":pr["U"],"p":pr["p"],"p_holm":pr["p_holm"],
                         "cliffs_delta":pr["cliffs_delta"],"omnibus_H":rec["omnibus"].get("H",np.nan),
                         "omnibus_p":rec["omnibus"].get("p",np.nan),"omnibus_p_holm":rec["omnibus"].get("p_holm_across_layers",np.nan)})
    for area,r in s["layer_within_area"].items():
        rows.append({"question":"responsive_fraction","test_family":"layer_within_area","layer":"L2/3_vs_L4/5",
                     "area1":area,"area2":area,"U":r["U"],"p":r["p"],"p_holm":r["p_holm_across_areas"],
                     "cliffs_delta":r["cliffs_delta"],"omnibus_H":np.nan,"omnibus_p":np.nan,"omnibus_p_holm":np.nan})
    pd.DataFrame(rows).to_csv(outdir/"sound_responsive_fraction_across_area_and_layer_statistics.csv", index=False)
    save_json(outdir/"sound_responsive_fraction_across_area_and_layer_statistics.json", stats_out)
    return stats_out


def v5_best_frequency_session_distribution(cells: pd.DataFrame, frequencies_hz: np.ndarray) -> pd.DataFrame:
    d = add_layer_columns(cells); d = d[d.layer_key.notna()].copy(); rows=[]
    for (sk, area, layer), sdf in d.groupby(["session_key", "canonicalArea", "layer_key"], dropna=False):
        pref = pd.to_numeric(sdf["preferred_tuning_value_Hz"], errors="coerce").to_numpy(float)
        pref = pref[np.isfinite(pref) & (pref > 0)]; denom=len(pref)
        if denom == 0: continue
        for f in frequencies_hz:
            rows.append({"session_key":str(sk),"canonicalArea":str(area),"layer_key":str(layer),
                         "frequency_Hz":float(f),"pct_cells":100.0*float(np.mean(np.isclose(pref,f))),
                         "n_cells_with_best_frequency":int(denom)})
    return pd.DataFrame(rows)


def v5_best_frequency_stats(cells: pd.DataFrame, frequencies_hz: np.ndarray, areas: list[str]) -> dict[str, Any]:
    tab = v5_best_frequency_session_distribution(cells, frequencies_hz)
    out: dict[str, Any] = {"area_within_layer_by_frequency":{}, "layer_within_area_by_frequency":{}}
    for lk,ll in V5_LAYER_ORDER:
        layer_recs={}; omni_ps=[]; flat_pair_refs=[]; flat_pair_ps=[]
        for f in frequencies_hz:
            groups={a:tab.loc[(tab.canonicalArea==a)&(tab.layer_key==lk)&np.isclose(tab.frequency_Hz,f),"pct_cells"] for a in areas}
            rec={"frequency_Hz":float(f),"layer_key":lk,"layer_label":ll,"omnibus":safe_kruskal(groups),"pairwise":pairwise_area_tests(groups,areas)}
            fkey=f"{float(f):g}Hz"; layer_recs[fkey]=rec; omni_ps.append(rec["omnibus"].get("p",np.nan))
            for pk,pr in rec["pairwise"].items(): flat_pair_refs.append((fkey,pk)); flat_pair_ps.append(pr.get("p",np.nan))
        for rec,ph in zip(layer_recs.values(),holm_adjust(omni_ps)): rec["omnibus"]["p_holm_across_frequency_bins"]=ph
        for (fkey,pk),ph in zip(flat_pair_refs,holm_adjust(flat_pair_ps)): layer_recs[fkey]["pairwise"][pk]["p_holm_layer_frequency_pair_family"]=ph
        out["area_within_layer_by_frequency"][lk]=layer_recs

    flat=[]; ps=[]
    for area in areas:
        out["layer_within_area_by_frequency"][area]={}
        for f in frequencies_hz:
            x=tab.loc[(tab.canonicalArea==area)&(tab.layer_key=="L23")&np.isclose(tab.frequency_Hz,f),"pct_cells"]
            y=tab.loc[(tab.canonicalArea==area)&(tab.layer_key=="L45")&np.isclose(tab.frequency_Hz,f),"pct_cells"]
            r=safe_mannwhitney(x,y); r.update({"area":area,"frequency_Hz":float(f),"group1":"L2/3","group2":"L4/5"})
            key=f"{float(f):g}Hz"; out["layer_within_area_by_frequency"][area][key]=r; flat.append((area,key)); ps.append(r.get("p",np.nan))
    for (area,key),ph in zip(flat,holm_adjust(ps)): out["layer_within_area_by_frequency"][area][key]["p_holm_all_area_frequency_layer_tests"]=ph
    return out


def v5_bf_profile_summary(tab: pd.DataFrame, area: str, layer: str, frequencies_hz: np.ndarray):
    sdf=tab[(tab.canonicalArea==area)&(tab.layer_key==layer)]; profiles=[]; ncell=[]
    for _,g in sdf.groupby("session_key",sort=True):
        prof=np.asarray([float(g[np.isclose(g.frequency_Hz,f)].iloc[0].pct_cells) if len(g[np.isclose(g.frequency_Hz,f)]) else np.nan for f in frequencies_hz])
        profiles.append(prof); ncell.append(int(g.n_cells_with_best_frequency.iloc[0]))
    M=np.vstack(profiles) if profiles else np.empty((0,len(frequencies_hz)))
    if len(M):
        mean=np.nanmean(M,axis=0); n=np.sum(np.isfinite(M),axis=0); sem=np.full(len(frequencies_hz),np.nan)
        valid=n>1; sem[valid]=np.nanstd(M[:,valid],axis=0,ddof=1)/np.sqrt(n[valid])
    else:
        mean=np.full(len(frequencies_hz),np.nan); sem=np.full(len(frequencies_hz),np.nan)
    return M,mean,sem,ncell


def v5_plot_bf_facets(cells: pd.DataFrame, frequencies_hz: np.ndarray, areas: list[str], cell_class: str,
                      outdir: Path, subset_label: str, filename: str) -> None:
    tab=v5_best_frequency_session_distribution(cells,frequencies_hz); x=xplot(frequencies_hz,"tuningFreq")
    fig,axes=plt.subplots(2,len(areas),figsize=(5.2*len(areas),9.2),sharex=True,sharey=True,squeeze=False)
    for r,(lk,ll) in enumerate(V5_LAYER_ORDER):
        for c,area in enumerate(areas):
            ax=axes[r,c]; M,mean,sem,ncell=v5_bf_profile_summary(tab,area,lk,frequencies_hz)
            for prof in M: ax.plot(x,prof,color=AREA_COLORS[area],alpha=.17,lw=.9,marker="o",ms=2)
            if np.any(np.isfinite(mean)): ax.errorbar(x,mean,yerr=sem,fmt="o-",lw=2.5,ms=4.5,capsize=3,color=AREA_COLORS[area])
            apply_log2_tuning_axis(ax,frequencies_hz,"tuningFreq"); ax.set_ylim(0,100); ax.grid(axis="y",alpha=.2); ax.spines[["top","right"]].set_visible(False)
            ax.set_title(f"{area} — {ll}\n{M.shape[0]} sessions, {sum(ncell)} cells")
            if c==0: ax.set_ylabel("Cells with this best frequency (%)\n(session-normalized)")
            if r==1: ax.set_xlabel("Best frequency (kHz)")
    fig.suptitle(f"{cell_class.upper()} only — best-frequency distributions by area and layer\n{subset_label}\nlight lines = individual sessions; dark = mean session distribution ± SEM",fontsize=13)
    fig.tight_layout(rect=[0,0,1,.94]); fig.savefig(outdir/filename,dpi=DPI,bbox_inches="tight"); plt.close(fig)


def v5_bf_area_test_lines(stats_out: dict[str,Any], layer_key: str, frequencies_hz: np.ndarray) -> list[str]:
    ll=dict(V5_LAYER_ORDER)[layer_key]
    lines=[f"{ll}: session-level BF-bin percentages across A1/AAF/A2.",
           "KW omnibus at each BF bin; omnibus Holm across BF bins. Pairwise MW Holm across all BF-bin x 3 area-pair tests within this layer."]
    recs=stats_out["area_within_layer_by_frequency"][layer_key]
    for f in frequencies_hz:
        rec=recs[f"{float(f):g}Hz"]; o=rec["omnibus"]
        lines.append(f"{f/1000:g} kHz: H={o.get('H',np.nan):.3g}, p={v5_fmt_p(o.get('p',np.nan))}, HolmBins={v5_fmt_p(o.get('p_holm_across_frequency_bins',np.nan))}")
        for pr in rec["pairwise"].values():
            lines.append(f"  {pr['area1']}-{pr['area2']}: U={pr['U']:.3g}, p={v5_fmt_p(pr['p'])}, HolmFreqxPair={v5_fmt_p(pr.get('p_holm_layer_frequency_pair_family',np.nan))}, delta={pr['cliffs_delta']:.3g}")
    return lines


def v5_plot_bf_area_tests(cells: pd.DataFrame, frequencies_hz: np.ndarray, areas: list[str], cell_class: str,
                          outdir: Path, stats_out: dict[str,Any], layer_key: str, suffix: str) -> None:
    tab=v5_best_frequency_session_distribution(cells,frequencies_hz); ll=dict(V5_LAYER_ORDER)[layer_key]; x=xplot(frequencies_hz,"tuningFreq")
    fig=plt.figure(figsize=(14.2,9.8)); gs=fig.add_gridspec(2,1,height_ratios=[1.25,1],hspace=.15); ax=fig.add_subplot(gs[0]); axstat=fig.add_subplot(gs[1]); axstat.axis("off")
    for area in areas:
        M,mean,sem,_=v5_bf_profile_summary(tab,area,layer_key,frequencies_hz)
        if np.any(np.isfinite(mean)): ax.errorbar(x,mean,yerr=sem,fmt="o-",lw=2.5,ms=5,capsize=3,color=AREA_COLORS[area],label=f"{area} (n={len(M)} sessions)")
    apply_log2_tuning_axis(ax,frequencies_hz,"tuningFreq"); ax.set_ylim(0,100); ax.set_ylabel("Cells with this best frequency (%)\n(session-normalized)"); ax.set_xlabel("Best frequency (kHz)"); ax.legend(frameon=False); ax.grid(axis="y",alpha=.2); ax.spines[["top","right"]].set_visible(False)
    recs=stats_out["area_within_layer_by_frequency"][layer_key]
    for xx,f in zip(x,frequencies_hz):
        rec=recs[f"{float(f):g}Hz"]; ph=rec["omnibus"].get("p_holm_across_frequency_bins",np.nan); sig=[]
        for pr in rec["pairwise"].values():
            pp=pr.get("p_holm_layer_frequency_pair_family",np.nan)
            if np.isfinite(pp) and pp<.05: sig.append(f"{pr['area1']}-{pr['area2']} {pp:.2g}")
        txt=("* " if np.isfinite(ph) and ph<.05 else "")+f"KW Holm p={v5_fmt_p(ph)}"
        if sig: txt+="\n"+"\n".join(sig)
        ax.text(xx,96,txt,ha="center",va="top",fontsize=7)
    ax.set_title(f"{cell_class.upper()} FDR-responsive cells — {ll}\nBest-frequency distribution: explicit cross-area tests")
    axstat.text(.01,.99,"\n".join(v5_bf_area_test_lines(stats_out,layer_key,frequencies_hz)),ha="left",va="top",family="monospace",fontsize=7.6,transform=axstat.transAxes)
    fig.savefig(outdir/f"best_frequency_distribution_FDR_responsive_only_AREA_TESTS_{suffix}.png",dpi=DPI,bbox_inches="tight"); plt.close(fig)


def v5_plot_bf_layer_tests(cells: pd.DataFrame, frequencies_hz: np.ndarray, areas: list[str], cell_class: str,
                           outdir: Path, stats_out: dict[str,Any]) -> None:
    tab=v5_best_frequency_session_distribution(cells,frequencies_hz); x=xplot(frequencies_hz,"tuningFreq")
    fig,axes=plt.subplots(len(areas),1,figsize=(12.8,4.8*len(areas)),sharex=True,squeeze=False); axes=axes[:,0]
    for ax,area in zip(axes,areas):
        for lk,ll,ls,marker in [("L23","L2/3","-","o"),("L45","L4/5","--","s")]:
            M,mean,sem,_=v5_bf_profile_summary(tab,area,lk,frequencies_hz)
            if np.any(np.isfinite(mean)): ax.errorbar(x,mean,yerr=sem,fmt=marker+ls,lw=2.3,ms=4.5,capsize=3,color=AREA_COLORS[area],label=f"{ll} (n={len(M)} sessions)")
        apply_log2_tuning_axis(ax,frequencies_hz,"tuningFreq"); ax.set_ylim(0,100); ax.set_ylabel("Cells (%)"); ax.grid(axis="y",alpha=.2); ax.spines[["top","right"]].set_visible(False); ax.legend(frameon=False)
        lines=[]
        for xx,f in zip(x,frequencies_hz):
            r=stats_out["layer_within_area_by_frequency"][area][f"{float(f):g}Hz"]; ph=r.get("p_holm_all_area_frequency_layer_tests",np.nan)
            ax.text(xx,96,("* " if np.isfinite(ph) and ph<.05 else "")+f"Holm p={v5_fmt_p(ph)}",ha="center",va="top",fontsize=7)
            lines.append(f"{f/1000:g}k: U={r['U']:.3g}, p={v5_fmt_p(r['p'])}, Holm={v5_fmt_p(ph)}")
        ax.set_title(f"{area}: L2/3 vs L4/5 | "+"; ".join(lines),fontsize=8.5)
    axes[-1].set_xlabel("Best frequency (kHz)")
    fig.suptitle(f"{cell_class.upper()} FDR-responsive cells — best-frequency distribution\nExplicit layer tests within each area; Holm across all area x BF-bin layer comparisons",fontsize=13)
    fig.tight_layout(rect=[0,0,1,.96]); fig.savefig(outdir/"best_frequency_distribution_FDR_responsive_only_LAYER_TESTS_BY_AREA.png",dpi=DPI,bbox_inches="tight"); plt.close(fig)


def v5_save_bf_stats_table(stats_out: dict[str,Any], frequencies_hz: np.ndarray, areas: list[str], outdir: Path) -> None:
    rows=[]
    for lk,ll in V5_LAYER_ORDER:
        for f in frequencies_hz:
            rec=stats_out["area_within_layer_by_frequency"][lk][f"{float(f):g}Hz"]
            for pr in rec["pairwise"].values():
                rows.append({"test_family":"area_within_layer_by_best_frequency_bin","layer":ll,"frequency_Hz":f,
                             "area1":pr["area1"],"area2":pr["area2"],"U":pr["U"],"p":pr["p"],
                             "p_holm":pr.get("p_holm_layer_frequency_pair_family",np.nan),"cliffs_delta":pr["cliffs_delta"],
                             "omnibus_H":rec["omnibus"].get("H",np.nan),"omnibus_p":rec["omnibus"].get("p",np.nan),
                             "omnibus_p_holm":rec["omnibus"].get("p_holm_across_frequency_bins",np.nan)})
    for area in areas:
        for f in frequencies_hz:
            r=stats_out["layer_within_area_by_frequency"][area][f"{float(f):g}Hz"]
            rows.append({"test_family":"layer_within_area_by_best_frequency_bin","layer":"L2/3_vs_L4/5","frequency_Hz":f,
                         "area1":area,"area2":area,"U":r["U"],"p":r["p"],"p_holm":r.get("p_holm_all_area_frequency_layer_tests",np.nan),
                         "cliffs_delta":r["cliffs_delta"],"omnibus_H":np.nan,"omnibus_p":np.nan,"omnibus_p_holm":np.nan})
    pd.DataFrame(rows).to_csv(outdir/"best_frequency_distribution_FDR_responsive_only_statistics.csv",index=False)
    save_json(outdir/"best_frequency_distribution_FDR_responsive_only_statistics.json",stats_out)


def v5_make_explicit_area_layer_questions(all_cells: pd.DataFrame, areas: list[str], mode: str, mode_root: Path) -> None:
    root=mode_root/"explicit_area_layer_questions"; root.mkdir(parents=True,exist_ok=True)
    layered=add_layer_columns(all_cells); layered=layered[layered.layer_key.notna()].copy()
    for cell_class in CELL_CLASSES:
        cdf=layered[layered.cell_class==cell_class].copy(); out=root/cell_class; out.mkdir(parents=True,exist_ok=True)
        if len(cdf)==0: continue
        v5_plot_responsive_fraction_area_layer(cdf,areas,cell_class,mode,out)
        if mode=="tuningFreq" and "preferred_tuning_value_Hz" in cdf:
            rdf=cdf[v4_bool_series(cdf.stim_responsive_fdr)].copy()
            # Use the full tested frequency grid from the response columns, not only
            # frequencies that happened to win as a preferred frequency in this dataset.
            _, freqs, _ = response_columns(cdf, "tuningFreq")
            freqs = np.asarray(freqs, float)
            freqs = freqs[np.isfinite(freqs) & (freqs > 0)]
            if len(freqs):
                if len(rdf):
                    stats_out=v5_best_frequency_stats(rdf,freqs,areas)
                    v5_plot_bf_facets(rdf,freqs,areas,cell_class,out,"PRIMARY: FDR sound-responsive cells only","best_frequency_distribution_FDR_responsive_only_BY_AREA_LAYER.png")
                    v5_plot_bf_area_tests(rdf,freqs,areas,cell_class,out,stats_out,"L23","L23")
                    v5_plot_bf_area_tests(rdf,freqs,areas,cell_class,out,stats_out,"L45","L45")
                    v5_plot_bf_layer_tests(rdf,freqs,areas,cell_class,out,stats_out)
                    v5_save_bf_stats_table(stats_out,freqs,areas,out)
                v5_plot_bf_facets(cdf,freqs,areas,cell_class,out,"DESCRIPTIVE SENSITIVITY: all classified cells; BF of nonresponsive cells may be arbitrary","best_frequency_distribution_all_cells_BY_AREA_LAYER_DESCRIPTIVE.png")
...
def main_v5() -> None:
    ...
    print("="*78); print("AM_TUNING CROSS-AREA CELL-CLASS ANALYSIS v5"); print("="*78); print(f"Subject: {subject}"); print(f"Dates:   {', '.join(dates)}"); print(f"Areas:   {', '.join(areas)}"); print(f"Modes:   {', '.join(modes)}"); print(f"Output root: {outroot}")
    expected={}
    for mode in modes:
    ...
    outroot.mkdir(parents=True,exist_ok=True); save_json(outroot/"run_manifest.json",{"script_version":"v5","subject":subject,"dates":dates,"areas":areas,"modes":modes,"inputs":expected,"prerequisite":"am_tuning_area_analysis_v9.py","primary_inference":"session-level; Kruskal-Wallis + pairwise Mann-Whitney, pairwise Holm","selectivity_family":list(V4_PRIMARY_SELECTIVITY),"responsiveness":"v9 tuning-value-specific BH-FDR union","alternate_dff_sensitivity":[V4_LIFETIME_SESSION_MEDIAN,V4_LIFETIME_PRESTIM60],"numeric_tuning_axes":"explicit x=log2(value) for frequency and AM rate","trial_reduction":{"fractions":list(TRIAL_REDUCTION_FRACTIONS),"resamples":args.trial_reduction_resamples,"response_permutations":args.trial_reduction_response_permutations,"seed":args.trial_reduction_seed,"enabled":not args.skip_trial_reduction},"v5_explicit_area_layer_questions":{"sound_responsive_fraction":"session-level percent responsive; area within layer plus layer within area, cell classes separate","best_frequency_distribution":"tuningFreq only; primary FDR-responsive cells; session-normalized BF-bin percentages; explicit area/layer tests"}})
    quick=[]
    for mode in modes:
        ...
        # V5 prespecified questions: responsive fraction and (tuningFreq only) best-frequency distributions.
        v5_make_explicit_area_layer_questions(cells,areas,mode,mode_root)
        # Existing targeted selectivity area x layer outputs.
