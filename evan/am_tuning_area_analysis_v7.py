#!/usr/bin/env python3
"""
Batch/area-level analysis of two-photon am_tuning sessions (selectivity-expanded v6).
...
Figures
-------
The expanded figure philosophy from the finalized sound_tuning v10 workflow is
used here as well:
...
Example
-------
python am_tuning_area_analysis_v7.py imag039 \
    --area A1 \
...
def analyze_session(meta: dict[str, Any], plane: int, n_permutations: int, seed: int,
    ...
    tuning_lifetime_sparseness = np.full(n_cells, np.nan)
    tuning_gini_coefficient = np.full(n_cells, np.nan)
    tuning_fano_factor = np.full(n_cells, np.nan)
    tuning_fwhm_values = np.full(n_cells, np.nan)
    for ci in np.flatnonzero(np.any(np.isfinite(tuning_curves), axis=1)):
        ...
        # Primary/new selectivity measures use response magnitude so excitation
        # and suppression are treated symmetrically and all measures are nonnegative.
        magnitude_profile = np.abs(curve[finite])
        tuning_lifetime_sparseness[ci] = lifetime_sparseness_nonnegative(magnitude_profile)
        tuning_gini_coefficient[ci] = gini_coefficient_nonnegative(magnitude_profile)
        tuning_fano_factor[ci] = fano_factor_nonnegative(magnitude_profile)
        tuning_fwhm_values[ci] = tuning_fwhm(curve, possible_values, tuning_mode)
...
def _apply_log2_tuning_axis(ax, values_hz: np.ndarray, mode: str) -> None:
    """Use a base-2 logarithmic tuning axis with ticks at the measured values.

    Frequency is displayed in kHz and AM rate in Hz.  All ordinary tuning values
    are positive; if a non-positive value is ever present, the plot is left on a
    linear axis rather than silently dropping that stimulus.
    """
    values_hz = np.asarray(values_hz, dtype=float).ravel()
    x = _xplot(values_hz, mode)
    ...
    if np.any(xfinite <= 0):
        print(
            f"WARNING: {mode} contains non-positive tuning values; "
            "cannot use a true log2 x-axis for this plot. Leaving x-axis linear."
        )
        ax.set_xticks(xfinite)
        ax.set_xticklabels(np.asarray(_log2_tick_labels(values_hz[finite], mode), dtype=object))
        return
    ax.set_xscale("log", base=2)
    ax.set_xticks(xfinite)
    ax.set_xticklabels(_log2_tick_labels(values_hz[finite], mode))
    # Small symmetric margin in log2 coordinates so edge points/error bars are not clipped.
    lo = float(np.min(xfinite)); hi = float(np.max(xfinite))
    if hi > lo:
        margin_oct = 0.12
...
def _log2_bin_edges(values_hz: np.ndarray, mode: str) -> np.ndarray | None:
    """Geometric bin edges for raster columns on a true log2 tuning axis."""
    values_hz = np.asarray(values_hz, dtype=float).ravel()
    x = _xplot(values_hz, mode)
    ...
    for xpos, vals0, label, color in zip(xvals, values_by_tuning, labels, colors):
        ...
        if len(vals):
            if np.isfinite(xpos) and xpos > 0:
                # Jitter symmetrically in log2 space so the visual spread is constant
                # across low and high frequencies/rates.
                xj = xpos * (2.0 ** rng.uniform(-0.045, 0.045, len(vals)))
            else:
...
def write_combined_report(path: Path, mode: str, cell_df: pd.DataFrame, class_summary_df: pd.DataFrame, stats_dict: dict[str, Any]) -> None:
    ...
    lines.append("FWHM is reported in octaves for tuningFreq and Hz for tuningAM; unbracketed half-max crossings are NaN.\nAll numeric frequency- and AM-rate x-axes use base-2 logarithmic scaling with ticks at the measured stimulus values.\n")
