...
* New in v12: the two alternate dF/F definitions already used for eventlocked
  QC are also propagated into per-cell condition-response outputs. For each
  alternate dF/F, the script exports FI, FO, every AM-rate response, best-AM
  response/rate, FI/FO selectivity, and FI/best-AM selectivity. The response
  measure is kept identical to the primary analysis: mean 0-1 s minus mean
  -1-0 s. Responsiveness/FDR labels remain those from the primary jaratoolbox
  dff=True analysis so the alternate baselines are sensitivity analyses rather
  than separate discovery families.

Example
-------
python sound_tuning_area_analysis_v12.py imag039 \
    --area A1 \
