...
plt.plot(groom, color='0.7')
plt.plot(moving_average(groom,avgwidth_grooming))
plt.xlabel('Frame')
...
## Whisking (Final Draft)
# Use moving average to combat repeated video frames
avgwidth_whisking = 5
whisking_sum = moving_average(whisking_sum,avgwidth_whisking)
# Plot whisking
plt.figure()
...
## Pupil Area (TO DO: Impute the missing values.)
pArea[blinks_bool==True]=np.nan
pArea[pArea==0.0]=np.nan
pArea[pArea<5]=np.nan # Hardcoded - a bit hacky.
# Plot Pupil Area
plt.figure()
ax1 = plt.subplot(2,1,1)
plt.plot(pArea)
...
## Plot Running (Final Draft)
plt.figure()
plt.plot(running_sum, color='0.7')
plt.plot(moving_average(running_sum,5),color='r')
plt.xlabel('Frame')
