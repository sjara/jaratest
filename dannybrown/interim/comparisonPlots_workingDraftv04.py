...
#filename_behav = infovideos.videos.sessions[3].behavFile 
filename_behav = selSession.behavFile # Hardcoded - sub in 3 for the date match from above.
...
def comparison_plot(time, valuesData, pVals, freqs_list, nTrials, firstSound): 
     ...
     for f_ind, freq in enumerate(freqs_list):
        iteration = f'Change = {firstSound}-->{freq} Hz, nTrials = {nTrials[f_ind]}, pval = {pVals[f_ind]}'
...
OverLapPlots = comparison_plot(pupilDilationTimeWindowVec, meanpAreaDilated, pvals, freqs_list=freqs_list, nTrials = nTrials, firstSound=15000)
