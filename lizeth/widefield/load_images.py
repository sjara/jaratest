...
#session = '120701' #Anesthesia Intrinsic
#session = '121856' #Anesthesia Intrinsic
#session = '131605' #Anesthesia Intrinsic
#session = '133151' #Anesthesia Intrinsic
#session = '142707' #Anesthesia Intrinsic
...
#stimulus_filename = '/data/widefield/' + mouse_name + '/' + mouse_name + '_am_tuning_curve_' + date + '_' + session + '.h5' #Thios one
...
INTENSITY_SCALE = [0.0, 0.005]#[None, None]  # [-0.05, 0.1]  # [None, None]  
...
for indf, freq in enumerate(possible_freq):
    if p==0:
        INTENSITY_SCALE = [-0.001, 0.01] #[-0.01, 0.008] [-0.01, 0.01]] [-0.01, 0.008]
    elif p==1:
        INTENSITY_SCALE = [-0.001, 0.015]
    elif p==2:
    ...
    plt.sca(axs[indf, 0])
    plt.imshow(np.rot90(avg_evoked_each_freq[indf]), cmap=cmap) 
    plt.colorbar()
    plt.title(f'Evoked')
    plt.ylabel(f'{freq:g} Hz')

    plt.sca(axs[indf, 1])
    plt.imshow(np.rot90(avg_baseline_each_freq[indf]), cmap=cmap) 
    plt.colorbar()
    plt.title('Baseline')

    plt.sca(axs[indf, 2])
    plt.imshow(np.rot90(signal_change_each_freq[indf]), cmap=cmap, vmin=INTENSITY_SCALE[0], vmax=INTENSITY_SCALE[1]) #signal_change_each_freq
    plt.colorbar()
...
#     plt.clf()
#     fig = plt.gcf()
#     axs = fig.subplots(int(n_freq), 6, sharex=True, sharey=True) 
#     cmap = 'viridis'
