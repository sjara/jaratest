"""
Shows the evoked signal, baseline and signal change
"""
...
for indf, freq in enumerate(wf_data['possible_freq']):
    plt.sca(axs[indf, 0])
    plt.imshow(np.rot90(wf_data['avg_evoked_each_freq'][indf]), cmap=cmap) 
    plt.colorbar()
    plt.title(f'Evoked')
    plt.ylabel(f'{freq:g} Hz')

    plt.sca(axs[indf, 1])
    plt.imshow(np.rot90(wf_data['avg_baseline_each_freq'][indf]), cmap=cmap) 
    plt.colorbar()
    plt.title('Baseline')
