...
celldb = celldatabase.generate_cell_database(inforecFile, ignoreMissing=True, onlyGood=False)
...
'''
We want to plot the raster for units where cluster_label=='good', 
but show both indices: the original one (that includes bad units),
and the one only for good units.
'''
