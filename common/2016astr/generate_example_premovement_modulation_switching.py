'''
Generate and store intermediate data for plot showing movement-onset-aligned firing activity of astr neurons recorded in switching task, only middle frequency is plotted. Data for raster and psth are saved separately. 
...
for cellParams in cellParamsList:
    ...
    # Calculate movement onset times
    diffTimes=bdata['timeCenterOut']-bdata['timeTarget']
    movementOnsetTimes = soundOnsetTimeEphys+diffTimes
    ...
    outputFile = 'example_switching_midfreq_movementaligned_raster_{}_{}_T{}_c{}.npz'.format(oneCell.animalName, oneCell.behavSession, oneCell.tetrode,oneCell.cluster)
    outputFullPath = os.path.join(dataDir,outputFile)
    np.savez(outputFullPath, spikeTimestamps=spikeTimestamps, eventOnsetTimes=movementOnsetTimes, indexLimitsEachTrial=indexLimitsEachTrial,spikeTimesFromEventOnset=spikeTimesFromEventOnset, trialsEachCond=trialsEachCond, colorEachCond=colorEachCond, script=scriptFullPath, EPHYS_SAMPLING_RATE=EPHYS_SAMPLING_RATE, soundTriggerChannel=soundTriggerChannel, plotByBlock=plotByBlock, minBlockSize=minBlockSize, timeRange=timeRange, colorMidFreqL=colorsDict['highBlock'], colorMidFreqR=colorsDict['lowBlock'], frequencyPloted=middleFreq, **cellParams)
