'''
Generate and store intermediate data for plot showing frequency-selectivity of astr sound-responsive neurons (using 2afc data). Data for raster and psth are saved separately. Only use correct trials to make movement effect more obvious.
...
cellParamsList = [{'firstParam':'test053',
                   ...
                   'tetrode':5,
                   'cluster':7}, #strong sharp response to low freqs, no movement
                  {'firstParam':'adap017',
                   'behavSession':'20160317a',
                   'tetrode':5,
                   'cluster':3},#both sound and movement responsive
                  {'firstParam':'test055',
                   'behavSession':'20150303a',
                   'tetrode':1,
                   'cluster':2}, #movement no sound response
                  {'firstParam':'adap017',
                   'behavSession':'20160328a',
                   'tetrode':8,
                   'cluster':5}, #movement no sound response
                  {'firstParam':'adap017',
                   'behavSession':'20160328a',
                   'tetrode':7,
...
for cellParams in cellParamsList:
    ...
    trialEachFreq = trialsEachFreq & correct[:,np.newaxis] #Use only valid trials where sound is played in full 
    ...
    outputFile = 'example_sound_movement_sel_raster_{}_{}_T{}_c{}.npz'.format(oneCell.animalName, oneCell.behavSession, oneCell.tetrode,oneCell.cluster)
    ...
    ### Save psth data ###
    #outputDir = os.path.join(settings.FIGURESDATA, figparams.STUDY_NAME)
    outputFile = 'example_sound_movement_sel_psth_{}_{}_T{}_c{}.npz'.format(oneCell.animalName, oneCell.behavSession,oneCell.tetrode,oneCell.cluster)
    outputFullPath = os.path.join(dataDir,outputFile)
