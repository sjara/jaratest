...
dbPath = '/tmp/db_with_maxFR.h5'
database = celldatabase.load_hdf(dbPath)

database['evokedFR'] = database['maxFR'] - database['baseFR']
...
goodPulseLatency = goodNSpikes.query('summaryPulseLatency<0.01')
...
messages.append("{} ATh median = {}".format(popStatCol, np.median(thalPopStat)))
messages.append("{} AC median = {}".format(popStatCol, np.median(acPopStat)))
