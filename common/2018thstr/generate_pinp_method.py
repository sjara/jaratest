...
for indIter, (indRow, dbRow) in enumerate(dataframe.iterrows()):
    ...
    if passPulse and passTrain:
        print "post PASS"
        dataframe.loc[indRow, 'survivedNBQX'] = 1
    else:
        print "post FAIL"
        dataframe.loc[indRow, 'survivedNBQX'] = 0
    ...
    #### Noiseburst washout - did the cell come back??
    try:
        ephysData, _ = cell.load('noiseburst_washout')
    except (IndexError, ValueError):
        continue
    ...
    if pVal<0.05:
        dataframe.loc[indRow, 'washout'] = 1
...
nbqxTagged = dataframe.query('isiViolations<0.02 and spikeShapeQuality>2 and autoTagged==1 and survivedNBQX==1')
nbqxUntagged = dataframe.query('isiViolations<0.02 and spikeShapeQuality>2 and autoTagged==0 and survivedNBQX==1')

nonNbqxTagged = dataframe.query('isiViolations<0.02 and spikeShapeQuality>2 and autoTagged==1 and survivedNBQX==0')
nonNbqxUntagged = dataframe.query('isiViolations<0.02 and spikeShapeQuality>2 and autoTagged==0 and survivedNBQX==0')
