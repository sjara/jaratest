...
colorUntagged = colorpalette.TangoPalette['Orange2']
...
oddsratio, pValue = stats.fisher_exact([[taggedSyncN, untaggedSyncN],
                                        [taggedNonSyncN, untaggedNonSyncN]])
if pValue < 0.05:
    starMarker = '*'
else:
    starMarker = 'n.s.'
plot_y_lines_with_ticks(axUntaggedNSYNC, xBars[1], yCircleCenters[0], yCircleCenters[1]-0.1,
...
colors = [colorTagged, colorUntagged, colorFarUntagged]
whiskerColors = [colorTagged, colorTagged, colorUntagged,
                 colorUntagged]
linewidth=2
for patch, color in zip(bp['boxes'], colors):
