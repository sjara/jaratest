...
colorAC = colorpalette.TangoPalette['ScarletRed2']
colorATh = colorpalette.TangoPalette['SkyBlue2']
...
goodISI = dbase.query('isiViolations<0.02 or modifiedISI<0.02')
goodShape = goodISI.query('spikeShapeQuality > 2')
goodFit = goodShape.query('rsquaredFit > 0.08')

#Select AC and ATh cells
dataframeAC = goodFit.query("brainArea == 'rightAC'")
dataframeATh = goodFit.query("brainArea == 'rightThal'")

acUntaggedCellsAM = dataframeAC[dataframeAC['taggedCond']==2]
thalUntaggedCellsAM = dataframeATh[dataframeATh['taggedCond']==2]

#Calculate the midpoint of the gaussian fit
goodFit['fitMidPoint'] = np.sqrt(goodFit['upperFreq']*goodFit['lowerFreq'])
goodFitToUseForFreq = goodFit.query('fitMidPoint<32000')

acUntaggedCellsFreq = goodFitToUseForFreq[goodFitToUseForFreq['taggedCond']==2]
thalUntaggedCellsFreq = goodFitToUseForFreq[goodFitToUseForFreq['taggedCond']==2]
...
## -- BW10 -- ##
feature="BW10"
dataAC = acUntaggedCellsFreq[feature][pd.notnull(acUntaggedCellsFreq[feature])]
dataATh = thalUntaggedCellsFreq[feature][pd.notnull(thalUntaggedCellsFreq[feature])]

ax = axBW10
ax.set_ylabel('BW10')

posAC = jitter(np.ones(len(dataAC))*0, 0.20)
posATh = jitter(np.ones(len(dataATh))*1, 0.20)
boxData = [dataAC, dataATh]

bp = axBW10.boxplot(boxData, widths=0.5, showfliers=False, positions=[0,1,2])

colors = [colorAC, colorATh]
whiskerColors = [colorAC, colorAC, colorATh, colorATh]
linewidth=2
...
ax.set_xticks([0,1])
ax.set_xticklabels(['Cortex\nUntagged\nN={}'.format(len(dataAC)),
                    'Thal\nUntagged\nN={}'.format(len(dataATh))])
...
starGapFactor = 0.1
fontSizeStars = 9
zVal, pVal = stats.mannwhitneyu(dataAC, dataATh)
if pVal < 0.05:
