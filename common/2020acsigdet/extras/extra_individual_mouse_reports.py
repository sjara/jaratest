...
def plot_report(mouse, laserSessions, indType):
    ...
    for band in range(len(numBands)):
        ...
        for laser in range(len(numLasers)):
            ...
            for snr in range(len(numSNRs)):
                ...
                CIthisSNR = np.array(
                    proportion_confint(np.sum(toneChoiceThisCond),
    ...
    if indType == 2:
        plt.suptitle(mouse + ' {}mW {} ({})'.format(ChR2LaserPower, sessionType, mouseLabel[indType]))
        figFilename = f'{mouse}_behav_report_{ChR2LaserPower}mW_{sessionType}'
    else:
        plt.suptitle(mouse + ' {}mW {} ({})'.format(ArchTlaserPower, sessionType, mouseLabel[indType]))
        figFilename = f'{mouse}_behav_report_{ArchTlaserPower}mW_{sessionType}'

    extraplots.save_figure(figFilename, 'png', [9, 4], '/tmp/', facecolor='w')
...
for indType, mice in enumerate(mouseType):
    for mouse in mice:
        ...
        if indType == 2:
            laserSessions = studyparams.miceDict[mouse][f'{ChR2LaserPower}mW {sessionType}']
            plot_report(mouse, laserSessions, indType)
        else:
            try:
                laserSessions = studyparams.miceDict[mouse][f'{ArchTlaserPower}mW {sessionType}']
            except(KeyError):
                pass
            else:
                plot_report(mouse, laserSessions, indType)
