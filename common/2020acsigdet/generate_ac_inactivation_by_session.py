"""
Preprocess datasets of AC inactivation sessions (PV-ChR2 mice).
"""
...
fractionNoChoice = np.zeros((2, len(PV_CHR2_MICE), 2)) # sort by: laser in/out, mouse, laser off/on
...
dprimeEachSession = len(PV_CHR2_MICE)*[[]]  # Create a list with nMice items
eachSessionLabels = [sessionType, ['none','bilateral']]  # WARNING! this is hardcoded

for indMouse, mouse in enumerate(PV_CHR2_MICE):
    dprimeEachSession[indMouse] = len(sessionType)*[[]] # List of session types
    for indSesType, sesType in enumerate(sessionType):
        ...
        for indLaser in range(len(numLasers)):
            fractionNoChoice[indSesType,indMouse,indLaser] = (nochoice[trialsEachLaser[:,indLaser]].sum() /
        ...
        dprimeEachSession[indMouse][indSesType] = np.zeros((len(uniqueSessions),len(numLasers)))
        for indsession, sessionID in enumerate(uniqueSessions):
            for indLaser in range(len(numLasers)):
                ...
                dprimeEachSession[indMouse][indSesType][indsession,indLaser] = (stats.norm.ppf(thisHitRateS) - \
                                                                                stats.norm.ppf(thisFARateS))
                #print((stats.norm.ppf(thisHitRateS) - stats.norm.ppf(thisFARateS)))
                #print(dprimeEachSession)
                #input()
...
# -- save data --
outputFile = 'all_behaviour_ac_inactivation_by_session.npz'
...
np.savez(outputFullPath,
         ...
         expLaserFractionNoChoice=fractionNoChoice[0,:,1], expNoLaserFractionNoChoice=fractionNoChoice[0,:,0],
         controlLaserFractionNoChoice=fractionNoChoice[1,:,1], controlNoLaserFractionNoChoice=fractionNoChoice[1,:,0],

         dprimeEachSession=dprimeEachSession, eachSessionLabels=eachSessionLabels)
print(outputFullPath + " saved")
