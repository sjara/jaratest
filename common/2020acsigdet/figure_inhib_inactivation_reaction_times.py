...
outputDir = '/tmp/'
if CORRECTED:
    figFilename = 'Fig5_inhib_inactivation_reaction_times_corrected'  # Do not include extension
else:
    figFilename = 'Fig5_inhib_inactivation_reaction_times'
...
if PANELS[0]:
    ...
    for type in range(len(laserReactions)):
        ...
        # -- compute median effect size (% change, change in ms)
        changeReaction = np.median(laserReaction - controlReaction)
        meanReaction = np.median(controlReaction)

        print(
            f'{xTickLabels[type]} median change in reaction times: {changeReaction} ms, {100 * changeReaction / meanReaction}% change')
...
if PANELS[2]:
    ...
    for type in range(len(laserDecisions)):
        ...
        # -- compute median effect size (% change, change in ms)
        changeDecision = np.median(laserDecision - controlDecision)
        medianDecision = np.median(controlDecision)

        print(f'{xTickLabels[type]} median change in decision times: {changeDecision} ms, {100 * changeDecision / medianDecision}% change')
