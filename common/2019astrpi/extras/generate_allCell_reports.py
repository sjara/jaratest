...
# FIXME: THe 3 lines below seem to specify creating a database so should be optional if a database already exists
subject = studyparams.SINGLE_MOUSE
studyname = studyparams.STUDY_NAME
celldb = celldatabase.generate_cell_database_from_subjects(subject)
outputDir = os.path.join(settings.FIGURES_DATA_PATH, studyname, 'reports/')

answer = input_func('if there\'s any duplicated sessions, do you want to use the first one[0] \
...
for indRow, dbRow in celldb.iterrows():
    ...
    for sessiontype in sessionsOrig:
        ...
        sessionIndToUse = sessionInds[0] if answer == '0' else (sessionInds[-1] if answer == '1' else sessionInds[randomchoice])  # FIXME
    ...
    for i, con in enumerate(sessions):
        ...
        # ------------------------------- Plot PSTH  --
            ...
            pPSTH = extraplots.plot_psth(spkMat[i]/binWidth, smoothWinSizePsth, timeVec, trialsEachCond=[],
    ...
    fig.savefig(outputDir + '[c#%s] %s_%s_tetrode%s_cluster%s.png' % (dbRow.name, dbRow['subject'], dbRow['depth'], tetnum, chanum))
    plt.clf()
    countreport += 1
