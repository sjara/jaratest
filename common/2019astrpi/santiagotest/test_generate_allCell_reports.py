...
'''
BENCHMARK:
500 reports (only noise+laser) in 350 s
4400 (all) report 6642 s (110 minutes)
'''
...
#nameDB = "ttDBR2.h5"
#nameDB = 'tempdb_laser_and_noise_original.h5'
nameDB = 'tempdb_full_original.h5'
print(f'Loading {nameDB}...')
...
for indRow, dbRow in celldb[0:].iterrows():
    ...
    title = '[{5}]{0}, {1}, {2}um, T{3}c{4}, session ={6}'.format(
        ...
        dbRow.name, sessions)
    plt.suptitle(title, fontsize=15, fontweight='bold') #fontname="Times New Roman Bold")
    fig.set_size_inches([20, 10])
    # pathtoPng = os.path.join(outputDir, 'cellreport/')
    pathToFig = os.path.join(outputDir,'cell{0:04d}_{5}_{1}_{2}_tetrode{3}_cluster{4}.png'.format(
        dbRow.name, dbRow['subject'], dbRow['depth'], tetnum, chanum, dbRow.date))
    #fig.savefig(outputDir + 'cell{0:04d}_{5}_{1}_{2}_tetrode{3}_cluster{4}.png'.format(
    #    dbRow.name, dbRow['subject'], dbRow['depth'], tetnum, chanum, dbRow.date))
    fig.savefig(pathToFig)
    plt.clf()
    countreport += 1
    print(f'Saved report {pathToFig}')
