...
if NO_TAG == 1:
    ...
    outputDirectory = os.path.join(settings.CELL_REPORTS_PATH, studyname, '{}_cells_D1andnD1Only\\'.format(subjects))
else:
    inputDirectory = os.path.join(settings.DATABASE_PATH, studyparams.STUDY_NAME, 
...
D1db = db.query(studyparams.nD1_CELLS)
...
for indRow, dbRow in db.iterrows():
    ...
    if "laserpulse" in sessions:
        ...
        if dbRow.laserpulsePval<studyparams.laserpulse_pVal_threshold and dbRow.laserpulseFRChange>0 and \
            dbRow.laserpulseResponseFR>studyparams.laserpulse_responseCount_threshold:
            plt.title("Laserpulse D1\n{}".format(dbRow.laserpulsePval), fontweight='bold')
        elif dbRow.laserpulsePval>studyparams.laserpulse_pVal_threshold:
            plt.title("Laserpulse nD1\n{}".format(dbRow.laserpulsePval))
        else:
            continue
            print("Cell {} neither D1 or nD1, report not generated".format(countreport))
