    ...
    for indIter, (indRow, dbRow) in enumerate(firstCells.iterrows()):
        ...
        for session in sessions:
            ...
            if session in sessionSingle: # single stimulus
                ...
                # if respSpikeMean > 2 or respSpikeMean < 0.5:
                try:
                ...
                firstCells.at[indRow,'{}_FR'.format(session)] = respSpikeMean#mean firing rate
            ...
            #PURE TONE RESPONSES and AM
                ...
                # [6] Find Frequency Response Area (FRA) unit: fra boolean set, yes or no, but it's originally pair
...
if __name__ == "__main__":
    # Cluster your data
    CLUSTER_DATA = 0  #Decide # of subjects to cluster, strongly think it has to be one at a time
    d1mice = studyparams.ASTR_D1_CHR2_MICE[1]
    if CLUSTER_DATA: #SPIKE SORTING
        inforecFile = os.path.join(settings.INFOREC_PATH,'TEST{}_inforec.py'.format(d1mice))
    ...
    inforecFile = os.path.join(settings.INFOREC_PATH,'TEST{}_inforec.py'.format(d1mice))
    basicDB = celldatabase.generate_cell_database(inforecFile)
