...
if PANELS[0]:
    ...
    for indCell, cell in enumerate(cellData):
        axRaster = plt.subplot(gs[indCell:indCell+1,0])
        plt.cla()
...
if PANELS[1]:
    ...
    for indType, cellTypeData in enumerate(spikeWidthData):
        axHist = plt.subplot(gs[indType:indType+1,1])
        ...
        axHist.annotate(panelLabels[indType], xy=(labelPosX[1],labelPosY[indType]), xycoords='figure fraction',
                         fontsize=fontSizePanel, fontweight='bold')
