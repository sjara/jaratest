...
if PANELS[0]:
    ...
    for indCell, cell in enumerate(exampleCells):
        ...
        for laser in laserTrials:
            axRaster = plt.subplot(gs[laser+2*indCell,0])
...
if PANELS[1]:
    ...
    for indCell, cell in enumerate(exampleCells):
        ...
        lines = []
        axCurve = plt.subplot(gs[2*indCell:2*indCell+2,1])
        plt.hold(1)
        for laser in laserTrials:
            thisResponse = sustainedResponseArray[:,laser].flatten()
            thisSTD = sustainedSTD[:,laser].flatten()
        ...
        axCurve.annotate(panelLabels[indCell], xy=(labelPosX[indCell+1],labelPosY[0]), xycoords='figure fraction',
                         fontsize=fontSizePanel, fontweight='bold')
        axCurve.set_ylim(bottom=0)
        axCurve.set_xlim(left=-0.3)
        yLims = np.array(plt.ylim())
        plt.legend(lines,['Control', figLegends[indCell]], loc='best', frameon=False)
