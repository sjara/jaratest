...
pfa.barplot_accu_rewards_time(data_filtered_grouped, len(data.index.get_level_values(0).unique().tolist()))
...
## RUN
data_filtered_grouped_2 = lbd.filter_and_group(bins=3, data=data_2, sessionLen=60)
    
pfa.barplot_accu_rewards_time(data_filtered_grouped, len(data_2.index.get_level_values(0).unique().tolist()))
