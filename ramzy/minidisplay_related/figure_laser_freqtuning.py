...
def plot_stim(yLims, stimDuration, stimOnset=0,stimLineWidth=6, stimColor=figparams.colorStim,alpha=1):
    yPos = 0.96*yLims[-1] + 0.075*(yLims[-1]-yLims[0])

    pstim = plt.plot([stimOnset, stimDuration], 2*[yPos], lw=stimLineWidth, color=stimColor,
                     clip_on=False, solid_capstyle='butt',alpha=alpha)
    return pstim[0]
...
dbFilename = os.path.join(dbPath,f'celldb_{subject}_laserfreqtuning.h5')
...
goodFit = studyutils.find_good_dprime(celldb,eventKey=eventKey, minDisc=studyparams.MIN_DPRIME,
...
posResponse = (celldb['offToneGaussianA'+eventKey]>0) #& (celldb['offToneFiringRateBestFreq'+eventKey] > 2)
...
if not justCurves:
    ...
    metrics = ['ToneMeanDiscrim'+eventKey, 'ToneGaussianBandwidth'+eventKey, 'ToneGaussianMaxChange'+eventKey]
    metricsLabel = ["d''", 'Width', 'Max Δ']
    #metricsLabel = ['BF', 'Bandwidth', 'Resp at BF']
    # metricsUnits = ['kHz', 'norm.', 'spk/s']
    metricsUnits = ['AU', 'norm.', 'spk/s']
    # metricLims = [[np.log2(2000), np.log2(40000)], [0, 6], [0, 10]]
    # metricTicks = [[2000, 8000, 32000], [0, 5, 10], [0, 25, 50]]
    metricLims = [[0, 2.5], [0, 6], [0, 10]]
    metricTicks = [[0, 1.5, 2.5], [0, 5, 10], [0, 25, 50]]
    ...
    for indcell, cellInd in enumerate(cellsToPlot):
        ...
        for indr, reagent in enumerate(reagentsToPlot):
            ...
            colorEachCond = [colorsRasterDark[sessionType][reagent], colorsRasterLight[sessionType][reagent]]*(nStim//2+1)
            ...
            if indr==0:
                ...
                plt.axvspan(xmin=-bdata['laserFrontOverhang'][0],xmax=stimDuration,
                            color=colorsRasterDark[sessionType]['on'],alpha=0.6)

            if 'off' in reagent:

                firingRatesOff = dbRow[f'off'+'ToneFiringRateEachFreq'+eventKey]
                firingRatesOn = dbRow[f'on'+'ToneFiringRateEachFreq'+eventKey]
                ...
                plt.plot(firingRatesOff,firingRatesOn,'.', color=colorsRasterDark[sessionType][reagent],label='Laser OFF')
                plt.plot(xfit,yfit,'-',color = colorsRasterDark[sessionType][f"on"],label='Laser ON')
            ...
            if cellInd < nCells:
                ...
                for indr, reagent in enumerate(reagentsToPlot):
                    ...
                    if 'off' in reagent:
                        ...
                        plt.plot(firingRatesOff,firingRatesOn,'.', color=colorsRasterDark[sessionType][reagent],label=reagent)
                        plt.plot(xfit,yfit,'-',color = colorsRasterDark[sessionType][f"off"])
                        plt.plot(xfit,xfit,'--',color='k')
