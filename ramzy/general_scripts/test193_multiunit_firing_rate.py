...
# ephysSession = '2025-08-07_18-11-51'; behavSession = '20250807e' # shank 2 bank A parallel optoAM
...
if paradigm == 'am_image_tuning':
    stimEachTrial = np.array([f"C{i}R{j}" if i+j>=0 else 'off' for i,j \
                            in zip(bdata['currentStimCol'],bdata['currentStimRow'])])
    
else: 
    stimEachTrial = list(bdata['currentFreq'])
    if 0:
        stimEachTrial = bdata['laserTrial']
    
    if 1:
        
        for i in range(len(stimEachTrial)):
            stimEachTrial[i] = f"{stimEachTrial[i]} Hz"

            if bdata['currentIntensity'][i]<0:
                stimEachTrial[i] += ' No Sound'

            if bdata['laserTrial'][i]==1:
                stimEachTrial[i] += ' Laser ON'
            else:
                stimEachTrial[i] += ' Laser OFF'

    stimEachTrial = np.array(stimEachTrial)
...
if not DEBUG:
    ...
    sortedChannels = sorted(list(np.arange(0,nChannels)),
                        key = lambda x: probeMap.ypos[chanOrder[x]]*(10**probeMap.channelShank[chanOrder[x]]))
    ...
    if 1:
        ...
        for indstim,stim in enumerate(sortedStims):
            ...
            plt.subplot(4,int(np.ceil(len(possibleStim)/4)),indstim+1)
            ax = plt.gca()
            # plt.imshow(chanAvgPSTHs[:,indstim,:])
            stimAdj = -1 if 'ON' in stim else 0
            plt.imshow(chanAvgPSTHs[:,indstim,:],
                vmin = np.percentile(chanAvgPSTHs[:,indstim+stimAdj,:],1),
                vmax = np.percentile(chanAvgPSTHs[:,indstim+stimAdj,:],99))

            # plt.imshow(chanAvgPSTHs[:,indstim,:],
            #     vmin = np.percentile(chanAvgPSTHs[:,0,:],1),
            #     vmax = np.percentile(chanAvgPSTHs[:,0,:],90))
