        ...
        for count, indcell in enumerate(cellInds[page*cellsPerPage:(page+1)*cellsPerPage]):
            if len(paradigms) ==2:
                for row,paradigm in enumerate(paradigms):
                    ...
                    # nCond = trialsEachCond[paradigm].shape[1]
                    # cmap = plt.get_cmap('Dark2',8)
                    # colorEachCond = [cmap(1+i%2) for i in range(nCond*trialsEachCond[paradigm].shape[2])]

                    # plt.plot(spikeTimesFromEventOnsetAll[paradigm][indcell],trialIndexForEachSpikeAll[paradigm][indcell],
                    #          '.k',ms=3)
                    ...
                    pRaster, hcond, zline = extraplots.raster_plot(spikeTimesFromEventOnsetAll[paradigm][indcell], 
                                                                   ...
                                                                   rasterTimeRange[paradigm], 
                                                                   np.reshape(trialsEachCond[paradigm],(trialsEachCond[paradigm].shape[0],np.prod(trialsEachCond[paradigm].shape[1:])),'F'), 
