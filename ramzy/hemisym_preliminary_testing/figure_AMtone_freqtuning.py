...
if eventKey == 'BTR':
    ...
    bestKeys = studyutils.find_best_time_keys(celldb,
                                              BTRmetric,['Onset','Sustained','Offset','Interim'],
...
posResponse = np.zeros(len(celldb))
for mod in modRates:
...
selectedCells = goodD & selective #& RScell
# selectedCells = responsive  & goodFit 
# selectedCells = steady & goodFit & posResponse & anyFitall
...
reagentsToRaster = []
for indc,dbRow in celldb.iterrows():
    thisGauss = sorted(reagents, key=lambda x: dbRow[x+'ToneGaussianA'+eventKey])
    reagentsToRaster.append((thisGauss[0],thisGauss[-1]))
...
if not justCurves:
    ...
    for indcell, cellInd in enumerate(cellsToPlot):
        ...
        nTrialsAll = len(modEachTrial)

        if 'opto' in sessionType:
            laserEachTrial = bdata['laserTrial']
            reagentEachTrial = np.array([f'{int(modEachTrial[i])}Hz{'ON' if laserEachTrial[i] else 'OFF'}' for i in range(nTrialsAll)])
        else:
            reagentEachTrial = modEachTrial
        ...
        for indr, reagent in enumerate(reagents):
            prefix = reagent
            trialInds = (reagentEachTrial == reagent)
            ...
            if eventKey == 'BTR':
                plt.title(dbRow[reagent+'BestTimeRange'])
    ...
    for indm, metric in enumerate(metrics):
        ...
        thisMetricoff = celldb[str(reagents[0])+metric][selectedCells]
        thisMetricEachMod = np.array([celldb[mod+metric][selectedCells] for mod in reagents])
        changeEachMod = np.array([metricThisMod - thisMetricoff for metricThisMod in thisMetricEachMod])
        for indt, modMetric in enumerate(thisMetricEachMod):
            mod = reagents[indt]
            thisMetricon = modMetric
            ...
            if reagents[indt] != 'off':
                print(f'[N={nCells}]  Median {mod}Hz,{metric}: off={medianoff:0.4f}, ' +
        ...
        thisAx = plt.subplot(gsMetrics[indm+1])
        violins = plt.violinplot(thisMetricEachMod.T,showmedians=True)
        ...
        kstat, pVal_KRUSK = stats.kruskal(*thisMetricEachMod)
        # kstat, pVal_KRUSK = stats.kruskal(*changeEachTile)
        resultDunn = stats.dunnett(*list(thisMetricEachMod[:-1]),control=thisMetricEachMod[-1],alternative='two-sided')
    ...
    for pageNum, cellsToPlot in enumerate(cellInds):
        ...
        for indcell,cellInd in enumerate(cellsToPlot):
            if cellInd < nCells:
                ...
                ephysData, bdata = oneCell.load(sessionType)  
                modEachTrial = bdata['currentMod']
                possibleTile = np.unique(modEachTrial)
                nTrialsAll = len(modEachTrial)
                eventOnsetTimesAll = ephysData['events']['stimOn'][:nTrialsAll]
                stimEachTrialAll = bdata['currentFreq']
