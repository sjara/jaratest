...
if not sessionTypes:
    sessionTypes = [i for i in np.unique(np.concat(celldb.sessionType)) if i[:8]=='poniFreq']

if trialSubset == '':
    outputFilename = os.path.join(dbPath, f'celldb_{subject}_ponifreqtuning.h5')
else:
    outputFilename = os.path.join(dbPath, f'celldb_{subject}_ponifreqtuning_{trialSubset}.h5')
