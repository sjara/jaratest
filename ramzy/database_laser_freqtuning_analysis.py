...
if trialSubset == '':
    outputFilename = os.path.join(dbPath, f'celldb_{subject}_freqtuning.h5')
else:
...
if DEBUG:
    ...
    celldbToUse = celldb[(celldb['sessionType'].apply(lambda x: 'Freq' in x))].iloc[[indRow]]
else:
    celldbToUse = celldb[(celldb['sessionType'].apply(lambda x: 'Freq' in x))]
