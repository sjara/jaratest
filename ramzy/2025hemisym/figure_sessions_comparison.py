...
studyDir = os.path.join(settings.FIGURES_DATA_PATH,studyName)
if SAVE_FIGURE and not os.path.exists(studyDir):
    os.mkdir(studyDir)
...
if 'Freq' in sessionType:
    gsMain = gridspec.GridSpec(2, 4, width_ratios=[0.00001, 0.2, 0.2, 0.4])
    gsMain.update(left=0.06, right=0.98, top=0.95, bottom=0.1, wspace=0.4, hspace=0.6)
else:
        ...
        for indr,mod in enumerate(modRates):
            ...
            for indo,oct in enumerate(octaves):
                if octRange[indo]:
                    ...
                    meanoff = np.nanmedian(thisMetricoff,axis=0)
                    stdoff = np.nanstd(thisMetricoff,axis=0)
                    meanon = np.nanmedian(thisMetricon,axis=0)
...
if BAR_CHARTS and 'AMtone' in sessionType:
    plt.clf()
    metricDiffsAll = {
    ...
    for implant in celldbEachImplant:
        ...
        for pageNum, cellsToPlot in enumerate(cellInds):
            ...
            for indcell, cellInd in enumerate(cellsToPlot):
                if cellInd < nCells:
                    ...
                    for indmod,mod in enumerate(possibleMod):
                        ...
                        #axRaster.set_yticklabels(['2']+['']*(nFreq-2)+['40'])
                        if indmod==0:
                            plot_stim(plt.ylim(), stimDuration, yposOffset=0.6)
                            plot_stim(plt.ylim(),stimDuration,yposOffset=2.1,xposOffset=-0.25,stimColor=colorsRasterDark[sessionType]['off'])
                            plt.title(f'Cell #{cellIDs[cellInd]:04d}, {dbRow.subject}, {dbRow.pdepth}μm, {dbRow.date}',loc='left')
                            axRaster.set_xticklabels([])
            ...
            if SAVE_FIGURE:
                ...
                extraplots.save_figure(f'{pageNum}_'+figFilename+'_Cell_Reports'+cellstring, 
                                    figFormat, [figSize[0]*1.6,figSize[1]*1.4], outputDirNew,transparent=False)
