...
class ComparisonConfig:
    ...
    def __post_init__(self):
        ...
        # Set data directories based on tuning type using config.py
        if self.tuning_type == 'frequency':
            object.__setattr__(self, 'ac_pstr_dir', 
                str(get_reports_subdir('tuning_freq_gaussian_fits')))
            object.__setattr__(self, 'th_pstr_dir',
                str(get_reports_subdir('arch024/tuning_freq_gaussian_fits')))
            object.__setattr__(self, 'output_dir',
                str(get_reports_subdir('AC_vs_Th_gaussian_comparison_freq')))
        elif self.tuning_type == 'am':
            object.__setattr__(self, 'ac_pstr_dir',
                str(get_reports_subdir('tuning_AM_gaussian_fits')))
            object.__setattr__(self, 'th_pstr_dir',
                str(get_reports_subdir('arch024/tuning_AM_gaussian_fits')))
            object.__setattr__(self, 'output_dir',
                str(get_reports_subdir('AC_vs_Th_gaussian_comparison_AM')))
...
class PathwayVisualizer:
    ...
    def create_basic_comparison_plot(
        ...
        ax.set_title(f'{metric_label}: Laser OFF vs ON Comparison\nAC-pStr vs Th-pStr Pathways ({tuning_label} - {gaussian_type})',
