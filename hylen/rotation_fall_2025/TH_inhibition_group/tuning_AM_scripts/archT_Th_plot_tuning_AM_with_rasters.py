...
class VisualizationConfig:
    ...
    response_window: Tuple[float, float] = (0.005, 0.495)  # Longer for AM (full modulation cycles)
...
class CellMetrics(NamedTuple):
    ...
    cell_idx: int
    best_rate: float  # Changed from best_freq to best_rate (AM rate in Hz)
    peak_response: float  # Changed from peak_rate to peak_response
    mean_response: float  # Changed from mean_rate to mean_response
    selectivity_index: float
    sparseness: float
    tuning_quality: float
    bandwidth_octaves: float
    bandwidth_hz: float  # Added: AM bandwidth in Hz
    q_factor: float  # Added: Q-factor
    stimulus_evoked_change: float  # Stimulus-evoked change from baseline
    tuning_category: str


# All available sessions (AM tuning sessions)
SESSIONS = {
...
class DataLoader:
    ...
    def load_session_data(
        ...
        ephys_data, bdata = ensemble.load('optoTuningAM')  # Changed to AM
        
        # Find the correct field name for AM rates (same as calculate_AM_tuning.py)
...
class TuningCurveComputer:
    ...
    def compute_tuning_curves(
        ...
        Returns:
            Tuple of (am_rates, rates_off, sem_off, rates_on, sem_on)
        """
        # Get AM rate field dynamically
        rate_field = getattr(bdata, '_am_rate_field', 'currentFreq')
        am_rates = bdata[rate_field]  # AM rates from behavioral data
        laser_trials = bdata.get('laserTrial', np.zeros(len(am_rates), dtype=bool))
        ...
        for rate in unique_rates:
            # Laser OFF
            off_trials = np.where((am_rates == rate) & (laser_trials == 0))[0]
            if len(off_trials) > 0:
            ...
            # Laser ON
            on_trials = np.where((am_rates == rate) & (laser_trials == 1))[0]
            if len(on_trials) > 0:
        ...
        return unique_rates, np.array(rates_off), np.array(sem_off), np.array(rates_on), np.array(sem_on)
...
class CombinedPlotter:
    ...
    def _plot_tuning_curve(
        ...
        am_rates, rates_off, sem_off, rates_on, sem_on = self.tuning_computer.compute_tuning_curves(
        ...
        # Plot (AM rates, not frequencies)
        x_pos = np.arange(len(am_rates))
        ...
        # Mark best AM rates (stars)
        if metrics_off.best_rate in am_rates:
            br_idx = np.where(am_rates == metrics_off.best_rate)[0][0]
            ax.plot(x_pos[br_idx], rates_off[br_idx],
                   marker='*', markersize=12, color='blue',
                   markeredgecolor='black', markeredgewidth=1, zorder=10)
        
        if metrics_on and metrics_on.best_rate in am_rates:
            br_idx = np.where(am_rates == metrics_on.best_rate)[0][0]
            ax.plot(x_pos[br_idx], rates_on[br_idx],
                   marker='*', markersize=12, color='red',
                   markeredgecolor='black', markeredgewidth=1, zorder=10)
        
        # Format
        ax.set_ylabel('Firing Rate (Hz)', fontsize=9)
        ax.set_xlabel('AM Rate (Hz)', fontsize=9)
        ax.set_xticks(x_pos)
        ax.set_xticklabels([f'{int(r)}' for r in am_rates], rotation=45, ha='right', fontsize=7)
        ax.set_xlim([-0.5, len(am_rates) - 0.5])
    ...
    def _add_metrics_text(
        ...
        text_off = (
            ...
            f"Evoked: {metrics_off.stimulus_evoked_change*100:.0f}%"
        )
        ...
        if metrics_on is not None:
            text_on = (
                ...
                f"Evoked: {metrics_on.stimulus_evoked_change*100:.0f}%"
            )
        ...
        if metrics_on is not None:
            ...
            delta_evoked = metrics_on.stimulus_evoked_change - metrics_off.stimulus_evoked_change
    ...
    def _plot_raster(
        ...
        """Plot raster sorted by AM rate with laser OFF/ON comparison (IDENTICAL to arch018 raster script)."""
        metrics_off = cell_data['metrics_off']
        cell_idx = metrics_off.cell_idx
        ...
        # Get AM rates dynamically
        rate_field = getattr(session_data['bdata'], '_am_rate_field', 'currentFreq')
        am_rates = session_data['bdata'][rate_field]
        laser_trials = session_data['bdata'].get('laserTrial', np.zeros(len(am_rates), dtype=bool))
        
        # Get unique AM rates and trial organization
        unique_rates = np.unique(am_rates)
        ...
        # Build trialsEachCond with laser grouping: [rate1_OFF, rate2_OFF, ..., rate1_ON, rate2_ON, ...]
        ...
        # Generate alternating colors for AM rates
        ...
        # Highlight the response window with a subtle background
        ax.axvspan(self.config.response_window[0], self.config.response_window[1],
                  facecolor='lightblue', alpha=0.1, zorder=0, label='Analysis window')
        ...
        n_on = len(on_trials)
        cumsum_trials = np.cumsum([trials_each_cond_laser[:, i].sum() for i in range(n_rates)])
        ...
        ax.set_ylabel('AM Rate (Hz)', fontsize=9)
...
class PDFGenerator:
    ...
    def _create_combined_page(
        ...
        gs = GridSpec(2, 3, figure=fig, height_ratios=[1, 1.5], 
                     hspace=0.15, wspace=0.3, top=0.88, bottom=0.08)
        ...
        fig.suptitle(
            f'Th-pStr Inhibition (ArchT) - AM Tuning - Raw Tuning Curves with Rasters\n'
            f'All Sessions | {category_display} Cells | Page {page_idx + 1}/{n_pages}',
    ...
    def _load_session_data(
        ...
        # Get number of trials from AM rate field
        rate_field = getattr(bdata, '_am_rate_field', 'currentFreq')
    ...
    def _organize_cells(self, df_merged: pd.DataFrame) -> Dict[str, List[dict]]:
        ...
        for _, row in df_merged.iterrows():
            ...
            metrics_off = CellMetrics(
                ...
                bandwidth_hz=row['bandwidth_hz_off'],
                q_factor=row['q_factor_off'],
                stimulus_evoked_change=row.get('stimulus_evoked_change_off', np.nan),
                tuning_category=category
            )
            
            metrics_on = None
            if not pd.isna(row['peak_response_on']):
                metrics_on = CellMetrics(
                    ...
                    bandwidth_hz=row['bandwidth_hz_on'],
                    q_factor=row['q_factor_on'],
                    stimulus_evoked_change=row.get('stimulus_evoked_change_on', np.nan),
                    tuning_category=''
                )
