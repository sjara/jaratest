...
class PlotConfig:
    ...
    experiment_name: str = 'Th - pStr Inhibition with AM'

    @property
    def gaussian_fits_dir(self) -> str:
        return str(get_reports_subdir('arch024/tuning_AM_gaussian_fits'))
...
class LogGaussianFitDataLoader:
    ...
    def load_log_gaussian_fits(self, session_id: int, laser_condition: str) -> pd.DataFrame:
        ...
        metrics_path = os.path.join(
            str(get_reports_subdir('arch024/tuning_AM_analysis')),
...
class CategoryPDFGenerator:
    ...
    def create_category_pdfs(self):
        ...
        # Try to load combined metrics as backup (in case individual files missing)
        print("\nChecking for combined metrics...")
        metrics_dir = str(get_reports_subdir('arch024/tuning_AM_analysis'))
        combined_off_path = os.path.join(metrics_dir, 'all_sessions_laser_off_tuning_metrics.csv')
        combined_on_path = os.path.join(metrics_dir, 'all_sessions_laser_on_tuning_metrics.csv')
        
        all_metrics_off = pd.read_csv(combined_off_path) if os.path.exists(combined_off_path) else None
        all_metrics_on = pd.read_csv(combined_on_path) if os.path.exists(combined_on_path) else None
        ...
        for session_id in sorted(SESSIONS.keys()):
            ...
            try:
                ...
                # Load Log-Gaussian fits
                fits_off = self.loader.load_log_gaussian_fits(session_id, 'off')
                fits_on = self.loader.load_log_gaussian_fits(session_id, 'on')
                
                # Try individual session metrics first, fall back to combined
                metrics_off_path = os.path.join(metrics_dir, f'session_{session_id}_laser_off_tuning_metrics.csv')
                
                if os.path.exists(metrics_off_path):
                    metrics_off = pd.read_csv(metrics_off_path)
                    has_tuning_category = 'tuning_category' in metrics_off.columns
                elif all_metrics_off is not None:
                    metrics_off = all_metrics_off[all_metrics_off['session_id'] == session_id].copy()
                    has_tuning_category = 'tuning_category' in metrics_off.columns
                else:
                    metrics_off = None
                    has_tuning_category = False
                ...
                for cell_idx in common_cells:
                    ...
                    # Check if empirically tuned
                    if has_tuning_category and metrics_off is not None:
                        cell_metrics = metrics_off[metrics_off['cell_idx'] == cell_idx]
                        if len(cell_metrics) > 0 and cell_metrics.iloc[0]['tuning_category'] == 'tuned':
    ...
    def create_category_pdfs(self):
        ...
        # Try to load combined metrics as fallback (in case individual files missing)
        print("\nChecking for combined metrics...")
        metrics_dir = str(get_reports_subdir('arch024/tuning_AM_analysis'))
        combined_off_path = os.path.join(metrics_dir, 'all_sessions_laser_off_tuning_metrics.csv')
        combined_on_path = os.path.join(metrics_dir, 'all_sessions_laser_on_tuning_metrics.csv')
