...
class TuningCurveAnalyzer:
    ...
    def compute_tuning_curve(
        ...
        # Compute baseline firing rate (across all trials for this laser condition)
        ...
        baseline_trials = np.where(laser_mask)[0]
        
        baseline_spike_counts = []
        for trial_num in baseline_trials:
...
class CellPopulationAnalyzer:
        ...
        for cell_idx in range(n_cells_total):
            ...
            if is_dead:
                ...
                if metrics_off is not None:
                    result = self._metrics_to_dict(cell_idx, spike_times, metrics_off)
            ...
            result_off = self._metrics_to_dict(cell_idx, spike_times, metrics_off)
            result_off['is_excluded'] = False  # Mark as included
            results_off.append(result_off)
            
            if metrics_on is not None:
                result_on = self._metrics_to_dict(cell_idx, spike_times, metrics_on)
    ...
    def _compute_cell_metrics(
        ...
        return self.metrics_calculator.calculate(am_rates, firing_rates, baseline_rate, fano_factor)
    
    @staticmethod
    def _metrics_to_dict(cell_idx: int, spike_times: np.ndarray, metrics: TuningMetrics) -> dict:
        """Convert TuningMetrics to dictionary for DataFrame."""
        return {
            'cell_idx': cell_idx,
