"""
Analyze Gaussian Parameter Changes: Laser OFF vs ON

This script analyzes how Gaussian tuning parameters change with laser stimulation:
- Baseline (spontaneous firing floor)
- Amplitude (dynamic range)
- Mean (best frequency shift)
- Sigma (tuning width/selectivity)

Focuses on two groups:
1. Good fits: Cells with high-quality Gaussian fits (R² ≥ threshold)
2. Empirically tuned: All cells classified as tuned by empirical metrics
...
class AnalysisConfig:
    ...
    experiment_name: str = 'AC Control Group - Laser Blocked'
    
    # Analysis parameters
    r_squared_threshold: float = 0.5  # For "good fits" group
    ...
    def gaussian_fits_dir(self) -> str:
        return str(get_reports_subdir('control_group/tuning_freq_gaussian_fits'))

    @property
    def output_dir(self) -> str:
        return str(get_reports_subdir('control_group/tuning_freq_gaussian_parameter_analysis'))
...
class GaussianFitLoader:
    """Loads and pairs Gaussian fit data for OFF vs ON comparison."""
    ...
    def load_paired_data(self) -> pd.DataFrame:
        """
        Load Gaussian fits for laser OFF and ON, pair them by cell.
        
        Returns:
            DataFrame with columns for both OFF and ON parameters
        """
        # Load combined OFF and ON data
        off_path = os.path.join(self.config.gaussian_fits_dir, 'all_sessions_laser_off_gaussian_fits.csv')
        on_path = os.path.join(self.config.gaussian_fits_dir, 'all_sessions_laser_on_gaussian_fits.csv')
        ...
        df_paired['delta_mean_octave'] = df_paired['mean_octave_on'] - df_paired['mean_octave_off']
    ...
    def filter_empirically_tuned(self, df: pd.DataFrame) -> pd.DataFrame:
        """Filter to empirically tuned cells."""
        tuned_mask = df['empirical_is_tuned_off'] == True
...
class ParameterChangeAnalyzer:
    ...
    def summarize_changes(self, df: pd.DataFrame, group_name: str) -> pd.DataFrame:
        ...
        params = ['baseline', 'amplitude', 'mean_octave', 'sigma']
...
class ParameterChangePlotter:
    ...
    def create_summary_figure(self, df: pd.DataFrame, group_name: str, output_path: str):
        ...
        params = ['baseline', 'amplitude', 'mean_octave', 'sigma']
        param_labels = ['Baseline (Hz)', 'Amplitude (Hz)', 'BF Shift (octaves)', 'Sigma (octaves)']
...
def main():
    ...
    print("="*70)
    print("GAUSSIAN PARAMETER CHANGE ANALYSIS")
    print("="*70)
    ...
    loader = GaussianFitLoader(config)
    ...
    print(f"  Empirically tuned: {len(df_tuned)} cells")
    
    # Statistical analysis
    print("\nPerforming statistical analysis...")
    summary_good = analyzer.summarize_changes(df_good_fits, 'Good Fits')
    summary_tuned = analyzer.summarize_changes(df_tuned, 'Empirically Tuned')
    
    summary_combined = pd.concat([summary_good, summary_tuned], ignore_index=True)
    
    # Save summary statistics
    summary_path = os.path.join(config.output_dir, 'parameter_change_summary_statistics.csv')
    ...
    # Good fits
    good_fits_path = os.path.join(config.output_dir, 'parameter_changes_good_fits.png')
    plotter.create_summary_figure(df_good_fits, 'Good Fits', good_fits_path)
    
    # Empirically tuned
    tuned_path = os.path.join(config.output_dir, 'parameter_changes_empirically_tuned.png')
    plotter.create_summary_figure(df_tuned, 'Empirically Tuned', tuned_path)
    
    # Bar chart summary comparing both groups
    bar_chart_path = os.path.join(config.output_dir, 'parameter_changes_bar_chart_summary.png')
