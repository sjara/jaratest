...
class AnalysisConfig:
    ...
    min_effect_size_cohens_d: float = 0.3
...
class StatisticalAnalyzer:
    ...
    def analyze_all_metrics(
        ...
        # Add optional metrics only if they exist in the DataFrame
...
class ComparisonPlotter:
    ...
    def create_comparison_pdf(
        ...
        with PdfPages(output_path) as pdf:
            ...
            with figure_context(figsize=(17, 11)) as fig:
                fig.suptitle(
                    f'Frequency Tuning: Laser OFF vs ON Comparison (Scatter Plots)\n'
            ...
            with figure_context(figsize=(17, 11)) as fig:
                fig.suptitle(
                    f'Frequency Tuning: Laser OFF vs ON Comparison (Bar Plots)\n'
                    f'Tuned Cells Only (n = {n_cells} cells)',
                    fontsize=16, fontweight='bold'
                )
            ...
            if len(stats_results) > 6:
                with figure_context(figsize=(17, 11)) as fig:
                    fig.suptitle(
                        f'Frequency Tuning: Laser OFF vs ON - Additional Metrics\n'
                        f'Tuned Cells Only (n = {n_cells} cells)',
                        fontsize=16, fontweight='bold'
                    )
            ...
            if len(stats_results) > 6:
                with figure_context(figsize=(17, 11)) as fig:
                    fig.suptitle(
                        f'Frequency Tuning: Laser OFF vs ON - Additional Metrics (Bar Plots)\n'
                        f'Tuned Cells Only (n = {n_cells} cells)',
                        fontsize=16, fontweight='bold'
                    )
    ...
    def create_significance_summary_figure(
        ...
        with figure_context(output_path, figsize=(16, 6)) as fig:
            ...
            fig.suptitle('Biological Significance Analysis: Laser Effects on Frequency Tuning\n'
