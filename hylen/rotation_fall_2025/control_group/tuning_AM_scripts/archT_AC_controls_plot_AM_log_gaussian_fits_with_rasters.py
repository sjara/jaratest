...
class LogGaussianFitDataLoader:
    ...
    def load_log_gaussian_fits(self, session_id: int, laser_condition: str) -> pd.DataFrame:
        ...
        metrics_path = os.path.join(
            '/data/reports/control_group/tuning_AM_analysis',
...
class LogGaussianAMPlotter:
    ...
    def _plot_metrics_box(
        ...
        text_off = (
            ...
            f"Pref Rate: {fit_off['preferred_rate']:.1f}Hz\n"
        ...
        text_on = (
            ...
            f"Pref Rate: {fit_on['preferred_rate']:.1f}Hz\n"
        ...
        text_effect = (
            ...
            f"ΔPref Rate: {fit_on['preferred_rate'] - fit_off['preferred_rate']:+.1f}Hz\n"
            f"ΔSigma: {fit_on['sigma'] - fit_off['sigma']:+.2f}log₁₀"
        )
        
        # Position text boxes
        ax.text(0.02, 0.5, text_off, transform=ax.transAxes,
               fontsize=9, verticalalignment='center', family='monospace',
               bbox=bbox_props)
        
        ax.text(0.35, 0.5, text_on, transform=ax.transAxes,
               fontsize=9, verticalalignment='center', family='monospace',
               bbox=bbox_props)
        
        ax.text(0.68, 0.5, text_effect, transform=ax.transAxes,
               fontsize=9, verticalalignment='center', family='monospace',
               bbox=bbox_props)
    ...
    def create_category_pdfs(self):
        ...
        for session_id in sorted(SESSIONS.keys()):
            ...
            try:
                ...
                # Also load metrics to get tuning classification
                metrics_off_path = os.path.join(
                    '/data/reports/control_group/tuning_AM_analysis',
                ...
                if os.path.exists(metrics_off_path):
                    ...
                    # Check if tuning_category column exists (used for classification)
                ...
                for cell_idx in common_cells:
                    ...
                    # Check if empirically tuned (from metrics classification)
                    if has_tuning_category and metrics_off is not None:
                        # Find this cell in metrics
                        cell_metrics = metrics_off[metrics_off['cell_idx'] == cell_idx]
                        if len(cell_metrics) > 0:
                            tuning_cat = cell_metrics.iloc[0]['tuning_category']
                            # Include if tuned (not excluded or untuned)
                            if tuning_cat == 'tuned':
    ...
    def _create_pdf(self, cell_data_list: List, filename_suffix: str, title: str):
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            for page_idx in range(n_pages):
                ...
                # Page title
                fig.suptitle(
                    f'{title} | Page {page_idx + 1}/{n_pages} | Total: {len(cell_data_list)} cells',
