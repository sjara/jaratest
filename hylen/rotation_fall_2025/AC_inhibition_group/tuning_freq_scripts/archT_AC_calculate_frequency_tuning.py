...
# Add hylen directory to path to import config
...
# Analysis subdirectory name (script-specific)
ANALYSIS_SUBDIR = 'tuning_freq_analysis'
...
class AnalysisConfig:
    ...
    baseline_window: Tuple[float, float] = (-0.095, -0.005)  # NEW: Pre-stimulus baseline
    response_window: Tuple[float, float] = (0.005, 0.095)  # Exclude laser artifact
    ...
    tuning_quality_threshold: float = 0.2  # Minimum tuning quality to consider valid
    min_cv_threshold: float = 0.425  # Coefficient of variation - reject flat curves
    max_bandwidth_threshold: float = 3.0  # Maximum bandwidth (octaves) - reject very broad tuning
    
    # Use centralized configuration for directories
    @property
    def output_dir(self) -> str:
        """Get output directory from centralized config."""
        return str(get_reports_subdir(ANALYSIS_SUBDIR))
    
    @property
    def override_file(self) -> str:
        """Get override file path from centralized config."""
...
class TuningMetricsCalculator:
    ...
    def _calculate_bandwidth(
        ...
        Args:
            ...
            rates: Firing rates at each frequency
...
def main():
    ...
    # Verify output directory (good practice)
