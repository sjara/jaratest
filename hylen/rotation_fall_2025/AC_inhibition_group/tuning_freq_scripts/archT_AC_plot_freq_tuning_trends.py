...
class VisualizationConfig:
    ...
    def output_dir(self) -> str:
        ...
        return os.path.join(self.metrics_dir, 'tuning_with_rasters')
...
class DataLoader:
    ...
    def _calculate_laser_effects(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Mean firing rate percent change
        df['percent_fr_change'] = ((df['mean_rate_on'] - df['mean_rate_off']) / 
                                   df['mean_rate_off'] * 100)
        
        # Peak firing rate percent change  
        df['percent_peak_change'] = ((df['peak_rate_on'] - df['peak_rate_off']) / 
                                     df['peak_rate_off'] * 100)
...
class CombinedVisualizationCoordinator:
    ...
    def __init__(self, config: VisualizationConfig):
        ...
        self.output_dir = config.output_dir  # Use config property instead of hardcoded path
    ...
    def _organize_cells(self, df_merged: pd.DataFrame) -> Dict[str, List[dict]]:
        ...
        # Create category distribution bar chart (tuning quality-based)
        self._create_category_distribution_chart(df_merged, categories)
        
        # NEW: Create selectivity index-based distribution chart
        self._create_si_distribution_chart(df_merged)
    ...
    def _create_si_distribution_chart(self, df_merged: pd.DataFrame):
        """
        Create bar chart showing distribution based on selectivity index changes AND firing rate changes.
        
        Classification (6 categories matching TQ-based structure):
        - Increase SI: ΔSI ≥ +0.05
        - Decrease SI: ΔSI ≤ -0.05
        - No Change SI: |ΔSI| < 0.05
        - Increase FR: ΔFR ≥ +10%
        - Decrease FR: ΔFR ≤ -10%
        - No Change FR: |ΔFR| < 10%
        
        Two-panel figure:
        - Left: Single category distribution (6 bars)
        - Right: FR distribution WITHIN cells that show increased SI
        """
        print("\n" + "="*70)
        print("SELECTIVITY INDEX & FIRING RATE CHANGE DISTRIBUTION")
        print("="*70)
        ...
        # Calculate delta SI and delta FR for each cell
        valid_cells = valid_cells.copy()
        valid_cells['delta_SI'] = (
            valid_cells['selectivity_index_on'] - valid_cells['selectivity_index_off']
        )
        
        # Remove cells with NaN delta_SI
        valid_cells = valid_cells[~valid_cells['delta_SI'].isna()]
        
        if len(valid_cells) == 0:
            print("  No cells with valid ΔSI data")
            return
        
        # Thresholds
        si_threshold = 0.05
        fr_threshold = 10.0  # percent
        
        # Classify cells into 6 categories (SI and FR)
        # Initialize category columns
        valid_cells['increase_si_cat'] = False
        valid_cells['decrease_si_cat'] = False
        valid_cells['no_change_si_cat'] = False
        valid_cells['increase_fr_cat'] = False
        valid_cells['decrease_fr_cat'] = False
        valid_cells['no_change_fr_cat'] = False
        
        # SI categories
        valid_cells.loc[valid_cells['delta_SI'] >= si_threshold, 'increase_si_cat'] = True
        valid_cells.loc[valid_cells['delta_SI'] <= -si_threshold, 'decrease_si_cat'] = True
        valid_cells.loc[
            (valid_cells['delta_SI'] > -si_threshold) & 
            (valid_cells['delta_SI'] < si_threshold), 
            'no_change_si_cat'
        ] = True
        
        # FR categories (using percent_fr_change column)
        valid_cells.loc[valid_cells['percent_fr_change'] >= fr_threshold, 'increase_fr_cat'] = True
        valid_cells.loc[valid_cells['percent_fr_change'] <= -fr_threshold, 'decrease_fr_cat'] = True
        valid_cells.loc[
            (valid_cells['percent_fr_change'] > -fr_threshold) & 
            (valid_cells['percent_fr_change'] < fr_threshold), 
            'no_change_fr_cat'
        ] = True
        ...
        # Count cells in each single category
        single_category_counts = {}
        for cat in si_fr_categories:
            count = valid_cells[cat].sum()
            single_category_counts[cat] = int(count)
        
        # Print summary
        print(f"\nSelectivity Index & Firing Rate Change Classification:")
        print(f"  SI threshold: ±{si_threshold}")
        print(f"  FR threshold: ±{fr_threshold}%")
        print(f"\n  SI Categories:")
        print(f"    Increase SI (ΔSI ≥ +{si_threshold}): {single_category_counts['increase_si_cat']} cells")
        print(f"    Decrease SI (ΔSI ≤ -{si_threshold}): {single_category_counts['decrease_si_cat']} cells")
        print(f"    No Change SI (|ΔSI| < {si_threshold}): {single_category_counts['no_change_si_cat']} cells")
        print(f"\n  FR Categories:")
        print(f"    Increase FR (ΔFR ≥ +{fr_threshold}%): {single_category_counts['increase_fr_cat']} cells")
        print(f"    Decrease FR (ΔFR ≤ -{fr_threshold}%): {single_category_counts['decrease_fr_cat']} cells")
        print(f"    No Change FR (|ΔFR| < {fr_threshold}%): {single_category_counts['no_change_fr_cat']} cells")
        print(f"\n  Total: {len(valid_cells)} cells")
        
        # NEW: Analyze FR distribution within cells that show INCREASED SI
        increase_si_cells = valid_cells[valid_cells['increase_si_cat']]
        
        if len(increase_si_cells) > 0:
            # Count how many of the increased-SI cells fall into each FR category
            fr_in_increased_si = {
                'increase_fr': int(increase_si_cells['increase_fr_cat'].sum()),
                'decrease_fr': int(increase_si_cells['decrease_fr_cat'].sum()),
                'no_change_fr': int(increase_si_cells['no_change_fr_cat'].sum())
            }
            
            print(f"\n  FR distribution within cells showing INCREASED SI (n={len(increase_si_cells)}):")
            print(f"    Increase FR: {fr_in_increased_si['increase_fr']} cells ({fr_in_increased_si['increase_fr']/len(increase_si_cells)*100:.1f}%)")
            print(f"    Decrease FR: {fr_in_increased_si['decrease_fr']} cells ({fr_in_increased_si['decrease_fr']/len(increase_si_cells)*100:.1f}%)")
            print(f"    No Change FR: {fr_in_increased_si['no_change_fr']} cells ({fr_in_increased_si['no_change_fr']/len(increase_si_cells)*100:.1f}%)")
        
        # Create figure with 2 subplots
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
        
        # === SUBPLOT 1: Single categories (6 bars) ===
        cat_labels = [
            'Increase SI',
            'Decrease SI',
            'No Change SI',
        ...
        cat_counts = [single_category_counts[cat] for cat in si_fr_categories]
        
        # Color code: SI categories in green/red/gray, FR categories in orange shades
        colors = [
            '#70AD47',  # Green - Increase SI
            '#E74C3C',  # Red - Decrease SI
            '#95A5A6',  # Gray - No Change SI
            '#C55A11',  # Orange - Increase FR
            '#ED7D31',  # Light orange - Decrease FR
            '#F4B183'   # Pale orange - No Change FR
        ]
        ...
        ax1.set_ylabel('Number of Cells', fontsize=12, fontweight='bold')
        ax1.set_title('All Cells: Category Distribution', fontsize=13, fontweight='bold')
        ax1.set_xticks(range(len(cat_labels)))
        ax1.set_xticklabels(cat_labels, rotation=45, ha='right', fontsize=10)
        ax1.grid(True, alpha=0.3, axis='y')
        ax1.set_ylim([0, max(cat_counts) * 1.15])
        ...
        # Add legend
        si_patch = plt.Rectangle((0, 0), 1, 1, fc='#70AD47', alpha=0.8)
        fr_patch = plt.Rectangle((0, 0), 1, 1, fc='#C55A11', alpha=0.8)
        ax1.legend([si_patch, fr_patch], 
                  ['Selectivity Index Changes', 'Firing Rate Changes'],
                  loc='upper right', fontsize=9)
        
        # === SUBPLOT 2: FR distribution WITHIN increased-SI cells (3 bars) ===
        if len(increase_si_cells) > 0:
            fr_labels = ['Increase FR', 'Decrease FR', 'No Change FR']
            fr_counts = [
                fr_in_increased_si['increase_fr'],
                fr_in_increased_si['decrease_fr'],
                fr_in_increased_si['no_change_fr']
            ]
            
            # Use FR color scheme
            fr_colors = ['#C55A11', '#ED7D31', '#F4B183']
            
            bars2 = ax2.bar(range(len(fr_labels)), fr_counts, color=fr_colors,
                           edgecolor='black', linewidth=1.5, alpha=0.8, width=0.6)
            
            # Add count and percentage labels
            for bar, count in zip(bars2, fr_counts):
                height = bar.get_height()
                percentage = (count / len(increase_si_cells) * 100) if len(increase_si_cells) > 0 else 0
                ax2.text(bar.get_x() + bar.get_width()/2., height,
                        f'{count}\n({percentage:.1f}%)',
                        ha='center', va='bottom', fontsize=11, fontweight='bold')
            
            ax2.set_ylabel('Number of Cells', fontsize=12, fontweight='bold')
            ax2.set_title(f'Cells with INCREASED SI Only\n(n = {len(increase_si_cells)} cells)',
                         fontsize=13, fontweight='bold')
            ax2.set_xticks(range(len(fr_labels)))
            ax2.set_xticklabels(fr_labels, fontsize=11, fontweight='bold')
            ax2.set_ylim([0, max(fr_counts) * 1.25])
            ax2.grid(True, alpha=0.3, axis='y')
            
            # Add text annotation explaining the breakdown
            total_with_increased_si = len(increase_si_cells)
            ax2.text(0.5, 0.95, 
                    f'Of {total_with_increased_si} cells with increased SI:\n'
                    f'How does their firing rate change?',
                    transform=ax2.transAxes, ha='center', va='top',
                    fontsize=10, style='italic',
                    bbox=dict(boxstyle='round,pad=0.5', facecolor='lightyellow',
                             edgecolor='black', linewidth=1.2, alpha=0.9))
        else:
            # No cells with increased SI
            ax2.text(0.5, 0.5, 'No cells with\nincreased SI',
                    ha='center', va='center', fontsize=14, 
                    transform=ax2.transAxes, fontweight='bold')
            ax2.set_title('Cells with INCREASED SI Only', 
                         fontsize=13, fontweight='bold')
            ax2.axis('off')
        
        # Overall title
        fig.suptitle(f'SI & FR Change Distribution (n = {len(valid_cells)} tuned cells)\n'
                    f'Thresholds: ΔSI ≥ ±{si_threshold}, ΔFR ≥ ±{fr_threshold}%',
                    fontsize=14, fontweight='bold', y=0.98)
        
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        
        # Save figure with clear filename
        output_path = os.path.join(
            self.output_dir, 
            f'SI_and_FR_distribution_thresholds_SI{si_threshold}_FR{fr_threshold}pct.png'
        )
        fig.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
        
        print(f"\n✓ SI & FR distribution chart saved: {output_path}")
    ...
    def _generate_pdfs(
        ...
        for category in categories:
            ...
            output_path = os.path.join(self.output_dir, f'{category}_archT_AC_tuning.pdf')
