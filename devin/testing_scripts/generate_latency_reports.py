...
if TEST:
    ...
    outputDir = os.path.join(settings.CELL_REPORTS_PATH, studyname, 'example_cell/')
    pathtoDB = os.path.join(settings.FIGURES_DATA_PATH, studyparams.STUDY_NAME, 
                                   '{}.h5'.format(d1mice[0]))
    # figFilename = 'figure_{}_latency'.format(d1mice[0])
else:
    d1mice = studyparams.ASTR_D1_CHR2_MICE
    outputDir = os.path.join(settings.CELL_REPORTS_PATH, studyname, 'latency_tests/') 
    pathtoDB = os.path.join(settings.FIGURES_DATA_PATH, studyparams.STUDY_NAME, 
                                   '{}_latencyTest.h5'.format(studyparams.DATABASE_NAME))
    # figFilename = 'figure_{}_latency'.format(studyparams.DATABASE_NAME)
...
if sys.argv[1:] != []:
    ...
    if arguements[0] == 'tuning':
        ...
        outputDir = os.path.join(settings.TEST_PATH, studyname, 'reports_freq_tuned_cells_in_db/')

    elif arguements[0] == 'am':
        celldb = db.query(studyparams.AM_FILTER)
        print("Generating reports for possible synced am cells")
        outputDir = os.path.join(settings.TEST_PATH, studyname, 'reports_am_cells_in_db/')
else:
    celldb = db
...
for indRow, dbRow in celldb.iterrows():
    ...
    if "laserpulse" in sessions:
        ...
        if dbRow.laserpulse_pVal < 0.05 and dbRow.laserpulse_SpikeCountChange > 0:
    ...
    if "tuningCurve" in sessions:
        ...
        if toCalculate:
            ...
            plt.text(500, 0, "Tuning Response/Baseline Ratio = {}".format(dbRow.tuningResponseRatio), fontsize=fontSizeLabels)
            plt.text(500, -100, "Tuning Baseline Rate = {}".format(dbRow.tuningBaseRate), fontsize=fontSizeLabels)
            plt.text(500, -200, "Tuning Onset response rate = {}".format(dbRow.tuningResponseRate), fontsize=fontSizeLabels)
