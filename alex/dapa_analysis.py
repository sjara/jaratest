...
parser.add_argument('--cells', action='store_true', help='specific cell to analyze')
...
#Plot raster for test tuning curves
def tuning_test_raster(bdata, ephysData, gs):
    ...
    title = "Tuning Test (Base FR: {} +/- {} spikes/s)".format(round(base_avg, 2), round(base_sem, 2)) 
...
def tuning_curve(bdata, ephysData, gs):
    ...
    for intInd, inten in enumerate(possibleInts):
        ...
        plt.plot(xpoints, freq_avgs, linestyle = line, color = 'black', marker = 'o', label = curveName)
        plt.fill_between(xpoints, freq_avgs - (freq_sems * 2), freq_avgs + (freq_sems * 2), alpha = 0.1, color = 'black')
    ...
    for count, sessionInd in enumerate(laserSessionInds):
        ...
        for intInd, inten in enumerate(possibleInts):
            for indLaser in possiblelaser:
                ...
                if len(tuningSessionInds) >= 2:
                    ax = plt.subplot(gs[indLaser*5:5+(indLaser*5), intInd + (count * 2) + 2])
                else:
                    ax = plt.subplot(gs[indLaser*5:5+(indLaser*5), intInd + (count * 2) + 1])
                if indLaser == 0:
...
def generate_report(indRow, dbRow):
    ...
    if len(tuningSessionInds) != 0:
        firstTuningSessionInd = True
        for sessionInd in tuningSessionInds:
            try:
                bdata = cell.load_behavior_by_index(sessionInd)
                ephysData = cell.load_ephys_by_index(sessionInd)
                if firstTuningSessionInd:
                    tuning_test_raster(bdata, ephysData, gs)
                    firstTuningSessionInd = False
    ...
    if len(laserTuningSessionInds) != 0:
        '''
        try:
            bdata = cell.load_behavior_by_index(laserTuningSessionInds[0])
            ephysData = cell.load_ephys_by_index(laserTuningSessionInds[0])
    ...
    figname = finaldir + '{}_{}_depth{}_T{}_C{}.png'.format(subject, dbRow['date'], int(dbRow['depth']), dbRow['tetrode'], dbRow['cluster'])
    plt.savefig(figname)
...
finaldir = '/home/jarauser/data/reports_alex/{}/test/'.format(subject)
...
if args.cells:
    ...
    cell_list = [[73,86,126,127,129,182,191],
                [157,159,161,223,227,436,453,454,455,456,
    ...
    dapa_ind = dapa_list.index(subject)

    for cell in cell_list[dapa_ind]:
        dbRow = goodCells.loc[cell]
        generate_report(cell, dbRow)
