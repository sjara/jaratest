...
def laser_tuning_raster(bdata, ephysData, gs):
    ...
    for indLaser in possiblelaser:
        #plt.subplot(2, 1, indLaser)
        plt.subplot(gs[1, indLaser])
        if indLaser == 0:
            laser = "Tuning Curve - No Laser"
        else:
            laser = "Tuning Curve - Laser"
...
for indRow, dbRow in goodCells.iterrows():
    ...
    print("Saving Cell " + str(indRow))
    #figname = '/home/jarauser/data/reports_alex/dapa010/test/{}_{}_depth{}_T{}_C{}.png'.format(dbRow['date'], dbRow['ephysTime'][sessionInd], int(dbRow['depth']), dbRow['tetrode'], dbRow['cluster'])
    figname = '/home/jarauser/data/reports_alex/dapa010/test/{}_depth{}_T{}_C{}.png'.format(dbRow['date'], int(dbRow['depth']), dbRow['tetrode'], dbRow['cluster'])
    plt.savefig(figname)
...
for ind in db.index:
    ...
    if len(thisCell['behavSuffix'])<2:
      continue
    behavFileName = subject + "_" + thisCell['paradigm'][2] + '_' + thisCell['date'] + thisCell['behavSuffix'][2]
    session = thisCell['date'] + '_' + thisCell['ephysTime'][2]
    ...
    figname = '/home/jarauser/data/reports_alex/dapa010/test/' + str(session) + '_depth' + str(depth) + '_T' + str(tetrode) + '_C' + str(cluster)+ '.png'
    plt.savefig(figname)
