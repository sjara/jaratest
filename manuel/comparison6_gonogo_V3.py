...
# --- Find the position of each frequency defined in each "if" statement ---      
     for i in preFreqArr:
...
def normalize_data_videos(pupilArea, pupilArea1, pupilArea2, pupilArea3, pupilArea4, pupilArea5, valuesToNormalize, valuesToNormalize2, valuesToNormalize3, valuesToNormalize4, valuesToNormalize5):
     ''' 
     ...
     '''
# --- Find min values of pupil area for each video ---
...
# --- Create array with all the min values of each pupil area and find the min value within it ---
     
     minArray = np.array([minVal1, minVal2, minVal3, minVal4, minVal5, minVal6])
     minValue = np.amin(minArray)
     
# --- Find max values of pupil area for each video ---     
...
# --- Create array with all the max values of each pupil area and find the max value within it ---     
     
     maxArray = np.array([maxVal1, maxVal2, maxVal3, maxVal4, maxVal5, maxVal6])
     maxValue = np.amax(maxArray) 

# --- Create empty arrays to save data and start normalization operation ---     
     ...
     for j in valuesToNormalize2:  
         substractMin2 = j - minValue  
         newData2 = substractMin/rangeValues  
         listData2.append(newData2)  
         normalizedData2 = np.asarray(listData2) 
     for k in valuesToNormalize3: 
         substractMin3 = k - minValue 
         newData3 = substractMin/rangeValues
         listData3.append(newData3) 
         normalizedData3 = np.asarray(listData3) 
     for m in valuesToNormalize4: 
          substractMin4 = m - minValue 
          newData4 = substractMin/rangeValues 
          listData4.append(newData4) 
          normalizedData4 = np.asarray(listData4) 
     for n in valuesToNormalize5: 
          substractMin5 = n - minValue 
          newData5 = substractMin/rangeValues 
          listData5.append(newData5) 
          normalizedData5 = np.asarray(listData5)
              
     return(normalizedData, normalizedData2, normalizedData3, normalizedData4, normalizedData5)
...
def comparison_plot(time, valuesData1, valuesData2, valuesData3, valuesData4, valuesData5, valuesData6): 
     ''' 
...
#     sp5 = np.round(pVal5, decimals=5)
     ...
     subplt.set_title('Pupil behavior for randomized paired frequencies: pure015 202218-25', fontsize = labelsSize)
     plt.grid(b = True)
     #plt.ylim([300, 800])
...
# --- Plot the results ---
     ...
     plt.yticks(fontsize = labelSize)
     plotVal[0,1].set_ylim([-0.01, 0.12])
     plotVal[1,1].set_ylim([-0.01, 0.12])
...
# --- Reshape arrays so they contain the corresponding rows and columns from the preIndicesTime and indexType variables respectively ---       
...
#     trialPl[4,1].set_ylim([400, 700])
     ...
     plt.suptitle('Pupil size for each frequency type across videos: pure15_20220517-19', fontsize = labelSize)
...
filesDict = {'loadFile1':np.load('./project_videos/mp4Files/mp4Outputs/pure014_20220606_twoChordPreExtreme_229_xconfig1_proc.npy', allow_pickle = True).item(),
	'sessionFile1':'20220606_twoChordPreExtreme_229_xconfig1', 'condition1':'detectiongonogo', 'sound':'ChordTrain', 'name1':'pure015', 
	'loadFile2':np.load('./project_videos/mp4Files/mp4Outputs/pure014_20220606_twoChordPreExtreme_230_xconfig1_proc.npy', allow_pickle = True).item(), 
	'config2':'2Sconfig3', 'sessionFile2':'20220606_twoChordPreExtreme_230_xconfig1', 'name2':'config12_2',
	'loadFile3':np.load('./project_videos/mp4Files/mp4Outputs/pure014_20220606_twoChordPreExtreme_231_xconfig1_proc.npy', allow_pickle = True).item(), 
	'config3':'2Sconfig3', 'sessionFile3':'20220606_twoChordPreExtreme_231_xconfig1', 'name3':'config12_3',
	'loadFile4':np.load('./project_videos/mp4Files/mp4Outputs/pure014_20220606_twoChordPreExtreme_232_xconfig1_proc.npy', allow_pickle = True).item(), 
	'config4':'2Sconfig4', 'sessionFile4':'20220606_twoChordPreExtreme_232_xconfig1', 'name4':'config14_1', 
	'loadFile5':np.load('./project_videos/mp4Files/mp4Outputs/pure014_20220607_twoChordPreExtreme_233_xconfig1_proc.npy', allow_pickle = True).item(), 
	'config5':'2Sconfig4', 'sessionFile5':'20220607_twoChordPreExtreme_233_xconfig1', 'name5':'config14_2', 
	'loadFile6':np.load('./project_videos/mp4Files/mp4Outputs/pure014_20220607_twoChordPreExtreme_234_xconfig1_proc.npy', allow_pickle = True).item(), 
	'config6':'2Sconfig4', 'sessionFile6':'20220607_twoChordPreExtreme_234_xconfig1', 'name6':'config14_3'}
	
scatBarDict = {'title':'Pupil behavior before and after sound stimulus: pure011 20220419', 'savedName':'pure0043ScatbarPlot', 'yLabel':'Mean Pupil Area', 'xLabelTitle':'Conditions', 'plotFreqName':'Pupil size for 5 different frequencies: pure015_20220517-19'}
...
blink2Bool = np.logical_and(blink2 > minBlink, blink2 < 45000) # Boolean values from the blink2 variable where True values will be within the established range.
...
timeRange = np.array([-7, 7]) # Range of time window, 0.5 seconds before the sync signal is on and 2 seconds after is on.
...
preSignal, postSignal, preTimeIndices, postTimeIndices = find_prepost_values(windowTimeVec, windowed_signal, -7, 0, 0, 7)
...
#--- Align trials to the event ---
timeRange1 = np.array([-7,7]) # Range of time window, one second before the sync signal is on and one second after is on. For syncSound [-0.95,0.95] and for controls [-0.6,0.6]
windowTimeVec1, windowed_signal1 = eventlocked_signal(timeVec1, pArea1, timeOfBlink2Event1, timeRange1)

#--- Obtain pupil pre and post stimulus values, and average size ---
preSignal1, postSignal1, preTimeIndices1, postTimeIndices1 = find_prepost_values(windowTimeVec1, windowed_signal1, -7, 0, 0, 7)
...
#--- Align trials to the event ---
timeRange2 = np.array([7,7]) # Range of time window, one second before the sync signal is on and one second after is on. For syncSound [-0.95,0.95] and for controls [-0.6,0.6]
windowTimeVec2, windowed_signal2 = eventlocked_signal(timeVec2, pArea2, timeOfBlink2Event2, timeRange2)

#--- Obtain pupil pre and post stimulus values, and average size ---
preSignal2, postSignal2, preTimeIndices2, postTimeIndices2 = find_prepost_values(windowTimeVec2, windowed_signal2, -7, 0, 0, 7)
...
#--- Align trials to the event ---
timeRange3 = np.array([7,7]) # Range of time window, one second before the sync signal is on and one second after is on. For syncSound [-0.95,0.95] and for controls [-0.6,0.6]
windowTimeVec3, windowed_signal3 = eventlocked_signal(timeVec3, pArea3, timeOfBlink2Event3, timeRange3)

#--- Obtain pupil pre and post stimulus values, and average size ---
preSignal3, postSignal3, preTimeIndices3, postTimeIndices3 = find_prepost_values(windowTimeVec3, windowed_signal3, -7, 0, 0, 7)
...
#--- Align trials to the event ---
timeRange4 = np.array([-7,7]) # Range of time window, one second before the sync signal is on and one second after is on. For syncSound [-0.95,0.95] and for controls [-0.6,0.6]
windowTimeVec4, windowed_signal4 = eventlocked_signal(timeVec4, pArea4, timeOfBlink2Event4, timeRange4)

#--- Obtain pupil pre and post stimulus values, and average size ---
preSignal4, postSignal4, preTimeIndices4, postTimeIndices4 = find_prepost_values(windowTimeVec4, windowed_signal4, -7, 0, 0, 7)
...
#--- Align trials to the event ---
timeRange5 = np.array([-7,7]) # Range of time window, one second before the sync signal is on and one second after is on. For syncSound [-0.95,0.95] and for controls [-0.6,0.6]
windowTimeVec5, windowed_signal5 = eventlocked_signal(timeVec5, pArea5, timeOfBlink2Event5, timeRange5)

#--- Obtain pupil pre and post stimulus values, and average size ---
preSignal5, postSignal5, preTimeIndices5, postTimeIndices5 = find_prepost_values(windowTimeVec5, windowed_signal5, -7, 0, 0, 7)
...
# --- Finding pupil area for each freq type ---
...
#--- Normalized data ---
normVal1, normVal2, normVal3, normVal4, normVal5 = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal22Type, postVal24Type, postVal28Type, postVal216Type, postVal232Type)

normVal6, normVal7, normVal8, normVal9, normVal10 = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal322Type, postVal324Type, postVal328Type, postVal3216Type, postVal3232Type)

normVal1a, normVal2a, normVal3a, normVal4a, normVal5a = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal22Typea, postVal24Typea, postVal28Typea, postVal216Typea, postVal232Typea)

normVal6a, normVal7a, normVal8a, normVal9a, normVal10a = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal322Typea, postVal324Typea, postVal328Typea, postVal3216Typea, postVal3232Typea)

normVal1b, normVal2b, normVal3b, normVal4b, normVal5b = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal22Typeb, postVal24Typeb, postVal28Typeb, postVal216Typeb, postVal232Typeb)

normVal6b, normVal7b, normVal8b, normVal9b, normVal10b = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal322Typeb, postVal324Typeb, postVal328Typeb, postVal3216Typeb, postVal3232Typeb)

normVal1c, normVal2c, normVal3c, normVal4c, normVal5c = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal22Typec, postVal24Typec, postVal28Typec, postVal216Typec, postVal232Typec)

normVal6c, normVal7c, normVal8c, normVal9c, normVal10c = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal322Typec, postVal324Typec, postVal328Typec, postVal3216Typec, postVal3232Typec)

normVal1d, normVal2d, normVal3d, normVal4d, normVal5d = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal22Typed, postVal24Typed, postVal28Typed, postVal216Typed, postVal232Typed)

normVal6d, normVal7d, normVal8d, normVal9d, normVal10d = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal322Typed, postVal324Typed, postVal328Typed, postVal3216Typed, postVal3232Typed)

normVal1e, normVal2e, normVal3e, normVal4e, normVal5e = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal22Typee, postVal24Typee, postVal28Typee, postVal216Typee, postVal232Typee)

normVal6e, normVal7e, normVal8e, normVal9e, normVal10e = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, postVal322Typee, postVal324Typee, postVal328Typee, postVal3216Typee, postVal3232Typee)



normPreVal1, normPreVal2, normPreVal3, normPreVal4, normPreVal5 = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal22Type, preVal24Type, preVal28Type, preVal216Type, preVal232Type)

normPreVal6, normPreVal7, normPreVal8, normPreVal9, normPreVal10 = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal322Type, preVal324Type, preVal328Type, preVal3216Type, preVal3232Type)

normPreVal1a, normPreVal2a, normPreVal3a, normPreVal4a, normPreVal5a = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal22Typea, preVal24Typea, preVal28Typea, preVal216Typea, preVal232Typea)

normPreVal6a, normPreVal7a, normPreVal8a, normPreVal9a, normPreVal10a = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal322Typea, preVal324Typea, preVal328Typea, preVal3216Typea, preVal3232Typea)

normPreVal1b, normPreVal2b, normPreVal3b, normPreVal4b, normPreVal5b = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal22Typeb, preVal24Typeb, preVal28Typeb, preVal216Typeb, preVal232Typeb)

normPreVal6b, normPreVal7b, normPreVal8b, normPreVal9b, normPreVal10b = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal322Typeb, preVal324Typeb, preVal328Typeb, preVal3216Typeb, preVal3232Typeb)

normPreVal1c, normPreVal2c, normPreVal3c, normPreVal4c, normPreVal5c = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal22Typec, preVal24Typec, preVal28Typec, preVal216Typec, preVal232Typec)

normPreVal6c, normPreVal7c, normPreVal8c, normPreVal9c, normPreVal10c = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal322Typec, preVal324Typec, preVal328Typec, preVal3216Typec, preVal3232Typec)

normPreVal1d, normPreVal2d, normPreVal3d, normPreVal4d, normPreVal5d = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal22Typed, preVal24Typed, preVal28Typed, preVal216Typed, preVal232Typed)

normPreVal6d, normPreVal7d, normPreVal8d, normPreVal9d, normPreVal10d = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal322Typed, preVal324Typed, preVal328Typed, preVal3216Typed, preVal3232Typed)

normPreVal1e, normPreVal2e, normPreVal3e, normPreVal4e, normPreVal5e = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal22Typee, preVal24Typee, preVal28Typee, preVal216Typee, preVal232Typee)

normPreVal6e, normPreVal7e, normPreVal8e, normPreVal9e, normPreVal10e = normalize_data_videos(pArea, pArea1, pArea2, pArea3, pArea4, pArea5, preVal322Typee, preVal324Typee, preVal328Typee, preVal3216Typee, preVal3232Typee)
...
OverLapPlots = comparison_plot(pupilDilationTimeWindowVec, pAreaDilatedMean,  pAreaDilatedMean1, pAreaDilatedMean2, pAreaDilatedMean3, pAreaDilatedMean4, pAreaDilatedMean5)
