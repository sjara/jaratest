...
def comparison_plot(time, valuesData1, valuesData2, valuesData3): 
     ...
     plt.title('Comparison of pupil behavior in different conditions: pure003_20210928_01')
     plt.grid(b = True)
     plt.ylim([300, 800]) 
     plt.legend()
     plt.show() 
...
def scatter_plots(preValues1, postValues1, preValues2, postValues2, preValues3, postValues3): 
     ...
     plt.suptitle('Average trials signals behavior during session 01: pure003_20210928')
     plt.show() 
     return(plt.show())
...
def bar_plots(firstPlotMeanValues1, firstPlotMeanValues2, xlabel1, xlabel2, firstPlotStdData1, firstPlotStdData2, secondPlotMeanValues1, secondPlotMeanValues2, secondPlotStdData1, secondPlotStdData2, thirdPlotMeanValues1, thirdPlotMeanValues2, thirdPlotStdData1, thirdPlotStdData2):
     ...
     barPlots[0].errorbar(xlabels, barMeanValues1, yerr = stdErrors1, capsize=5,  alpha=0.5, ecolor = 'black') 
     barPlots[0].set(title = 'Positive control data', ylabel = 'Mean Pupil Area') 
     barPlots[1].bar(xlabels, barMeanValues2, yerr = stdErrors2) 
     barPlots[1].errorbar(xlabels, barMeanValues2, yerr = stdErrors2, capsize=5,  alpha=0.5, ecolor = 'black') 
     ...
     barPlots[2].errorbar(xlabels, barMeanValues3, yerr = stdErrors3, capsize=5,  alpha=0.5, ecolor = 'black') 
     barPlots[2].set(title = 'chordTrain data')  
     plt.ylim(200, 700)
     plt.suptitle('Pupil behavior before and after stimulus: pure003_20210928')
     plt.show() 
     return(plt.show())
...
timeRange = np.array([-0.68, 0.68]) # Range of time window, before the sync signal is on and at the onset of it.
...
proc2 = np.load('./project_videos/projectOutputs/pure003_20210928_syncSound_01_proc.npy', allow_pickle = True).item()
#Note: the proc.npy is the output file generated from facemap.
...
#--- Align trials to the event ---
timeRange2 = np.array([-0.3, 0.3]) # Range of time window, one second before the sync signal is on and one second after is on. For syncSound [-0.95,0.95] and for controls [-0.6,0.6]
windowTimeVec2, windowed_signal2 = eventlocked_signal(timeVec2, pArea2, timeOfBlink2Event2, timeRange2)
preSignal2 = windowed_signal2[0:9] # Takes the values of the pArea between timeRange within the time window. [0:28] for experimental and [0:18] for controls
postSignal2 = windowed_signal2[9:18] # Takes the values of the pArea between timeRange within the time window. [28:57] for experimental and [18:36] for controls
