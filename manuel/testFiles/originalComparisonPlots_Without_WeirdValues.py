...
def scatter_plotting(preStimulusValuesArray, postStimulusValuesArray):
     '''
     ...
     dataToPlot = [preStimulusValuesArray, postStimulusValuesArray] 
...
maxValuesIndices1 = np.where(windowed_signal1 == np.amax(windowed_signal1)) #Finds the indices of the max value in the windowed_trials array
correctedWindowedSignal1 = np.delete(windowed_signal1, maxValuesIndices1, axis = 1) #Eliminates the max value in each column(trial) in the array provided.
...
#---obtain mean trial values pre and post signal, plot slope plot---
preSignal1 = correctedWindowedSignal1[0:18] # Takes the values of the pArea between timeRange within the time window. [0:28] for experimental and [0:18] for controls
postSignal1 = correctedWindowedSignal1[18:36] # Takes the values of the pArea between timeRange within the time window. [28:57] for experimental and [18:36] for controls
...
maxPupilValuesIndices1 = np.where(pAreaDilated1 == np.amax(pAreaDilated1)) #Finds the indices of the max value in the windowed_trials array
correctedPupilWindowedSignal1 = np.delete(pAreaDilated1, maxPupilValuesIndices1, axis = 1) #Eliminates the max value in each column(trial) in the array provided.


pAreaDilatedMean1 = correctedPupilWindowedSignal1.mean(axis = 1)
...
maxValuesIndices2 = np.where(windowed_signal2 == np.amax(windowed_signal2)) #Finds the indices of the max value in the windowed_trials array
correctedWindowedSignal2 = np.delete(windowed_signal2, maxValuesIndices2, axis = 1) #Eliminates the max value in each column(trial) in the array provided.
...
#---obtain mean trial values pre and post signal, plot slope plot---
preSignal2 = correctedWindowedSignal2[0:28] # Takes the values of the pArea between timeRange within the time window. [0:28] for experimental and [0:18] for controls
postSignal2 = correctedWindowedSignal2[28:57] # Takes the values of the pArea between timeRange within the time window. [28:57] for experimental and [18:36] for controls
...
maxPupilValuesIndices2 = np.where(pAreaDilated2 == np.amax(pAreaDilated2)) #Finds the indices of the max value in the windowed_trials array
correctedPupilWindowedSignal2 = np.delete(pAreaDilated2, maxPupilValuesIndices2, axis = 1) #Eliminates the max value in each column(trial) in the array provided.


pAreaDilatedMean2 = correctedPupilWindowedSignal2.mean(axis = 1)
...
def multiple_plots(xValues1, yValues1, xValues2, yValues2, xValues3, yValues3): 
     xLabelling = ['Pre stimulus onset', 'Post stimulus onset'] 
     dataToPlot1 = [xValues1, yValues1] 
     dataToPlot2 = [xValues2, yValues2] 
     dataToPlot3 = [xValues3, yValues3]  
     fig, scatterPlots = plt.subplots(3, constrained_layout = True, sharey = True) 
     scatterPlots[0].plot(xLabelling, dataToPlot1, marker = 'o', linewidth = 1) 
     scatterPlots[0].set(title = 'Average trials signals behavior during session 01: positive control') 
     scatterPlots[1].plot(xLabelling, dataToPlot2, marker = 'o', linewidth = 1) 
     scatterPlots[1].set(title = 'Average trials signals behavior during session 01: negative control') 
     scatterPlots[2].plot(xLabelling, dataToPlot3, marker = 'o', linewidth = 1) 
     scatterPlots[2].set(title = 'Average trials signals behavior during session 01: experimental') 
     plt.show() 
     return(plt.show())

scatterPlots = multiple_plots(averagePreSignal, averagePostSignal, averagePreSignal1, averagePostSignal1, averagePreSignal2, averagePostSignal2)
