...
# -*- coding: utf-8 -*-
'''
MODIFIED BY BILLY WALKER
used for allcells with cluster quality and tuning ephys
'''
...
class EphysSessionInfo(object):
     def __init__(self, animalName, tuningSession, ephysSession, tuningBehavior, behavSession, depth,#########################
                  clustersEachTetrode={}, clusterQuality={}, soundResponse={}, trialsToExclude=[]):
         '''
         ...
         self.tuningSession = tuningSession##########################################
         self.ephysSession = ephysSession
         self.tuningBehavior = tuningBehavior######################################
...
class CellInfo(object):
    ...
    '''
    def __init__(self, animalName, tuningSession, ephysSession, tuningBehavior, behavSession, tetrode, cluster, depth, quality,#############################
                 trialsToExclude=[]):
        # -- Basic info --
        self.animalName = animalName
        self.tuningSession = tuningSession##########################################
        ...
        for tetrode in sorted(sessionInfo.clustersEachTetrode.keys()):
            for cluster in sessionInfo.clustersEachTetrode[tetrode]:
                oneCell = CellInfo(animalName = sessionInfo.animalName,
                                   tuningSession = sessionInfo.tuningSession,########################################
                                   ephysSession = sessionInfo.ephysSession,
                                   tuningBehavior = sessionInfo.tuningBehavior,######################################
                                   behavSession = sessionInfo.behavSession,
                                   tetrode = tetrode,
                                   cluster = cluster,
                                   quality = sessionInfo.clusterQuality[tetrode],#################NEED THIS HERE FOR SEARCH?????
                                   depth = sessionInfo.depth,###################################################3
                                   #response = sessionInfo.soundResponse[tetrode],
                                   trialsToExclude = [])
