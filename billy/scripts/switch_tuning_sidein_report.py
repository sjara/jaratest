'''
raster and histogram for switching task ALIGNED TO SOUND AND CENTER POKE OUT
...
reportname = 'switch_report_sidein_movement'
outputDir = '/home/billywalker/Pictures/switching_tuning_sidein_reports/'
...
sideinTimeRange = [-0.5,0.5]#In secs. Time range for rastor plot to plot spikes (around side in port as 0)
...
def main():
    ...
    global spikeTimesFromSideinOnset
    global indexLimitsEachSideinTrial
    global titleText

    print "switch_tuning_sidein_report"
    for cellID in range(0,numOfCells):
        ...
        try:
            if (behavSession != oneCell.behavSession):
                ...
                ###############################################################################
                #################THIS IS FOR SIDE IN PORT######################################
                sideInTimes = bdata['timeSideIn'] #This is the times that the mouse goes in the side port
                timeDiff = sideInTimes - soundStartTimes
                if (len(eventOnsetTimes) < len(timeDiff)):
                    timeDiff = timeDiff[:-1]
                    eventOnsetTimesSide = eventOnsetTimes + timeDiff
                elif (len(eventOnsetTimes) > len(timeDiff)):
                    eventOnsetTimesSide = eventOnsetTimes[:-1] + timeDiff
                else:
                    eventOnsetTimesSide = eventOnsetTimes + timeDiff
                ###############################################################################
                ###############################################################################
            ...
            #################THIS IS FOR SIDE IN PORT######################################
            (spikeTimesFromSideinOnset,sideinTrialIndexForEachSpike,indexLimitsEachSideinTrial) = \
                spikesanalysis.eventlocked_spiketimes(spkTimeStamps,eventOnsetTimesSide,sideinTimeRange)
            ...
            plt.title('Top: movement aligned, Bottom: side-in port aligned')
            ...
            ax7.axvline(x=0, ymin=0, ymax=1, color='b')
...
def hist_sound_allFreq_switching(ax):
    rightward = bdata['choice']==bdata.labels['choice']['right']
    leftward = bdata['choice']==bdata.labels['choice']['left']
    invalid = bdata['outcome']==bdata.labels['outcome']['invalid']
    correct = bdata['outcome']==bdata.labels['outcome']['correct']
    ...
    extraplots.plot_psth(spikeCountMat/binWidth,smoothWinSize,timeVec,trialsEachCond=trialsEachCond,
                         colorEachCond=colorEachCond,linestyle=None,linewidth=2,downsamplefactor=1)
    ax.axvspan(0.0, 0.1, color=[0.5,0.5,0.5], alpha=0.5, lw=0)
...
def modulation_index_switching():
    ...
    titleText = 'Mod Index: '+str(round(modIDict[behavSession][clusterNumber],3))+', (p='+str(round(modSigDict[behavSession][clusterNumber],3))+'), Mod Direction Score: '+str(modDirectionScoreDict[behavSession][clusterNumber])
...
def raster_sidein_block_switching():
    ...
    extraplots.raster_plot(spikeTimesFromSideinOnset,indexLimitsEachSideinTrial,sideinTimeRange,trialsEachCond=trialsEachCond,colorEachCond=colorEachBlock,fillWidth=None,labels=None)
    
    #plt.ylabel('Trials')
    #plt.title('Blocks Movement Aligned')

def hist_sidein_block_switching():
    correct = bdata['outcome']==bdata.labels['outcome']['correct']
    ...
    if bdata['currentBlock'][0]==bdata.labels['currentBlock']['low_boundary']:
        ...
        colorEachBlock = 4*['r','g']
    timeVec = np.arange(sideinTimeRange[0],sideinTimeRange[-1],binWidth)
    spikeCountMat = spikesanalysis.spiketimes_to_spikecounts(spikeTimesFromSideinOnset,indexLimitsEachSideinTrial,timeVec)
