...
print 'WARNING: THIS ISI CALCULATIONS FIND THE ISI FOR THE BEHAVIOR SESSION, NOT THE BEHAVIOR AND TUNING TOGETHER'
for mouseName in mouseNameList:
    allcellsFileName = 'allcells_'+mouseName
    sys.path.append(settings.ALLCELLS_PATH)
    allcells = importlib.import_module(allcellsFileName)

    outputDir = ephysRootDir
    nameOfFile = 'ISI_Violations'
    finalOutputDir = outputDir+'/'+mouseName+'_processed'
    ...
    ISIDict = nestedDict()
    ISIList = [] #List of behavior sessions that already have ISI values calculated
    ...
    for badSes in badSessionList:
        print badSes
    print 'finished ISI violation check for', mouseName
