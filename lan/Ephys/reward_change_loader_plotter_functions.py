...
def plot_rew_change_per_cell_raster_psth(behavSession, ephysSession, tetrode, cluster, freqToPlot='low', byBlock=False, colorCondDict=colorDictRC, alignment='sound', timeRange=[-0.3,0.4], binWidth=0.010):
    ...
    if freqToPlot == 'low' or freqToPlot=='high':
        ...
        for freq in ['low','high']:
            _, trialsEachCondThisFreq, colorEachCondThisFreq = get_trials_each_cond_reward_change(behavSession, ephysSession, tetrode, cluster, freqToPlot=freq, byBlock=byBlock, colorCondDict=colorCondDict)
            trialsEachCondList.appen(trialsEachCondThisFreq)
