'''
This script includes functions to load ephys (cluster files) and behavior data for a cell of interest from the switching and psychometric curve mice (stored in jarastore).
Also functions to plot tuning raster/psth and differently-aligned 2afc rasters/psths.
...
def get_trials_each_cond_switching(oneCell, freqToPlot='middle', byBlock=True):
    ...
    if freqToPlot == 'middle':
        ...
        if byBlock:
            ...
            if bdata['currentBlock'][0]==bdata.labels['currentBlock']['low_boundary']:
                colorEachCond = 5*[colorsDict['colorL'],colorsDict['colorR']] #assume there are not more than 5 blocks
            else:
                colorEachCond = 5*[colorsDict['colorR'],colorsDict['colorL']]
    ...
    # -- When plotting all 3 frequencies will not be plotting by block, just plot by type of block (low_boundary vs high_boundary) -- #
        ...
        colorEachCond = ['y',colorsDict['colorL'],colorsDict['colorR'],'b']
