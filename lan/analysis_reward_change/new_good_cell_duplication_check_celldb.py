...
def get_all_waveforms_behav_session(celldb, wavesize, sessionToUse='behavior'):
    numCells = len(celldb)
...
def find_within_session_duplicates(celldb, threshold):
    ...
    for (date,cellsThisSess) in sameSessionCellsDict.items():
        waveformsThisSess = get_all_waveforms_behav_session(cellsThisSess, wavesize=160)
...
def find_cross_session_duplicates(celldb, threshold):
    ...
    for (pairOfDates,[cellsSess1,cellsSess2]) in consecutiveSessCellsDict.items():
        waveformsSess1 = get_all_waveforms_behav_session(cellsSess1, wavesize=160)
        waveformsSess2 = get_all_waveforms_behav_session(cellsSess2, wavesize=160)
        ...
        if np.any(dupCellsInds):
            ...
            if excludeDf.ix[cellToKeepIndex, 'duplicate_cross_keep'] == 0:  #cell with biggest sound response has not been compared before
