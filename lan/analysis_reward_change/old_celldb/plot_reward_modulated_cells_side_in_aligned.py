...
for indRegion, animal in enumerate(animalList):
    ...
    for indw, modWindow in enumerate(modulationWindows['side-in']):
        ...
        goodCellModSigLeft = goodQualCells[leftModSigName]
        goodCellModSigRight = goodQualCells[rightModSigName]
        sigModulatedLeft = goodCellModSigLeft < alphaLevel
        sigModulatedRight = goodCellModSigRight < alphaLevel
        sigModLeftCells = goodQualCells[sigModulatedLeft]
        sigModRightCells = goodQualCells[sigModulatedRight]
        
        outputDir = '/home/languo/data/ephys/reward_change_stats/modulated_cells_reports/mod_{}_side-in'.format(modWindow)
        ...
        for freq, cells in zip(freqLabels, [sigModLeftCells, sigModRightCells]):
            for indc, cell in cells.iterrows():
                if freq == 'Low':
                    ...
                    indf = -1
                modIndName = 'modInd'+freq+'_'+modWindow+'_side-in'
                modSigName = 'modSig'+freq+'_'+modWindow+'_side-in'
                ...
                figname = '{}_{}_T{}_c{}_{}_{}_side-in.png'.format(animal,date,tetrode,cluster, freq, modWindow)
                ...
                alignment = 'side-in' #modWindow.split('_')[1]
                ax2 = plt.subplot(gs00[0:2, :])
                rcfuncs.plot_reward_change_raster(animal, rcBehavThisCell, rcEphysThisCell, tetrode, cluster, freqToPlot=freq.lower(), byBlock=True, alignment=alignment, timeRange=[-0.8,0.5])
                ...
                rcfuncs.plot_reward_change_psth(animal, rcBehavThisCell, rcEphysThisCell, tetrode, cluster, freqToPlot=freq.lower(), byBlock=True, alignment=alignment, timeRange=[-0.8,0.5], binWidth=0.010)
