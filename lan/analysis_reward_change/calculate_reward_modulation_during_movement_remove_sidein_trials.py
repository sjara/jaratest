...
parser.add_argument('--DBNAME', type=str, required=True, help="Enter the name of the mouse to process")
...
if CASE == 'calculate':
    ...
    databaseFullPath = os.path.join(settings.DATABASE_PATH, NEW_DATABASE_FOLDER, dbName)
    outFilename = '/var/tmp/reward_change_modulation_{}.h5'.format(processedDate)
    ...
    for chunk in chunks:
        ...
        for (alignment, countTimeRange) in conditions:
            ...
            modulationDict.update({'modIndLow_'+window+'_removedsidein': [],
                                   'modSigLow_'+window+'_removedsidein': [],
                                   'modDirLow_'+window+'_removedsidein': [],
                                   'modIndHigh_'+window+'_removedsidein': [],
                                   'modSigHigh_'+window+'_removedsidein': [],
                                   'modDirHigh_'+window+'_removedsidein': []})
        ...
        for ind,cell in chunk.iterrows():
            ...
            for (alignment, countTimeRange) in conditions:
                ...
                for indf,freq in enumerate([lowFreq, highFreq]):
                    ...
                    modulationDict['modInd{}_'.format(freqLabels[indf])+window+'_removedsidein'].append(modIndex) 
                    modulationDict['modSig{}_'.format(freqLabels[indf])+window+'_removedsidein'].append(modSig)
                    modulationDict['modDir{}_'.format(freqLabels[indf])+window+'_removedsidein'].append(modDir)
...
if CASE == 'merge':
    ...
    outFilePath = '/var/tmp/'
    outFilenames = [os.path.join(outFilePath,filename) for filename in os.listdir(outFilePath) if 'reward_change_modulation' in filename]

    fullDb = celldatabase.load_hdf(databaseFullPath) #pd.read_hdf(databaseFullPath,key='reward_change')
    ...
    dfAll = reduce(lambda left,right: pd.merge(left,right,on=['subject','date','tetrode','cluster'],how='inner'), dfs)
    dfAll.drop_duplicates(['subject','date','tetrode','cluster','depth'],inplace=True)
