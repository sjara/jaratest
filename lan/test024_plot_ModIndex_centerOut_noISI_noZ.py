...
modIFilename = os.path.join(processedDir,'modIndex_alignedCenterOut_'+subject+'.txt')
modSFilename = os.path.join(processedDir,'modSig_alignedCenterOut_'+subject+'.txt')
...
figformat = 'png'
filename = 'modIndex_%s_center_out.%s'%(subject,figformat)
...
sigModI_file = open('%s/%s.txt' % (fulloutputDir,'sigMod_centerOut'), 'w')
for (key,value) in sorted(sigModIDict.items()):
    sigModI_file.write('%s:' %key)
    sigModI_file.write('%d %s %f\n' %(value[0],value[1],value[2]))
sigModI_file.close()

####Make plots of modulated cells and save them to the stats folder in their own 'center-out modulated' folder#####
numOfModulatedCells = len(modulatedCellDB)
for cellID in range(0,numOfModulatedCells):
    oneCell = modulatedCellDB[cellID]
    #####make new plots######
    dstDir = processedDir+'/all_cells_CenterOut_sig_modulated/'
