...
if CASE == 1:
    ...
    animalListByTask = {'switching':['test059','test017','test089','adap020'],
                        'psychometric':['adap013','adap017','adap015','test053','test055']}
    # -- Add metadata about which script generated the database to databases
    import pandas as pd
    import os
    for task,animalList in animalListByTask.items():
        for animal in animalList:
            scriptFullPath = os.path.realpath(__file__)
            waveformData = pd.read_hdf('/home/languo/data/ephys/{0}_summary_stats/{1}_{2}_waveforms.h5'.format(task,animal,task),key=animal+'_'+task)
            waveformData['script'] = scriptFullPath
            waveformData.to_hdf('/home/languo/data/ephys/{0}_summary_stats/{1}_{2}_waveforms.h5'.format(task,animal,task),key=animal+'_'+task)
