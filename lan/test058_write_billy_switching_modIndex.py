...
Finds modulation index for all cells (now not checking cell quality) for ONLY the middle frequency in the switching task. Comparing response to one frequency going left versus going right.
...
#countTimeRange is the time range in which to count spikes for calculating modulation index
if sys.argv[2]=='0':
...
Frequency = 1 #This chooses the middle frequency to look at, numbered out of possible frequencies starting with the lowest frequency as number 0
...
###################Choose time window to calculate mod Index#######################
    ...
    # -- Open modI/modS text files to check sessions alreay calculated or append to file new sessions -- #
    ...
    # -- Adhering to Billy's analysis, switching animals will only have one modI file which has modulation index, modulation significance, and modulation direction information in it. -- #
    '''
    ...
    for cellID in range(0,numOfCells):
        ...
        if (behavSession != oneCell.behavSession):
            ...
            if behavSession != '':
                ...
                for modIInd in modIDict[behavSession]:
                    modI_file.write("%s," % modIInd)
                modI_file.write("\n")

                modI_file.write("modSig:")
                for modSigInd in modSigDict[behavSession]:
                    modI_file.write("%s," % modSigInd)
                modI_file.write("\n")

                modI_file.write("modDir:")
                for modDirectInd in modDirectionScoreDict[behavSession]:
            ...
            # -- Only calculate Modulation index for the middle frequency
