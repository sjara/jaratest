'''
Modified from Billy's script addMinPerformancePsyCurve.
...
nameOfFile = 'minPerformance.txt'
finalOutputDir = outputDir+'/'+subject+'_stats'
minPerfList = []

minPer_file = os.path.join(finalOutputDir, nameOfFile)
if os.path.isfile(minPer_file):
    text_file = open(minPer_file, 'r+') #open a text file to read and write in
    text_file.readline() #first line contains information about minimal performance criterion
    minPerfList=text_file.read().split() 
else:
    text_file = open(minPer_file, 'w') #open a text file to read and write in
